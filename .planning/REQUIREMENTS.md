...
- [x] **SCORE-10** (Phase 24 — 감점 엔진 교체): `vision_veto.SEVERITY_CAP` + `apply_downward_cap` + `SEVERITY_CAP_PROVENANCE`(severity→고정천장 밴드) 가 제거되고, `점수 = baseline(100) − Σ(criterion별 측정편차×명시규칙 감점)` 투명 tally 엔진(`deduction_engine.tally`)으로 교체된다. 최종점수의 유일한 clamp 는 `max(0, …)` — `min(100, …)` 또는 severity→고정천장 금지. 엔진은 `_apply_vision_veto` seam 한 곳에서 통합(분기 0, 코드 1벌). (출처: 24-CONTEXT.md ND-01 / [[scoring-must-be-transparent-deduction-tally]]) **(2026-09-18 판정, quick-260918-qm2: 구현 완료 — `deduction_engine.tally` 실재(:268), `vision_veto.SEVERITY_CAP`·`apply_downward_cap` 코드에서 소멸. Pending 표기는 누락이었다.)**
...
> **2026-09-18 (quick-260918-qm2):** Pending 행은 아무도 열어보지 않았다. SCORE-10 만 코드 실측으로
> Complete 정정, 나머지는 "재판정 대상" 표시만 — 전수 판정은 범위 밖.

| Requirement | Phase | Status |
|-------------|-------|--------|
| POSE-01 | Phase 1 | Pending — 재판정 대상(2026-09-18, 미검토) |
| POSE-02 | Phase 1 | Pending — 재판정 대상(2026-09-18, 미검토) |
| BODY-01 | Phase 2 | Pending — 재판정 대상(2026-09-18, 미검토) |
| BODY-02 | Phase 3 | Complete |
| POSE-03 | Phase 4 | Complete |
| SCORE-01 | Phase 5 | Pending — 재판정 대상(2026-09-18, 미검토) |
...
| FEED-01 | Phase 12 | Pending — 재판정 대상(2026-09-18, 미검토) |
| VIS-01 | Phase 12 | Pending — 재판정 대상(2026-09-18, 미검토) |
| PERS-03 | Phase 13 | Pending — 재판정 대상(2026-09-18, 미검토) |
...
| SCORE-09 | Phase 20 → Phase 24 (generalization gate body) | Pending — belle 지시로 열림(STATE 20-04/SCORE-09 ownership 노트). 잔여 = Phase 18 sensitivity 셋 |
| SCORE-10 | Phase 24 | Complete (2026-09-18 코드 실측) |
| SCORE-11 | Phase 24 | Pending — 재판정 대상(2026-09-18, 미검토) |
| SCORE-12 | Phase 24 | Pending — 재판정 대상(2026-09-18, 미검토) |
| SCORE-13 | Phase 24 | Pending — 재판정 대상(2026-09-18, 미검토) |
| SCORE-14 | Phase 24 | Pending — 재판정 대상(2026-09-18, 미검토) |
| SCORE-15 | Phase 24 | Pending — 재판정 대상(2026-09-18, 미검토) |
...
| DELIV-01 | Phase 15 | Pending — 재판정 대상(2026-09-18, 미검토) |
| SCORE-05 | Phase 16 | Pending — 재판정 대상(2026-09-18, 미검토) |
| TERM-01 | Phase 16 | Pending — 재판정 대상(2026-09-18, 미검토) |
| TERM-DATA-01 | Phase 16 | Pending — 재판정 대상(2026-09-18, 미검토) |
| TERM-COPY-01 | Phase 16 | Pending — 재판정 대상(2026-09-18, 미검토) |
...
*Updated 2026-09-18 (quick-260918-qm2) — SCORE-10 을 Pending → Complete 로 정정(코드 실측: `deduction_engine.tally` 실재, `vision_veto.SEVERITY_CAP`·`apply_downward_cap` 소멸). SCORE-09 는 belle 지시로 열려 있음을 표에 명시. 나머지 Pending 17행은 "재판정 대상" 표시만 — 전수 판정은 이번 범위 밖. 근거 = .planning/quick/260918-qm2-doc-truth-repair-18/260918-qm2-FINDINGS.md*
