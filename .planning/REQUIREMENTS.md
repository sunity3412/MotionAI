...
- **PERS-01**: 체형별 개인화 코칭 ("내 몸 기준 어디에 힘을 줘야 하는지"). 접근: ① 영상 기반 자동 정규화(내 몸 비율·가동범위 대비, 거의 무료) ② 멀티모달 Gemini 추론(돈 OK — 가장 유효). 자가입력(키/몸무게/유연성)은 부정확 → 보조. **리서치 스파이크로 ①+②를 먼저 검증한 뒤 v1/v2 확정.** ②는 Phase 1과 같은 Gemini API 키 필요.
