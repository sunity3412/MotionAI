...
갈 수 없다** ②교사가 Gemini 라 **증류만으로는 선생을 못 넘는다**(천장 = Gemini).

#### 층을 넷으로 가른다 — 셋은 모델, 하나는 모델이면 안 된다

```
1. 몸      관절 표현 · 결함 문법("견갑이 솟는다", "관절이 무너진다")
           종목 무관 · 제일 오래 감 · 다음 종목에 그대로 감   ← 지금 없음. 이 phase 의 본체
...
증류만으로는 Gemini 를 못 넘는다. 우리 스택에서 Gemini 가 모르는 걸 아는 건 **사람 눈**뿐이다.
...
**필요 데이터 (우선순위 — belle 만 구할 수 있는 것):**
1. ★★★★ **같은 동작을 여러 방식으로 틀린 영상.** 지금 동작당 실수 1편뿐이라 판정 지점이
   그 한 편에 과적합된다. 2~3가지로 틀린 것이 있어야 **공통으로 갈리는 관절**이 나온다.
2. ★★★★ **강사 ○× — 부위와 시점만.** 점수 라벨 금지는 그대로 지킨다(아래 금지 사항).
3. ★★★ **같은 동작을 다른 사람(강사 2~3명)이.** 지금 기준이 정은지 한 사람이라
   "동작의 본질"과 "정은지 개인의 버릇"이 안 갈린다.
4. ★★★ 학생 영상(요청해 둠).
...
`JOINT_KEYS` 8개가 전부 관절 굽힘각이다. 데이터를 더 넣어서 되는 게 아니라 **몸을 무엇으로
표현하느냐**의 문제라 1층 설계에서 같이 푼다.

**Success Criteria**:

1. **층 분리 실증** — 몸 층 산출물이 종목 이름(`ref-*`) 없이 표현된다. 폴 데이터로 학습한 몸 층이
   다른 종목 영상 1편에서 결함 어휘를 낸다(정확도 아닌 **작동** 확인).
2. **판정 지점 자동 발견** — 정타/실수 쌍에서 갈리는 관절을 코드가 찾아낸다. 2026-09-22 에
   사람이 손으로 낸 결과(kip-up = 왼어깨)를 **재현**하는 것이 최소 기준.
3. **kip-up 변별 회복** — 정타 100 / 실수 <100. 단 **허용오차를 낮춰서 달성하면 실패**(커브핏).
4. **강사 판정 유입 경로** — 강사가 짚은 부위가 데이터로 들어와 학습 재료가 되는 경로가 돈다
   (수집→저장→학습 투입까지 1건이라도 완주).
5. **베이스 교체 내성** — 베이스 모델을 바꿔도 같은 책으로 재학습해 동등 이상이 나온다(B 검증).
6. **채점 투명성 무회귀** — 위 전부를 하고도 감점 내역·출처 표시가 그대로다.

**Depends on:** 실증(2026-10 중순) 이후 착수 — belle 2026-09-19 *"실증까지 분석만"*.
단 **첫 실행 자리(kip-up 에서 `reference_relative` 가 왜 침묵하나)는 분석 정확도 직결이라
실증 전에도 값이 있다.** Phase 22(재학습 파이프라인)·Phase 34(일반화) 위에 얹힌다.
