...
- [x] 22-07-PLAN.md — QLoRA SFT + 16-bit 병합→AWQ + D-15 assert_gates 판정 (Wave 4, FT-04)
...
- [x] 22-11-PLAN.md — 연속 수집 watch 러너(D-09 재사용) + collection_batches 배치 원장(마감 무결성 공존) + FLYWHEEL-RUNBOOK §1 (Wave 8, FT-02)

**Wave 9** *(blocked on Wave 8 completion)*

- [x] 22-12-PLAN.md — 주기 재학습 배치 루프(신규만 라벨→병합 조립→SFT→D-15 게이트→래칫 승격) + 런북 §2 flashinfer env 박제 (Wave 9, FT-03/FT-04)
...
- [x] 26-04-PLAN.md — 카톡 압축본 경고 다이얼로그 + not_pole 구도 안내 (wave 2)
...
- [x] 27-01-PLAN.md — stage-timing 계측 + timingsMs + fake genai fixture (wave 1, pod-free)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 27-02-PLAN.md — [POD] cold baseline before 실측 (wave 2, checkpoint)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 27-03-PLAN.md — GeminiFileSession 코어 + client.py 핸들 주입 + 누수 0 테스트 (wave 3)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 27-04-PLAN.md — 핸들 세션 소비처 배선 + still PNG inline + _process 배선 (wave 4)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 27-05-PLAN.md — prefetch 겹치기 + veto fan-out 병렬 + coach 동시화 + status 시점 교정 (wave 5)

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 27-06-PLAN.md — fault_zoom 사후 분리 + faultZoomStatus 3-way lockstep + 프레임 재사용 (wave 6)
...
- [x] 27-08-PLAN.md — [POD] Pro→Flash 조건부 실험 — 기각도 정상 종료 (wave 8, checkpoint)

**Wave 9** *(blocked on Wave 8 completion)*

- [x] 27-09-PLAN.md — [POD] phase gate: EVAL18 무회귀 + 결정론 + before/after 표 + env 박제 (wave 9, checkpoint)
...
- [x] 28-01-PLAN.md — Wave 0 검증-먼저: reference fps 실측(A1 해소) + test_motion_alignment RED + alignmentWarp.ts 순수 모듈 (wave 1)
...
- [x] 28-04-PLAN.md — 파이프라인 방출 배선(mode1+mode3 second+) + 채점 무접촉 테스트 (wave 3, 27-06 후)
- [x] 28-05-PLAN.md — fault_zoom D-04: fps 정합(표시 전용)+근사 제거+전신 폴백+refMatch (wave 3)
- [x] 28-06-PLAN.md — VideoCompare 워핑 소비: targetRefTime 단일 경유+rate feedforward+tier 배지 (wave 3)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 28-07-PLAN.md — result.tsx: alignment 전달+refMatch 캡션+D-05 legacy 배너 (wave 4, 26-02/27-07 후)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 28-08-PLAN.md — phase gate: affected 게이트+Pod 재분석 1회 실데이터 확인+OTA 발행+belle 실기기 항목 (wave 5)
...
- [x] 31-05-PLAN.md — typed 어댑터 create_task/poll 통일(B2-02) + 다운로드 경계(실 302 테스트 H2-04, 스트리밍 H2-05) + judge (H-03) [W2]
- [x] 31-06-PLAN.md — 결정론 pose 게이트: Pod /pose-image + pose_gate fail-closed (H-03) [W2]
- [x] 31-07-PLAN.md — 페어 적재 privacy: strict 동의 + versioned HMAC(rotation-safe 삭제 H2-06) + decision 일치 assert(M2-05) [W2]
- [x] 31-08-PLAN.md — 앱 컴포넌트: 2D 비교 뷰어(amended D-10, B-01 해소) + 참고코너 섹션 + 목업 checkpoint [W2]
...
- [x] 31-10-PLAN.md — HTTP 표면: asset exact 재서명(M2-01) + 요청 Lambda(outbox H2-02) + replay-safe enqueue(B2-03) + IaC(visibility 1800/maxReceive 5, H2-01) + dry-run only(H2-09) [W4]
...
- [x] 32-01-PLAN.md — 백엔드 수리 2건: D-16 사다리 재배치 + D-20 크롭 parity + phase32 테스트 스캐폴드 (w1)
- [x] 32-02-PLAN.md — 앱 수리: 수동 ±초 슬라이더·대략 맞춤 배지·legacy 폴백 + D-03 겹침 최소수리 (w1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 32-03-PLAN.md — 실물 게이트: Pod 배포·재분석 실측 + OTA + belle 실기기 → D-17 3건 확정 (w2, checkpoint)
- [x] 32-05-PLAN.md — 문구집+용어 맵: phrasebook fixture·조립 함수·금지어 게이트 + belle 감수 (w2, checkpoint)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 32-04-PLAN.md — 리서치+목업 게이트: 순서안 제시 + 적용 범위 목업(확정 3요소 공통 하한) → D-02/D-03/D-05/D-10 확정 (w3, checkpoint)
- [x] 32-06-PLAN.md — 미션 엔진+계약 3면: select_mission·streak·에스컬레이션 + validator + normalize (w3)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 32-07-PLAN.md — 타이포 토큰 + SummaryCard + summarySource + 코치마크 (w4)
- [x] 32-08-PLAN.md — 재생 중 자막 큐 + TTS·일러스트 샘플 게이트 → D-18/D-21 확정 (w4, checkpoint)
- [x] 32-09-PLAN.md — 파이프라인 방출 배선: 3단 문구·미션·코치 질문 + LLM 가변부 한정 (w4)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 32-10-PLAN.md — 감점 카드 재구성: DeductionCard 3단+인라인 줌+게이지+미션 배지 + 드릴다운·위험 톤 (w5)

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 32-11-PLAN.md — result.tsx 대배선: 게이트 확정 순서 재배치 + 보완 운동 가로(2회차 우회 제안 포함) + 코치 질문 + 참고 지표 전환 (w6)

**Wave 7** *(blocked on Wave 6 completion)*

- [x] 32-12-PLAN.md — 오디오 큐 구현(A/B 어느 확정이든 phase 내 완결 — B안 조건부 Polly 태스크 포함) + 실패 UX(D-29/D-30) + D-09 전 화면 검증 + OTA + belle 확인 (w7, checkpoint)
...
- [x] 32-14-PLAN.md — RTMW 측정층 확장 1단: keypointReport 8→12 + 하위호환 + 용량 검증 + 6동작 전수 스윕 (w9)
...
- [x] 33-02-PLAN.md — SEED Task 0: reference 11 doc 백업 강화 (temp→rename, whole-file SHA-256 S3 metadata + re-download byte-compare + restore rehearsal) [autonomous] (codex concern 10)
- [x] 33-17-PLAN.md — 신규: staging+activation 프리미티브 (candidate version id `phase33-cm3-*` refuse-overwrite + shadow-reference resolver + idempotent atomic flip/global pointer) [autonomous] (codex concern 1/3/7)
- [x] 33-18-PLAN.md — 신규: release manifest tuple + Pod /health commit-SHA+canary + gate_check.py JSON data-gate [autonomous] (codex concern 5/6/8, suggestion 2/7)
- [x] 33-19-PLAN.md — 신규: 33-M3-SPEC.md 잠금 (paired user+reference range + coverage floor + fail-closed + byte-identical invariants) — 33-05 선행 [autonomous] (codex concern 4)
- [x] 33-20-PLAN.md — 신규: 33-COVERAGE-MATRIX.md 단일 11동작 커버리지 표 (climb/sideway-spin/combo 해소) [autonomous] (codex suggestion 10)

**Wave 3** *(blocked on 33-02,17,18)*

- [x] 33-03-PLAN.md — SEED Task 1: 11종 재추출 @9fps → candidate versions(run1+run2) + commit-pinned Pod deploy(canary) + traffic isolation + manifest rows [Pod, checkpoint] (codex concern 1/5/6/14)

**Wave 4** *(blocked on Wave 3)*

- [x] 33-04-PLAN.md — SEED Task 2: candidate-aware 백필 (versions/{candidate} read+merge, fps candidate/CLI, real bodyComparisonSourcePose, warm-Pod canary blocking) [Pod, checkpoint] (codex concern 2/6/15)
...
- [x] 33-06-PLAN.md — SEED Task 4: 재검증 (shadow-candidate sweep + JSON 8항 data-gate + predeclared boundaries + coverage matrix + elbow-twist → 33-21 route) [Pod, checkpoint] (codex concern 3/8/12/13/14)
...
- [x] 33-22-PLAN.md — 엔진 2트랙 감점상한: `deduction_engine.tally` 실행 −40 집계캡(바닥 60) + 치명 트랙(관절캡·집계캡 우회, 절대바닥 25, DORMANT) + additive breakdown(track/executionRawTotal/…) + 계약 3파일 미러 + 합성 단위테스트(INV-3/6/7/8, elbow-twist Σ−111.4→60) [autonomous] (D-33~D-37, R1~R4)

**Wave 22** *(blocked on 33-22 — belle GPU greenlight 선행)*

- [x] 33-23-PLAN.md — 6 fixture 전수 재검증: 새 Pod(EU-RO-1 4090) 직렬 shadow 재분석 → 구조적 불변식 INV-1/2/4/5/6 (per-fixture 목표점 금지, INV-4 캡 위 변별 명시 확인). PASS 시 flip 33-07 unblock [Pod+belle, checkpoint] (D-38, R5)
...
- [x] 33-08-PLAN.md — A-1 동작별 기준 자세 4질의 표 (공통 재료, reprocessed ref 대조, 33-20 커버리지 매트릭스 인용)

**Wave 10** *(blocked on Wave 9)*

- [x] 33-09-PLAN.md — A-2 코칭 문구 동작별 phrasebook (code-change-0, 방향 전수 대조 fail-closed, 33-20 인용)

**Wave 11** *(blocked on Wave 9)*

- [x] 33-10-PLAN.md — A-3 확대비교 조사 (D-07 기준 + 근거 3갈래 + 안 2~3개 + seam #1 결정)

**Wave 12** *(blocked on Waves 10,11)*

- [x] 33-11-PLAN.md — A-4 목업 (실데이터 로컬 다운로드+provenance, 최악케이스, 3 상태전이) → belle 확인 ① [checkpoint] (codex concern 16)

**Wave 13** *(blocked on Wave 12)*

- [x] 33-12-PLAN.md — A-5 확대비교 구현 (crop provenance/join joint-exact, 8종 결함, contract lockstep)

**Wave 14** *(blocked on Wave 13)*

- [x] 33-13-PLAN.md — A-6 영상 위 표시 구현 (키포인트 opt-in, 마커 자기응답, 음성 정지+강조+재개)

**Wave 15** *(blocked on Waves 10,14)*

- [x] 33-14-PLAN.md — A-7 일러스트 세트 생성 + 해부학 전수 검수 + 앱 배선 (silent-hidden)

**웨이브 B — 정돈 (A 앞으로 당기지 않음)**

**Wave 16** *(blocked on Waves 13,14,15)*

- [x] 33-15-PLAN.md — 웨이브 B: 수치 자리 이동 (D-16) + 타이포·safe-area 버그·레이아웃 (D-17)
...
### Phase 37: 시간이 갈수록 나아지는 분석 모델 — 층 분리(몸/종목) + 데이터 누적형 재학습

**Goal:** belle 2026-09-22 — *"1년이든 몇 년이든 계속 똑똑해지는 모델을 만들고 싶은 거지.
폴스포츠에 많이 써먹겠지만 다른 분석 종목에서도 쓸 수 있게 사람 동작에 대해 학습을 해야 할 거 아냐."*
시간이 갈수록 **실제로** 나아지고, 다음 종목(예: 발레)으로 옮겨갈 때 몸에 대해 배운 것을
들고 갈 수 있는 구조를 만든다.

**왜 신설하나 — 지금 이 목표를 향한 설계가 없다.** 현행 Phase 22 는 Qwen3-VL-8B 한 덩이가
짚기·측정·코칭을 전부 하도록 **Gemini 에서 증류**한다(`backend/training/distill/gemini_teacher.py`).
두 가지가 막힌다: ①한 덩이라 몸 지식과 폴 지식이 같은 가중치에 섞여 **다음 종목에 폴만 떼고
갈 수 없다** ②교사가 Gemini 라 **교사와 독립된 교정·평가 자료가 없으면 교사의 사각지대를
재생산한다**. `[정정 2026-09-23 외부리뷰]` 처음 쓴 *"증류는 선생을 못 넘는다"* 는
**절대명제로는 틀렸다**(Born Again Networks 등 학생이 교사를 능가한 사례 존재).
우리 조건(단일 교사·소량·독립 교정자료 부재)에서 실질 제약인 것은 맞다.

#### 층을 넷으로 가른다 — 셋은 모델, 하나는 모델이면 안 된다

```

1. 몸      관절 표현 · 결함 문법("견갑이 솟는다", "관절이 무너진다")
           종목 무관 · 제일 오래 감 · 다음 종목에 그대로 감   ← 이 phase 의 본체

2. 종목    어느 동작의 어느 순간을 보나 (파워스핀 hold 국면)
           종목마다 얇게 추가 · 1번 위에 얹음                 ← 지금 Gemini 가 임시로

3. 채점    ★모델 아님. 투명한 감점 합산. 학습 금지            ← 지금 있고, 이대로가 맞다
4. 말      감점 → 한국어 코칭. 소모품, 갈아끼움                ← 지금 Cerebras/Gemini

```

**1과 2를 가르는 것이 전이의 전부다.** 한 덩이면 발레 데이터를 얹는 순간 폴이 밀린다.
`[정정 2026-09-23 외부리뷰]` 단 **1층을 반드시 별도 학습 모델로 둔다는 전제는 근거가 약했다.**
신체 관측과 맥락 의존 결함 판정을 **입출력 계약으로** 분리하면 목적은 달성된다 —
별도 모델은 그 결과일 수 있어도 전제가 아니다.
근거: 오늘 나온 감점 문장이 이미 몸 언어다 — *"어깨가 귀 쪽으로 으쓱 올라가지 않게 견갑을
눌러 잡고"*, *"왼쪽 팔과 몸통 사이 각"*. 폴 용어가 하나도 없다. 반면 "파워스핀 hold 국면"은
폴에서만 뜻이 있다.

#### 재학습은 **데이터 누적형(B)** — 가중치 누적(A) 아님 (belle 2026-09-22 문답에서 확정)

```
A 가중치 누적   지난 모델 위에 새 데이터를 얹는다
                → 망각 · 망친 사이클이 영구히 박힘 · 재현 불가
                → ★2026년 베이스에 묶인다. 더 좋은 베이스가 나와도 못 갈아탄다

B 데이터 누적   책이 두꺼워지고, 매번 베이스에서 처음부터 다시 읽는다   ← 채택
                → 망한 회차는 버리면 끝 · (베이스+데이터+설정) 셋으로 재현
                → ★베이스를 갈아끼워도 책은 그대로 = 베이스 발전을 공짜로 탄다
```

몇 년 단위에서는 B 가 결정적이다 — 그 사이 베이스 모델이 크게 좋아지는데 A 는 그걸 못 받는다.
**복리로 쌓이는 자산은 가중치가 아니라 책(코퍼스)이고, 그 책은 미래의 어떤 모델이든 다시 읽는다.**
대가 = 책이 두꺼워질수록 매 사이클 비용이 는다(받아들이는 청구서).

`[정정 2026-09-23 외부리뷰]` ★**위 A 칸의 "망각·재현 불가·베이스 교체 불가"는 A 를 불가능한
방식으로 규정한 과장이었다.** A 도 replay(과거 데이터 재사용)·실행 계보 보존·주기적 re-base 로
완화된다. **B 를 고르는 정확한 이유는 하나 — 지금 규모에서 B 가 단순하고 재검증이 쉽다.**
전환 기준도 영상 수가 아니라 *"승인된 비용 안에 B 가 끝나는가 / A 가 같은 평가셋에서 이득을 내는가"*.
★**누적 저장소 ≠ 이번 학습 데이터셋** — 무조건 전부 재학습이 아니라 중복 제거·라벨 수정·
동의 상태·종목 균형을 반영한 **선별 스냅샷**으로 학습한다.
★B 의 재현성도 (베이스+데이터+설정) 셋으로 끝나지 않는다 — 전처리·디코딩·샘플링·토크나이저·
라이브러리·seed·평가 코드까지 고정해야 한다.
※ `SFT_RESUME` 은 A 가 아니다 — 죽은 학습 재개(같은 데이터·같은 런)다.

#### 천장을 뚫는 재료 = 강사 판정

교사의 사각지대를 교정할 재료가 필요하다. `[정정 2026-09-23 외부리뷰]` *"사람 눈이 유일한
돌파구"* 는 **과했다** — 다중 시점 관측·검증된 측정·이벤트 시각·모델 간 비교도 교정 재료다.
다만 현 단계에서 **독립적인 강사 판정의 우선순위가 높은 것은 맞다.** 사람의 역할은 총점을
매기는 것이 아니라 **교사의 사각지대를 찾고, 정상 변형과 결함을 구분하고, 평가 기준을 안정시키는 것**.
2026-09-22 실증: belle 이 kip-up 정타/실수 정지컷을 보고 *"다리 높이가 다르다 · 왼팔이 덜 굽어
힘을 덜 받는다 · 둘이 서로 영향을 미친다"* 고 판독했다. Gemini 는 같은 영상에 *"자세 차이가
거의 없어요"* 를 냈고, 2026-06-27 에 사람이 적어둔 라벨("공중 양 무릎 굽음")도 실측과 반대였다
(무릎 최소각 정타 149.8 / 실수 160.6 — 정타가 더 굽는다).

**필요 데이터 (우선순위 — 2026-09-23 외부리뷰 반영해 재배열):**

1. ★★★★ **실제 학생 수행 + 독립 검증.** `[변경]` 원안 4순위에서 올렸다. 챔피언이 일부러 낸
   실수는 초보자의 복합 실수와 다르고, **정은지 한 사람의 정타·실수만 늘리면 동작이 아니라
   수행자·촬영환경을 외운다.**

2. ★★★★ **강사 판정 — 관측/결함판단/원인가설을 분리해서.** 부위·시점만으로는 부족하다(37-DATA-SPEC).
3. ★★★ **여러 강사의 정상 수행** — 챔피언과 다르지만 **허용되는 변형**을 확보한다.
   없으면 "정은지와 다름"을 결함으로 확정하게 된다.

4. ★★★ **같은 동작을 여러 방식으로 틀린 영상** (원안 1순위에서 강등).
5. ★★★ **가림·국면불명·지원 밖 동작** — "판정불가"를 학습·평가하려면 일부러 모아야 한다.
- ✕ 다른 종목 영상은 **아직**. 1·2층이 갈리기 전에 넣으면 섞인다.
- ★병목은 **양이 아니라 짝과 라벨**이다. 수집 548편인데 학습 투입은 244개고 5사이클 전부 탈락.
  안 붙은 100편보다 **강사가 짚어준 10편**이 크다.

#### 금지 (belle 기존 결정 — 이 phase 가 흔들면 안 되는 선)

- **채점(감점 계산)을 모델로 만들지 않는다.** 점수는 "왜 깎였는지"를 보여야 하고 블랙박스는
  그걸 없앤다 ([[scoring-must-be-transparent-deduction-tally]]).

- **사람 점수 라벨 금지** ([[analysis-objectivity-no-human-scores]]). 강사 입력은 "어디가
  틀렸나"이지 "몇 점인가"가 아니다.

- **각도 기준값을 데이터에 맞춰 재보정하지 않는다** — 커브핏. IPSF 인용이 근거고 출처가 사라진다
  (`kismam.py` 의 `보유 sweep 재calibrate 금지`, [[chasing-the-number-is-the-tangle]]).

- 기준값이 필요하면 **기준 선수의 그 순간**을 쓴다(`reference_relative`) — 사람이 각도를 안 정한다.

#### 지금 있는 토대

- `reference_relative` 축 — 2026-09-22 실측에서 criteria 가 빈 동작 3편(climb/peter-pan/pdshape)을
  정은지 대비 **기하 seed** 로 실제 감점했다(11~15점 분리). 2026-06-27 이 "빈 criteria 의 후임"으로
  지목한 그 축이 돈다. ★kip-up 에서만 침묵 — **원인 미규명, 이 phase 의 첫 실행 자리**.

- 정타/실수 쌍 6개 · 플라이휠 수집(548편) · promotion 게이트 + 래칫(`promotion.py`).
- ✕ 없는 것: 쌍을 자동 비교해 판정 지점을 찾는 코드(2026-09-22 에 사람이 손으로 1회 했을 뿐,
  리포에 없음 [[build-it-and-schedule-it-or-it-never-ran]]) · 찾은 지점을 criterion 으로 등재하는 경로 ·
  승격 모델을 앱에 꽂는 서빙 배선(22-08/09/10 미실행).

#### 표현의 구멍 (2026-09-22 발견)

belle 이 육안으로 **제일 먼저** 짚은 "다리가 뜬 높이"가 우리 표현에 **한 칸도 없다** —
`[정정 2026-09-23]` *"특징이 관절 굽힘각 8개가 전부"* 는 **틀렸다** — `features.py` 에
`split_angle_series`(IPSF 벌림각)·`head_spine_alignment_series`·`torso_uprightness_series`·
`posture_axis_summary`·`joint_uncertainty` 등 공개 함수 13개가 있다.
**그러나 높이·환경 상대 위치 특징은 실제로 0건**(height/floor/ground/pelvis-relative 검색 0).
핵심은 특징의 존재가 아니라 **측정·배선·평가 적용 여부**다.
추가 후보(외부리뷰 §7): 신체 상대 위치(골반 대비 발목 높이) · 환경 상대 위치(바닥 대비 골반) ·
이벤트 시각(발 이탈→골반 상승→최고점) · 접촉/지지 · **관측 품질(가시성·가림·추적 교체)**.
★DTW 는 비교 시점을 맞추면서 **속도 차이를 지운다** — 정렬용과 속도·간격 평가용을 분리해 보존할 것.

**Success Criteria** (2026-09-23 외부리뷰 반영 — 순서가 곧 우선순위):

0. ★**검사 범위와 결론이 일치한다** — `감점 records=[]` 가 지금 **정상 / 미검사 / 판정불가**를
   뭉개고 전부 "100점 · 지금 자세를 유지하세요"로 나간다. 셋을 **별도 상태로 표시**한다.
   검사 범위의 분모는 모델이 찾은 결함 수가 아니라 **그 동작에서 원래 검사해야 하는 항목 목록**.
   → 모델과 무관하게 신뢰가 무너지는 자리라 **0번**이다.

1. **kip-up 원인이 단계로 분리된다** — 좌표 / 국면 / 특징 / 기준 / 감점 5단계를 각각 계측해
   어디서 실패하는지 확정. ★*"전체 프레임 중앙값 31.8도"는 동일 국면의 기준 위반 편차가 아니다* —
   수행 속도·국면 길이·촬영 방향·가림일 수 있다. 원인 확정 전 처방 금지.
   **허용오차 20도를 낮춰 이 한 쌍을 가르는 것은 실패로 친다**(커브핏).

2. **인물·세션 단위로 분리된 평가셋** — 현행은 video hash 분리라 같은 인물·같은 세션 누수를
   못 막는다. ★*같은 6쌍을 보며 고쳤다면 회귀 테스트이지 일반화 증거가 아니다*
   (2026-09-22 측정이 그 경우다). 정상 변형·판정불가도 포함한다.

3. **학습이 더하는 가치를 분리 측정** — 같은 평가셋에서 E0(현행) / E1(규칙·배선 수정) /
   E2(+구조화 프롬프트·검증사례 검색) / E3(학습) 비교. **E3 가 E2 를 넘을 때만 학습이 정당화된다.**

4. **강사 판정 유입 경로** — 관측/결함판단/원인가설이 분리된 형태로 수집→저장→학습 투입 1건 완주.
5. **층 분리는 입출력 계약으로** — 신체 관측과 맥락 의존 결함 판정이 계약으로 갈린다.
   (별도 모델은 결과일 수 있어도 성공 기준이 아니다)

6. **베이스 교체 내성** — 베이스를 바꿔도 같은 선별 스냅샷으로 재학습해 동등 이상(B 검증).
7. **채점 투명성 무회귀** — 위 전부를 하고도 감점 내역·출처 표시가 그대로다.
   단 *"사람 점수를 안 쓰므로 객관적"* 이라는 설명은 과하다 — 기준 영상 선택·허용오차·감점 기울기·
   하한은 사람이 정한다. 객관성의 실무 근거는 **출처·재현성·측정오차·독립검증·변경이력**이다.

**Depends on:** 실증(2026-10 중순) 이후 착수 — belle 2026-09-19 *"실증까지 분석만"*.
단 **Success 0(미검사/판정불가 표시)은 실증 전 자리**다 — 틀린 자세에 "유지하세요"를 내보내는 것을
분석 결함으로 판단했다(belle 이 다르게 보면 되돌린다). Phase 22·34 위에 얹힌다.

**재학습 시작 조건** `[2026-09-23 추가]`: `TRAINING-DUE.md` 의 **건수 조건만으로 시작하지 않는다.**
①새로 검증된 관측이나 실측으로 확인한 실패 원인이 있다 ②이번 변경이 무엇을 개선할 **가설**이 있다
③전후 데이터·설정·전처리 차이를 설명할 수 있다 ④독립 평가와 비교 대상이 준비됐고 예산 안이다.
**현재 잔액 $5 · 사이클 $33~38 — 같은 구성 즉시 재실행은 보류.**

**Plans:** 0 plans (미착수)

Plans:

- [ ] TBD (실증 후 /gsd-plan-phase 37)

### Phase 38: 공급자 링크 — 실증 최소 (supplier-link)

**Goal:** 정은지가 폰 브라우저 링크 한 장으로 동작을 올리면 **사람 손 없이** 채점 가능한 기준이 되고 앱 목록에 보인다. 이 링크는 임시물이 아니라 같은 앱의 공급자 모드 **첫 화면**이다(기획안 §7-1·§7-4, belle Q1·Q2 ○). CLAUDE.md 파일럿 Step 2 그 자체. belle 09-26: *"공급자쪽 안 만들거야? 다음 섹션에 진행할 수 있도록 클리어하고"* — "실증까지 새 기능 금지"(09-19)의 belle 예외.
...
4. 등록 실패 4형 문구(사람 미검출 · 여러 명 · 서 있는 시작 없음 · 저신뢰) + **자기 재현성 한 줄**(본인 영상을 자기 기준으로 재면 N점, §7-3·§B-3-2).
5. 내 코드(강사 코드) 표시(§C-2 (a)). 크레딧은 숫자만, 결제 없음(§C-6).
6. (belle 09-26 ○) 길이 하한·상한(수요자 3~90초, 공급자 5~30초) 즉시 검사 + 문구 정정("측면 45°, 2~3m" 폐기 → "기준 영상처럼").
7. (belle 09-26 ○) 수요자 원본 영상 보관 **영구** — `uploads/` 30일 만료 수명주기 해제.
8. (belle 09-26 ○) **공급자 가이드** — 링크 페이지 체크 5 + 상세 화면 + `docs/supplier-guide.md`(쉬운 말, 촬영 = 정은지 영상 조건).

**Success:** ① belle 이 아닌 사람이 링크만으로 새 동작 1개를 올려 앱 picker 에 뜨고 mode1 이 RuntimeError 없이 돈다 ② 실패 4형이 각각 문구로 나온다(합성 입력으로 강제) ③ 자기 재현성 = 정은지 본인 영상 100 근처 ④ 기존 기준 11개·저장 분석 무접촉(불변 id 조인).
...
*Roadmap updated: 2026-09-26 (Phase 38 신설 — 공급자 링크 실증 최소. belle "공급자쪽 안 만들거야? 다음 섹션에 진행". 기획안 260925-pln §A·§C 판정 전부 반영(같은 앱 공급자 모드 · 링크 = 첫 화면 · 뒤쪽 자동화 · 강사 코드 · 크레딧 초기 버전). 착수점 = 38-SEED.md.)*
*Roadmap updated: 2026-09-23 (Phase 37 외부리뷰(Codex) 반영 — 결론 B 유지, **우선순위 역전**. Success 0 신설 = 정상/미검사/판정불가를 뭉개 "100점 지금 자세를 유지하세요"로 내보내는 것을 최우선 수리. 정정 6건: "증류는 선생을 못 넘는다"는 절대명제로 틀림 / A 비판(망각·재현불가·베이스교체불가)은 과장, B 의 진짜 이유는 "지금 규모에서 단순·재검증 용이" / 1층을 별도 학습모델로 두는 전제 철회, 입출력 계약 분리로 / "특징이 관절각 8개가 전부"는 틀림(features.py 13함수), 단 높이·환경상대위치는 실제 0건 / 데이터 우선순위 역전 — 학생 실제 수행+독립검증을 1순위로(챔피언 의도적 실수만 늘리면 수행자·촬영환경을 외운다) / 평가 분할을 인물·세션 단위로(같은 6쌍 반복은 회귀 테스트이지 일반화 증거 아님). 학습 정당화 = E0/E1/E2/E3 비교에서 E3>E2 일 때만. 재학습 시작 조건에 가설·비교대상 추가. 판정문 = 37-REVIEW-VERDICT.md.)*
