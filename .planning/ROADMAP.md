...
**Plans**: 15 plans (Wave 순서: Wave 0 → Wave 1 → Wave 2 spike & 검증 → Wave 3 swap)
  ...
  - [ ] 01-16-PLAN.md — **측정 신뢰도 trace spike** — 단일 frame 좌우 noise / lifter occlusion 좌우 헷갈림 / multi-engine averaging 검토. path A (frame-by-frame trace + Plan 08/11/13 cross-engine 비교) 우선 + path B (multi-engine averaging) / path C (multi-view 조기) / path D (Gemini 직접 EXTEND/BENT) 옵션 박제. (Wave 2, NEW 2026-06-01, **Plan 14 진입 gate**)
