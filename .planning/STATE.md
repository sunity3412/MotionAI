...
status: executing
stopped_at: "2026-09-24 오전. **인계서 = .planning/quick/260923-t57-2026-09-23-night/260923-t57-HANDOFF.md (§6 두 건 닫힘) + 오늘 규명 = .planning/quick/260924-c3m-kip-up-100/260924-c3m-SUMMARY.md.** belle 09-24 *\"넌 왜 자꾸 이걸 100점이라고 치는거야\"* → 규명 완료(코드 0·Pod 0): **측정은 맞다**(실수 왼어깨가 정렬 스텝 절반에서 30~60도, 정타는 전부 10도 아래) — **집계가 지운다**: 영상 전체 |Δ| 중앙값 하나가 두 봉우리(<15도 36스텝 / ≥25도 41스텝) 사이 틈 20.6 에 떨어지고 그 자리가 허용오차. 낮은 표본의 절반은 **정체**(기준이 3.67~4.6s 에 한 바퀴 더 도는 동안 학생 f33 한 장, 22스텝). 집계 창 변형(스텝 빼기만): 정체 제외 27.6(CI 20.8~) · exec 창 21.1 · Gemini clip 31.4, 정타 3.0~3.5 불변. correct.mp4 == ref-kip-up.mp4 같은 테이크 확인. ★허용오차 20 은 죽은 축(무접촉) · 짝·편차 손 계산 금지. ★belle 판독(09-24, SUMMARY §3-1): 실수 = sheet1 **k24(학생 1.0s 점프)부터 끝까지** 발 높이가 낮음, 원인은 왼팔 굽힘, kip-up 은 같은 자세로 계속 도는 동작. → 저장값으로 직접 잼(§2-6, 새 모델 0): **발 높이는 저장 2D 키포인트에 있다** — belle 구간 짝의 94% 에서 학생이 키의 21% 낮고, 정체 구간이 최대(−30%). 짝 없이 낮은발 5% vs 정은지 25%(정타 24%). 왼팔꿈치 덜 굽음 방향 일치(+6~12°, 20 아래). 어깨 raw 35° 차인데 짝 중앙값이 정체·진입 전 표본에 끌려 20.6. → **i38 (belle \"응\") 5동작 일반화 측정 = 기준 3/3 통과**(`.planning/quick/260924-i38-5-exec/260924-i38-SUMMARY.md`): 기준 clipRange exec 창 안 **짝 없는 각도 중앙값**이 5동작 전부 실수를 가른다(kip-up 왼어깨 +33.9 vs DTW 19.0, 4동작은 DTW 보다 크게), 정타 0 근처(예외 −10/−16/+13, 허용 20 안), 판 간 차 ≤5(DTW 12.1). 새 신호 아니라 집계만 바꾼 것. → **ig3 배선 + Pod 검증 완료**(`.planning/quick/260924-ig3-window-constant/260924-ig3-SUMMARY.md`, 커밋 1d9a41e9 + 구간 제거): 빌더 `ref_exec_window` = pointed(window) > constant > DTW, 플래그 REFERENCE_CONSTANT_WINDOW_ENABLED(default 1). L4 Pod 10편(앱 경로): **kip-up 실수 100→83**(왼어깨 33.9, −16.6) · 정타 5/5=100 dims 동일 · climb 86→60 · peter-pan 80→60 · pdshape 87→80 · power-spin 73→80(↑, 원인 = 상수 경로 순서통계 구간이 관절 움직임 폭을 잡음으로 읽어 어깨 2건 억제 → **구간 제거**로 수리, 산식 재계산 68, Pod 미재검증). Pod $0.16, 종료·자리표시자 복귀, 잔액 $22.74. **다음 = belle 판단 3건**: ①kip-up 확대 카드 사진(`kipup_fault_zoom_card_ig3.png`) 순간 ○× ②60 바닥(캡)에서 실수 크기가 뭉치는 것 ③높이 축(발·엉덩이, 저장 키포인트) 별건 여부. belle 영상 오면 t57 §3 절차 + 같은 10편 재분석으로 구간 제거 판 Pod 확인. belle 영상 오면 t57 §3 절차. Success 0 은 belle 확인 대기. Pod 0 · 잔액 $23.12(09-23 밤 기준) · origin 동기."
...
> 아래 페이즈 수치는 **이번에 `.planning/phases/` 를 다시 셌다**(PLAN vs SUMMARY 개수) —
> 39 페이즈(37 신설) · plan 205 · SUMMARY 대응 195. frontmatter `progress`(38/25/118/108)는
> GSD 관리값이라 본문과 다르다 — **본문이 실측이다.**
> 이력 문서를 인용할 때: 260920-cac SUMMARY 는 §14 부터(§11~§13 판정이 뒤집혔다),
> 260919-cls 인계서는 cac §4 의 정정 6건부터 읽을 것.
...
37-continuous-learning               ← 신설 2026-09-22 · 외부리뷰 반영 09-23. 본체는 실증 뒤.
...
Last activity: 2026-09-23 - Completed quick task 260923-weq: Pod 다운로드 멎음을 S3 가속으로 우회(30분→3초) + fixture 10편 재분석 (정타 100×5 · 실수 변별 4/5 유지 · GPU 종류가 각도를 바꾼다 8/8). 게이트 5010/0.
...
| 260924-i38 | **유지 구간 상수 축 5동작 일반화 측정 — 통과 3/3.** 기준 clipRange exec 창(DTW 로 가장자리만 옮김) 안 짝 없는 중앙값: kip-up 왼어깨 +33.9(운영 DTW 19.0), 4동작은 DTW 보다 크게 갈림, 정타 높이 ≤1.9·각도 대부분 ≤7, 판 간(09-22/23) 상수 ≤5 vs DTW 12.1. 높이 축(저장 키포인트)은 kip-up·power-spin·peter-pan 만. 코드 0. 다음 = 배선 belle 승인 | 2026-09-24 | 0d8f645a | [260924-i38-5-exec](./quick/260924-i38-5-exec/) |
| 260924-c3m | **kip-up 실수 100점 규명 — 측정은 맞고 집계가 지운다.** belle "왜 자꾸 100점이라고 치는거야". 운영 경로 재구성(median==운영 dev 3/3)·운영 extractor 정지컷(프레임 수 3/3): 실수 왼어깨 |Δ| 는 두 봉우리(<15도 36스텝 / ≥25도 41스텝), 전체 중앙값이 그 틈 20.6 에 떨어짐. 기준이 한 바퀴 더 도는 동안 학생 f33 한 장에 **정체** 22스텝이 낮은 표본 공급. 정체 제외 27.6 / exec 창 21.1 / Gemini clip 31.4, 정타 불변. correct==ref 같은 테이크. 대조표 3장 + 내 ○× 예측. 코드 0·문턱 무접촉·수리 방향 belle 결정 대기 | 2026-09-24 | 65440ce3 | [260924-c3m-kip-up-100](./quick/260924-c3m-kip-up-100/) |
| 260923-weq | **Pod 영상 다운로드 멎음 → S3 가속 엔드포인트 우회 + fixture 10편 재분석.** EU-RO↔서울 직결 30MB 4회 중 3회 5~7KB/s(호스트 교체로도 같음, egress 는 59MB/s) → 버킷 가속 ON + `S3_USE_ACCELERATE=1`(다운로드만, `_s3_download`) → 1~3.5초. 재분석: 정타 100×5 · 실수 73/100/86/80/87(09-22 66/100/89/85/87), veto 10/10 불변 → 오늘 수리 2건이 점수를 깨지 않았다. ★GPU 종류가 같으면 각도 0.0° 동일(2/2), 다르면(4090↔L4) 3~16% 프레임에서 ~20° 다름(8/8) → 실수 점수 ±3~7. 실증 Pod 은 GPU 한 종류로. power-spin 정타 stability 84→96 은 각도 동일·Gemini 창 차이[추정]. 게이트 5010/0. 잔액 $23.12. | 2026-09-23 | 86fa0f89 | [260923-weq-s3-accelerate-download](./quick/260923-weq-s3-accelerate-download/) |
| 260923-u2q | **Gemini 에 보내는 정은지 still 이 다른 순간이던 결함 수리 — 모든 Mode 1 분석.** 기준 각도(~15fps) 번호를 기준 영상 프레임(~10fps) 배열에 그대로 꽂았다 — 확대 카드는 28-05(07-08)에 같은 변환을 넣었고 veto still 경로(23-01, 06-23)만 빠짐. 실측(kip-up 정타 = 기준과 같은 테이크, 픽셀 대조): 같은 순간 **0/10 → 9/10**. 학생 2.0초에 정은지 3.0초, 후반 1/3 은 기준 마지막 프레임. 운영 기록: Gemini 판정 실행된 실수 4건 중 3건 still 1.1~3.1초 어긋남. 정량화도 영상 프레임 수로 clamp 돼 후반 기준 각도를 못 읽었다. `_matched_ref_frame`+ref_start · `SelectedFramePair.ref_image_idx` · 캐시 키 = 보낸 이미지 번호(stale-hit 방지). 점수 영향은 [미확인] — Pod 에서 fixture 재분석 필요. 게이트 **5007 passed / 0 failed**. | 2026-09-23 | e347911e | [260923-u2q-veto-still-time](./quick/260923-u2q-veto-still-time/) |
| 260923-swh | **영상 받는 도구 + 짝 비교 (m49 §9 의 1·2).** `intake_clips.py register`(메모 시트 → 내용 해시 · S3 영구 사본 `fixtures/intake/` · clips/subjects/pairs 원장) · `analyze`(Pod 있을 때만, 직렬, 타임아웃 시 배치 중단 → `analysis_runs.jsonl` = video_hash→doc 연결). 짝 비교는 **새 계산 없이** `measure_reference_axis.py --pairs/--pair/--clips` — 편차는 운영 `_deviation_against`, 점수·감점은 저장값. ★사람 영상은 manifest 에 안 넣는다: `eligible_for_distill` 이 동의를 안 보고 s3_key 행을 증류 후보로 자동 선택(37-DATA-SPEC §2-2 개정). 소급 = fixture 12편 + 09-22 분석 11건(ETag 대조 11/11). 인수 = kip-up 짝이 저장값(20.62도·−0.7점 억제) 재현. 드라이런이 사용성 버그 1건 잡음(새 사람 두 줄째 거절) → 수리. 게이트 **4999 passed / 0 failed**. Pod 분석 경로는 미실행(거절 경로만 확인). | 2026-09-23 | be7d0c7d | [260923-swh-clip-intake](./quick/260923-swh-clip-intake/) |
| 260923-sqt | **감점 재료가 기준의 엉뚱한 구간과 비교되던 결함 수리 (위양성·위음성 둘 다).** `_build_deduction_measured_deviations` DTW fallback 이 window-local path 로 **전체** 기준 배열을 인덱싱했다 — 33-M3-SPEC 이 점수 경로·표시·segment 는 잘라 고쳤고 이 builder 만 빠짐. 터지는 조건 = 학생 영상이 기준의 1.2~1.5배 길이(학생 10fps vs 기준 15fps, 창 미끄럼 문턱 0.80). 재현(운영 함수+실제 기준 doc, 정은지 영상 앞 15% 절단): 점수 경로 0도인데 감점 seed 29~33도 → **kip-up 89 · power-spin 85 · climb 84** (수리 후 전부 100). 합성에선 진짜 25도 차이가 19.7도로 읽혀 감점이 사라지는 반대 방향도 확인. 운영 기록 11건은 전부 창 [0,nr) = 영향 밖, 과거 점수 불변. 회귀 테스트 3건(수정 전 3 failed). 게이트 **4992 passed / 0 failed** 직접 실측. 산식·허용오차 무접촉. | 2026-09-23 | 3348a8e4 | [260923-sqt-seed-ref-window](./quick/260923-sqt-seed-ref-window/) |
| 260923-smt | **섹션 착수 정리 + kip-up 수치 정정.** 착수점 1개(CLAUDE.md 경로 → STATE stopped_at 간접참조, 3회 어긋남이라 구조로 막음) · CONTINUE-09-07 archive · **빈 `phases/37-` 제거 — `find-phase 37` 이 그 빈 디렉터리를 돌려주던 실제 고장** · 스테일 WAITING.json 해제 · STATE 실측 39 페이즈(완료 30/진행 4/미착수 5, plan 205/195) · TRAINING-DUE 잔액 $4.26 · MEMORY 착수점 항목 5개(23줄) → 경고 1항목. ★kip-up: k02 표는 원시 각도 DTW·마스크 없음 = **운영 계산 아님**. 저장된 감점 기록 = 어깨 20.62도, −0.7점이 측정오차 구간(15.89~24.33)에 걸려 억제 → 100점. 판정 '배선 고장 아님' 유지. 코드 변경 0. | 2026-09-23 | 95b5dfac | [260923-smt-section-start](./quick/260923-smt-section-start/) |
| 260922-gnj | **Mode1 안정화 실측 (belle 지시 *"mode1 정은지와 비교하는 것만이라도 오늘 좀 안정화를 찾아봐"*).** 판정 셋 — ①**재현성은 문제 아니다**: 같은 영상(power-spin 정타) 3회가 **차원까지 동일**(angle 100·line 91·stability 84), 조건을 L4→4090 GPU 교체 + **Gemini 캐시 doc 삭제(신규 호출)** 까지 바꿨는데 동일. belle 09-19 *"할 때마다 다르니까 문제"* 는 현행 판에서 재현 안 됨. 수강생 첫 업로드는 항상 캐시 미스이므로 3번째 런이 그 경로를 대표한다. ②**변별은 5동작 중 4동작에서 된다** — 정타/실수: power-spin 100/**66**(분리 34) · peter-pan 100/**85**(15) · pdshape 100/**87**(13) · climb 100/**89**(11) · kip-up 100/**100**(**0**). 정타는 5/5 전부 100·감점 0건. 감점마다 한국어 cueLine+coachQuestion 동반(수치 아닌 처방). ③**kip-up 만 침묵 — 고장이 아니다**: Pod 로그가 `fault_zoom_anchor_check joint=left_shoulder criterion=none` → `angle_bake=omitted:no_criterion`, 즉 **어깨를 찾았는데 붙일 기준이 없다**. `deductionBreakdown` records 0 · **coverageGaps `[]`**(=검사 못 했다는 표시조차 없이 100점이 나간다). 원인은 허용오차 문턱(어깨 20.6도 vs 허용 20도 → 0.6도 초과 = −0.7점 + CI 억제)이고 **낮추면 커브핏** — belle 판단 *"둘 다 점수로서는 옳다"* 유지, 결함 정의는 yaml 주석이 belle 도메인으로 지목. ★**내 오진 3건 정정**: (a) "빈 criteria 동작은 채점을 아예 안 한다"는 **틀렸다** — climb·peter-pan·pdshape 가 빈 criteria 로도 `reference_relative`(정은지 대비)에서 **기하 seed**(`source=geometry`)로 감점한다. 06-27 이 후임으로 지목한 축이 실제로 돈다. kip-up 1건으로 일반화하려던 것이 성급했다. (b) "s3_download 15분은 지역 탓, 실증 대기시간 문제" → **그 Pod 링크 고장**이었다(57→20→0.9 KB/s 붕괴). 교체 Pod 은 3.29 MB/s, 같은 구간 16초. (c) "kip-up 이 멈췄다" → 느렸을 뿐 88분에 완주. ★**사고 1건**: 드라이버 `--timeout` 900s 고정 탓에 앞 런이 안 끝났는데 다음 런이 제출돼 **동시 분석 2건** 발생 — 겹친 수치는 전부 버리고 새 Pod 에서 재측정했으므로 위 표는 전부 단독 실행분. 러너를 `--timeout 5400`+terminal 대기로 교체. ★**채점 코드·허용오차 변경 0건**(커브핏 금지). 학생 영상 0편 — 전부 정은지 fixture. `elbow-twist-sister` 정타는 112.6MB > MAX_VIDEO_BYTES 100MB 로 앱 경로 측정 불가(결함 아님). Pod 종료 완료·Lambda 자리표시자 복귀·잔액 $5.64 | 2026-09-22 | (측정 전용, 코드 변경 없음) | [260922-gnj-mode1](./quick/260922-gnj-mode1/) |
| 260922-gav | **`pod_teardown.py` 안전 수리 2건 — 안전장치가 작동하지 않는 상태로 belle 에게 안내돼 있었다.** ① Cloudflare 1010: RunPod GraphQL 앞단이 urllib 기본 UA(`Python-urllib/*`)를 차단해 **HTTP 403** 으로 스크립트 전체가 죽었다 (실측 대조 — None=403 / `curl/8.7.1`=200 / 커스텀 UA=200, 즉 키·권한 문제 아님) → `User-Agent` 헤더 추가. ② **무인자 실행이 계정 전체 Pod 을 종료했다** — RunPod 계정은 프로젝트 공용이라 `myself{pods}` 에 다른 프로젝트 Pod 이 섞인다. 2026-09-22 실측으로 Sunityfunding 학습 Pod `sunity-pipe-translator-q9b` 가 같이 잡혔다. 아침에 belle 에게 *"제가 못 끄면 이걸 누르세요"* 로 안내한 그 명령이므로, ①만 고치고 ②를 놓쳤으면 **안내대로 눌렀을 때 남의 학습이 죽었다** (두 결함이 서로를 가리고 있었다) → `OWNED_NAME_PREFIX="sunity-motion"` 도입, 접두사 불일치 Pod 은 종료하지 않고 건너뜀 출력, 명시 id 인자는 그대로 존중. 드라이런 실측: 종료대상 `['m2czli3w4wqpe5']` / 건너뜀 `sunity-pipe-translator-q9b`. `[미확인]` 종료 경로 전체(terminate→Lambda 자리표시자→SSM) 끝까지 실행 검증은 없다 — 같은 시각 Mode1 측정이 그 Pod 에서 돌아 실행을 피했고, 오늘 Pod 을 내릴 때의 실행이 곧 검증이 된다 | 2026-09-22 | `74b17c21` | [260922-gav-pod-teardown](./quick/260922-gav-pod-teardown/) |
| 260920-m3r | **mode3 를 기준 선수 각도 축에 배선 (플래그 뒤, 기본 OFF)** — belle *"mode3 학생 비교는 틀리고 맞고가 아니라 발전해냐 안했냐겠지?"* mode3 종합점수는 **떨림 단독**이었다: 동작을 모르니 `line` 차원이 구조적으로 안 생기고, 이전 영상 대비 유사도는 **일부러 종합에서 뺐다**(발전하면 못하던 과거의 나와 덜 비슷해져 점수 역전 — 그 판단은 옳다). 그래서 **움직이지 않는 잣대가 없었다.** 인식된 동작의 기준 각도를 그 잣대로 건다. ★범위를 좁게: `reference_dtw_match`/`reference_angles_for_veto` 는 소비처가 **여섯**이라 mode3 에서 채우면 **확대카드가 '지난 영상 대비'→'기준 대비'로 조용히 바뀐다** — belle 승인은 채점 축이지 카드 문법이 아니다. mode3 전용 변수 3개를 따로 두고 **감점 builder 한 곳에만** 주입, 경계를 구조 게이트로 상시 강제(변이 검증 통과). ★라벨도 같이: 절대트랙 라벨을 단 채 기준 비교 점수를 내면 화면이 채점 출처를 틀리게 말한다(TRUST-03 이 막으려던 것) → mode3 basis 4값→6값, 계약 4곳 동시 갱신. **라벨은 의도가 아니라 산출을 따라간다**(기준 못 찾으면 종전 라벨). 운영 함수 실측: 정타>결함 동작 **3/6 → 5/6**, 분리 중앙 **+2 → +19점**, 발화 32/32. ★**kip-up 만 ±0 이던 것도 재고 고쳤다** — 내 옛 진단("축이 결함을 못 본다")은 **틀렸다**: 축은 정확히 본다(왼어깨 3.5→20.6도). 막힌 건 ①허용오차 20도(0.6도 초과 = −0.7점) ②신뢰구간 억제(CI 15.9~24.3 이 문턱을 걸침) — **둘 다 점수로서는 옳다.** 틀린 건 *그 점수 차이로 발전을 읽으려 한 것*. 발전은 문턱이 필요 없는 양(`이전 편차 − 지금 편차`) → **6/6**, 전 동작 관절 8/8 만장일치. `comparison.referenceApproach` 관측 전용 방출(점수·앱 무접촉). 허용오차 20도는 **안 건드림**(커브핏 금지). ★**yaml 이 빈 이유도 규명**(belle *"많이 분석했었던건데"*): 방치가 아니라 **06-27 의도적 제거**다 — 양 무릎 EXTEND 를 걸었더니 **정타가 위양성**을 맞았고(peter-pan 정타 116°가 결함 121.6°보다 더 굽음 등) belle 원칙 "굽힘 form 에 신전기준 강요 금지"로 지웠다. ★**파일 네 편이 후임으로 `reference_relative(정은지 대비)` 를 지목해 놨다** — mode1 엔 있었고 mode3 에만 없었다. 즉 오늘 배선 = 06-27 결정의 mode3 이행. `[정정]` 내가 적었던 "belle 라벨링 필요"는 오진. kip-up 만 별 트랙으로 미뤘는데 그 판정은 **무릎 한정** — 정은지 대비로 재면 신호가 **어깨**에 있다(3.5→20.6도, 재분석 137건 흔들림 없음; 단 원본은 1쌍). 진짜 미작성은 `ref-combo` 하나. 게이트 **4989/0**(신규 24) · 앱 tsc clean. ★**아직 안 켰다** — 학생 영상 3관문(같은 기준에 일관되게 붙나·강사 ○× 방향 일치·카메라 각도 내성) 통과 후 Pod env `MODE3_REFERENCE_RELATIVE_ENABLED=1`, 배포 없이 스위치만 | 2026-09-20 | `37da77fc`·`7a3f7d36` | [260920-m3r-mode3-reference-axis](./quick/260920-m3r-mode3-reference-axis/) |
| 260920-stn | **D-08 억제 표시 복원** — belle D-08("confident 97 금지")의 구현이 2026-09-09 시안 정리에 **부수 피해로 쓸려나갔고**(철회 기록 0), 그 사이 라이브 mode3 7건이 59~98점을 띄웠다(중앙 97). ★"D-08 vs 09-09 충돌"은 **내 오판** — D-08 이 UX 강도를 Claude 재량으로 열어놨다. 샌 표면은 다이얼 하나가 아니다: **진행 호**(숫자를 지워도 호 길이가 점수를 그린다) · 접근성 라벨 · 교정포인트 탭 밴드(100−감점=종합 항등식) · 요약 척도 단언 · **공유 문구**(앱 밖) · 홈 카드 · 기록 행 · 마이 평균. ResultScoreDial 이 억제를 직접 그려 원반 기하를 재사용한다. **정적 게이트 신설**(게이트 부재가 09-09 회귀의 원인) — 변이 검증 통과. 시뮬 실물 확인에서 tsc 가 못 잡은 2건(import 관례·레이아웃 파손) 적발 | 2026-09-20 | `5c29f91f` | [260920-stn-d-08-confident](./quick/260920-stn-d-08-confident/) |
| 260920-ruj | **측정 장비를 리포에 박제 + 바닥 수치 정정** — `backend/scripts/measure_reference_axis.py` (편차·바닥대비·문턱까지 여유·DTW 매칭·line_score 한 표, 운영 함수 호출만, 읽기 전용, 전수 스캔 차단) + `reference_axis_floors.json`. ★**정정**: §14~§17 에서 보고한 "여유 2.0~2.3도"는 **옛 판** 값이다. 오늘 Pod 실측(rot180 양쪽)은 pdshape 바닥이 **15.0 → 5.8**(역립 89.9% 동작 — 09-17 rot180 이 실제로 먹었다), 여유 12.1~17.2도. `ref-elbow-twist-sister`(역립 80.2%, 옛 판 15.8)는 **현재 판 미측정** — 우선 측정 대상. 부수: 기준 doc `techniqueProfile` 과 criteria yaml 이 어긋난다(power-spin 무릎 bent_ok vs EXTEND) — 운영은 yaml 을 본다 | 2026-09-20 | `29b35bd4` | [260920-ruj-axis-measure](./quick/260920-ruj-axis-measure/) |
| 260920-ra8 | **EXTEND 채점 hold 창을 국면 힌트에서 기하 창으로** (belle 승인 *"너무 과하게 오르지 않고, 분석이 그게 맞다면 인정해야지"*). Pod 실측 — 정은지 파워스핀 정타가 80점이었고 범인은 `leg_extension`(`reference_relative` 0건), 원인은 Gemini 가 10.60초 클립 hold 를 **1.0초(진입부)**라 답한 것. 같은 영상 5벌(캐시 우회)에서 hold 가 **8.0초/1.0초 이봉, 2/5 오답**. `_select_window` 가 힌트를 버리고 기하 창만 쓴다 — 소비처 1개라 점수·화면이 같은 창(TRUST-01). **새 상수 0개.** 저장 122건 재계산 = 정타 위양성 4건 제거 + 실수 8건 신규 검출(양방향). 부수: `_hold_window_from_moments` 는 EXTEND 경로 소비처 0. ★`[정정 09-20 밤]` "`profile.hold_window` 소비처 0" 은 **틀렸다** — `_select_stability_window`(dimensions.py:366)가 아직 읽고, mode3 는 종합이 stability 단독이라 **국면 힌트가 mode3 점수에 그대로 닿는다**(성공기준 1번 직결), 사용자 문구 일상어화(신전→펴짐, hold 구간→버티는 동안). 게이트 4965/0. ★실패 16건은 건별 판정 — 단언 완화 0건 | 2026-09-20 | `97a451f8` | [260920-ra8-extend-hold](./quick/260920-ra8-extend-hold/) |
| 260919-tkv | **분석 버전 각인 (`result.analysisVersion`)** — belle 09-19 *"분석이 할 때마다 다르니까 문제"* 의 기계적 원인 수리. 어느 커밋·어느 기준 라이브러리(`activeCandidate`, 실재 확인 후에만)·어떤 플래그로 나온 분석인지 doc 에 찍는다. ★**비결정성은 아니다** — 같은 코드 3회 실행 지문 동일(두 묶음 3/3). 구멍은 **추적 부재**였다. 채점 무접촉을 시험으로 박제. ★부수 발견: `RTMW_DETERMINISTIC` 판정이 `/health`(느슨) vs 엔진(`=="1"`) 불일치 — `=true` 면 health 는 켜졌다는데 엔진은 꺼진다(현행 운영 영향 0, `start_server.sh` 가 `=1`) | 2026-09-19 | `23f0b6d6` | [260919-tkv-analysis-provenance](./quick/260919-tkv-analysis-provenance/) |
| 260919-t2v | **Pod 주소 동기를 사람 절차에서 제거** (belle 09-19 *"pod 주소는 매일매일 새롭게 하는데"*). `start_server.sh` 가 health 통과 뒤 Lambda env + SSM 을 **스스로** 갱신, `pod_teardown.py` 가 종료+되돌리기 한 단위. 라이브 Lambda 가 **3세대 전 Pod** 을 가리키던 것 수리. ★볼륨 사본은 자동 안 바뀜 — 기동 전 리포에서 복사 필수 | 2026-09-19 | `54a4f5de` | (스크립트 직접) |
| 260919-oxg | **각도 표시 게이트에 시간축 안정성 축 추가** (belle 09-19 "왠만하면 다 찾아내야지"). ★실측으로 드러난 것: `_KP_CONF_MIN=0.5` 가 **좋은 좌표와 나쁜 좌표를 못 가른다** — 문턱 바로 위아래 품질이 같고(5.06 vs 6.23도) 신뢰도는 **0.6 위**부터만 의미가 있다. 그 선 하나가 각도 카드 절반을 침묵. 답 = 문턱 내리기가 아니라 `conf>=0.5 OR (conf>=0.35 AND 시간축 안정)` **순수 additive**. 꼭짓점은 미완화(크롭 중심 = belle 승인 문법 4R#1). 개통 **50%→70%**(+21장). additive 를 **PNG sha256 바이트 동일**로 증명 | 2026-09-19 | `543b4604` | [260919-oxg-angle-stability-gate](./quick/260919-oxg-angle-stability-gate/) |
| 260919-o8v | **확대 사진 — 억제가 각도까지 지우던 것을 원 마커로 좁힌다** (belle 2026-09-18 승인 규칙 2 "대신→함께" 구현). 기계 눈이 부위를 확인 못 해도 **각도·사이각은 살고** `userMarked`/`refMarked` 도 사실대로 True 로 남는다 — 원만 그린 패널은 종전대로. ★각도·원이 이미 배타적이라 새 스냅샷 불필요. 기존 시험 3건은 삭제 0, **증인을 억제 로그로 옮겨** 성질 유지. `kept_angle=` 로그로 라이브 발화량 수확 예정 | 2026-09-19 | `26b8330a` | [260919-o8v-zoom-angle-sentence](./quick/260919-o8v-zoom-angle-sentence/) |
| 260919-mhl | **짝 시간 정렬 이탈 방출 (관측 전용)** — 카드마다 `pairDeviationSec`+`pairDeviationTier` 를 doc 에 싣는다. 게이트·점수·앱 무접촉(additive 2필드). ★라이브 실측으로 드러난 것: motionAlignment **100% `trim_only`**(warped 0/36, distance 52~72 vs 임계 25) → warp 가 사실상 항등 · `refMatch='failed'` **0/192** · `pairState` `match` **24/24** = doc 에 짝 품질 신호 0건. ★인계서 260919-0d3 §6 의 belle 4/4 표는 **네 해석 전부 재현 실패**(크기·순서 모두) — §11.12 는 잠정 | 2026-09-19 | `3ed5ac0b` | [260919-mhl-card-pair-time-deviation](./quick/260919-mhl-card-pair-time-deviation/) |
