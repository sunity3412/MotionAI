...
Last activity: 2026-08-31 - **1.2.0 양 플랫폼 빌드 발행 (belle "빌드해보자, 안드로이드도 같이")** — iOS 빌드 31 EAS FINISHED + TestFlight 자동 제출(빌드 30은 EAS 서버측 업로드 오류 SERVER_ERROR → 재시도로 해소), Android APK preview-android FINISHED (설치 링크 CONTINUE 상태 블록). 남은 확인 = belle 실기기(TestFlight 업데이트 + 구글 로그인 uid 승계 + 결과 화면 재구성 육안). 부수: apple-design 스킬 설치(.claude/skills/ — 오늘 결과 화면 진단에 사용), taste-skill 플러그인 존재 확인(설치는 belle 대기)
