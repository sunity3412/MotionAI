...
|---|-------------|------|--------|-----------|
| 260923-u2q | **Gemini 에 보내는 정은지 still 이 다른 순간이던 결함 수리 — 모든 Mode 1 분석.** 기준 각도(~15fps) 번호를 기준 영상 프레임(~10fps) 배열에 그대로 꽂았다 — 확대 카드는 28-05(07-08)에 같은 변환을 넣었고 veto still 경로(23-01, 06-23)만 빠짐. 실측(kip-up 정타 = 기준과 같은 테이크, 픽셀 대조): 같은 순간 **0/10 → 9/10**. 학생 2.0초에 정은지 3.0초, 후반 1/3 은 기준 마지막 프레임. 운영 기록: Gemini 판정 실행된 실수 4건 중 3건 still 1.1~3.1초 어긋남. 정량화도 영상 프레임 수로 clamp 돼 후반 기준 각도를 못 읽었다. `_matched_ref_frame`+ref_start · `SelectedFramePair.ref_image_idx` · 캐시 키 = 보낸 이미지 번호(stale-hit 방지). 점수 영향은 [미확인] — Pod 에서 fixture 재분석 필요. 게이트 **5007 passed / 0 failed**. | 2026-09-23 | (this) | [260923-u2q-veto-still-time](./quick/260923-u2q-veto-still-time/) |
