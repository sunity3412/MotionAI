...
Last activity: 2026-08-06 밤 - Completed 260806-usc: V-1 재생 편측 갈림 불변식 집행(`d70355a4`, OTA prod `ac79c72e`). **개발 빌드 #29 + 시뮬 재현 루프 첫 가동** — V-1 시뮬 미재현(상태머신 정상 실증) → 불변식 집행 수리 → 시뮬 회귀 PASS(큐 4사이클 대칭). 실기기 실효 = belle 재확인 대기(안 돌 때 **양쪽이 같이 멈춰 있으면** 집행 성공). V-2(시작음성)= belle 결정+데이터 대기. 같은 날: 260806-sjt 내 영상 회귀 수리 + Pod 재분석 4건(72/80 belle 확인) + belle 계정 반영 PASS. 보드 아티팩트 갱신 규칙 신설([[feedback-status-board-artifact-is-canonical]])
