...
Last activity: 2026-08-07 - V-A 자막-음성 불일치 종결 수순: 원인 확정(08-02 mrg 자막 재조립 때 합성만 옛 전문 — 용의점 4개 전부 소거) → 1차 수리(합성=자막 미러 + 3계정 27키 재합성 + 구세대 42키 정리) → belle 킵업 반려 2건 → 2차 수리(마침표 lockstep + 큐·틱 인증 순간 atVideoSec 배선·미인증 억제, 시뮬 3동작 실측, OTA prod `e096f9f2`) → belle "엘보 좋다·파워스핀 잘 나옴" + 신규 4건 → 260807-fpw(번호 시간순·재개 백오프·큐 체이닝·색 반전). V-2 표시측 해소(데이터측 = Pod 재분석 대기). GSD 모델 fable 복귀
