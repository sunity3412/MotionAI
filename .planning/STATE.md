...
Last activity: 2026-09-18 - ★★★**확대 카드의 짝이 틀렸다 — belle 이 실물 카드로 발견** (quick-260919-0d3). belle "8.1초는 서로 다른 사진이다". 원인은 고장이 아니라 **설계 + 거짓 라벨**: 확정 카드는 DTW 를 끄고(dtw_match=None) 만들어지고 짝은 표시층 캐스케이드가 관절마다 따로 고른다(팔꿈치 = 학생 폴 최대/기준 최소, belle 5차 승인 문법). 그런데 `refMatch='dtw'`·`refMatched=true` 가 **아무것도 검사 않고 찍히는 상수**라 앱의 정직 캡션·뷰어 게이트가 **원리적으로 발화 불가**. ★★★★**belle 이 내 측정 설계를 무너뜨렸다** — "자세가 다르니까 같은 순간에 자세가 다른 걸 나타낸 것". 자세 차이는 카드가 보여주려는 결함 그 자체라 신호와 잡음이 섞인다. 대체 = **시간 정렬 이탈**, belle 4/4 순서 일치(−4.28초 확실히다름 / +0.67 경계 / −0.13·−0.20 통과). belle 기준은 "동작의 **구간**이 같은가"(회전량은 구간을 안 바꾼다). 통과선 0.2~0.7초 사이. ★내가 틀렸다가 고친 것 **5건**(인계서 §8) — 감점 상한 두 번째 재질문 · ROT180 env 누락 수정안 · Pod 종료 오처방 · **판정 사진을 내가 새로 그림** · 짝을 자세로 잼. ||| ★★★**착수점 정리 — belle "뭐 하나 정리가 안 되고 있는 것 같다"** (quick-260918-k06). 실증이 **약 한 달 뒤(10월 중순, 날짜 미확정)** 로 밀려 실증 Pod 종료 + `pod-expected=down`. 전 트랙 실측(에이전트 10대) 결과 진원지 셋을 닫았다: **착수점 4개→1개**(plan.md·CONTINUE·STATE 에 이력 배너) · **원장 누락 46건 복구**(184/184, 단일 표 날짜순, `(SUMMARY 없음)` 10건이 미종결을 드러냄) · **유령 판정 8건 정정**(§8 원장 신설). 상태 보드 = https://claude.ai/code/artifact/733f91d2-af6f-41d3-9b67-8f8f43ff07ed . 새로 코드로 짚은 것: **"한쪽만 동그라미"는 게이트가 아니라 advisory 카드**(refMarked None/False 로 갈린다) · **재학습은 이번 실증에 안 닿는다**(서빙 배선 0건) · **동시 분석 오염 재측정 0건 = 실증 1순위 위험**. 게이트 4835 passed / 0 failed(직접 실측), 앱 tsc clean. ||| **카메라앵글 트랙 종결 + 회전 묶음 배포 검증 + 강사 교정 OTA 발행** (quick-260918-et1 / 260918-fyf). ★★★**카메라앵글 트랙이 두 갈래 다 닫혔다**: (a) **Omni 1.1 은 카메라를 안 돌린다** — 입력을 그대로 재인코딩해 돌려준다. belle 이 "호출 탓일 가능성도 있다"고 짚어 **대조군으로 갈랐다**: 같은 REST shape 으로 구모델을 때리니 오늘도 7월과 똑같이 돈다(원본 대비 픽셀차 **7월 preview 38.7 / 오늘 preview 37.7 / 오늘 1.1 = 2.4**). belle 이 준 모델 카드가 같은 말을 한다 — preview 설명의 **"conversational editing" 이 1.1 에서 빠지고** keyframe control/scene extension/upscale 로 갔다. (b) **아바타 트랙은 belle 판정으로 닫힘** — 실물 두 벌(점선 스켈레톤 / 살 입힌 아바타)을 보여드리고 *"아니야 퀄리티가 있으나 마나일것같아"*. 논리가 옳다: 아바타 값어치는 관절 좌표 정확도에 달렸고 그게 미해결이라 틀린 좌표를 3D 로 예쁘게 보여주면 값어치가 음수다. **재개 조건 = 모델 발전이 아니라 좌표 정확도 해결**. ★★**만들다가 드러난 것: 저장 `joints3d` 의 z 가 전부 0** — T×17×3 으로 저장되고 앱 계약에도 열려 있는데 깊이가 없다(로컬 doc 8건 전수 z std 0.0000). 운영은 RTMW 2D 이고 리프터는 `backend/research/` 에만 있다. ★**단가 전제 21배 오독 정정** — 카드의 Video output $17.50 은 영상 1편 값이 아니라 100만 토큰 단가(8초=47,302토큰=$0.83). 오늘 실제 지출 **$1.66**. ★**7월 게이트 맹점 발견** — 벤더가 원본을 그대로 돌려주면 RTMW 가 같은 관절을 뽑아 MAE 가 0° 로 수렴해 **아무것도 안 한 출력이 만점 통과**한다. 스모크 없이 10건 게이트를 돌렸으면 거짓 통과를 보고할 뻔했다 → `run_gate_v11.py` 에 회전 관문(임계 10.0) 부착. ★★★**회전 묶음 배포 검증(260918-fyf)**: **kip-up 99→99 · power-spin 73→73 — 나빠진 동작 없음**. 인계서가 걱정한 kip-up 위양성 재발 없음. 기준 11편 중 5편은 허용오차 이내, 6편 초과(pdshape 는 어제 E2E 확인분). 계기부터 검증했다(belle pdshape 60→87 · 정은지 60→100 재현). ★**내가 틀렸다가 고친 것 1건** — `pre_phase4` 를 배포 전 기준으로 써서 pdshape −54 를 냈고 하마터면 "배포가 점수를 망가뜨렸다"고 보고할 뻔했다. 어제 박제 15.6° 와 대조해 **배포 직전 라이브 = `phase4_v1`** 로 판별. **백업 이름이 `pre_` 라고 직전 상태가 아니다.** ★**ref-climb 의 옛 기준이 틀려 있었다** — 배포 최대 변화 68.28° 인데 회전 기여 0.00, 독립 추출 3벌이 새 값과 일치하고 옛 phase4_v1 만 혼자 다르다(엉덩이·무릎 125~137 vs 57~89). 배포가 고친 것으로 보이나 학생 doc 이 없어 점수 영향 미측정. 인계서가 걱정한 부분 미러 상태는 아니다(플립 정상 완료). ★**확대 카드 게이트 fail-open 확인** — 눈이 `unclear/unreadable` 인데 `action=pass`. 원 위치 판정과 별개 결함. ★**강사 교정 v2 OTA 발행** — `0dddb65f` preview/1.2.4 iOS+Android (tsc CLEAN · 앱 테스트 304/0 · 시뮬 왕복 확인 후 belle 승인). **남은 belle 판정 1건 = 확대 카드 `zoom_adv_left_shoulder` 원 위치**(판정지 = quick-260918-fyf/evidence/JUDGE-zoom-card.png, 내 예측 박제: 원 중심이 몸 왼쪽 바깥·실제 어깨는 원 오른쪽 가장자리) **[정정 2026-09-18: 이 문장(커밋 d3afd4e9, 11:53) 3분 뒤 11:56 belle "어깨 맞음" 판정 수신 — 커밋 5d6deef1. 미결 0건]**. **다음 일 후보** = 게이트 fail-open 수리 · 학생측 ON 재분석으로 남은 4편 측정(Pod). Pod 0대, RunPod $2.65(오늘 $0 소모), Firestore 읽기 34회 ─ 이전: 2026-09-18 - **Omni 1.1 카메라앵글 재게이트 — 안 된다, $1.66 에서 중단** (quick-260918-et1, 커밋 570ec4c6/c5bd69b7). belle 지시("omni 모델이 업데이트 되서 나왔고 지금 다시 시도해보려고 하는거야")로 착수. ★★★**인계서 §2 의 전제가 셋 다 틀렸다** — (1) "10건 pose-consistency 게이트 미착수"는 **틀렸다**: 2026-07-17~18 에 실호출 9건 + 생성 영상 10편 + metrics.json 으로 **이미 완주**했고(`.planning/spikes/004-gemini-omni-view-editing/`) 3파전 결과 **Wan2.7 9.9° > Omni 22.8° > GEN3C 41.1°** 로 Omni 가 졌다 (2) 단가 **$17.50 은 영상 1편 값이 아니라 100만 토큰 단가**다 — 8초 영상 = 47,302 video-out 토큰 = **$0.83**, 7월 journal `cost_usd: 0.82` 와 일치 (승인 예산이 21배 과대 전제로 잡혔다) (3) belle 이 **2026-07-20 에 회전 영상을 이미 껐다**(`31-CLOSEOUT.md`) — 이유는 품질 숫자가 아니라 *"사용자가 그것을 자기 자세로 착각한다 … 없는 결함을 만들어내는 시각물"* 이라는 제품 안전 판단. ★★★**판정: Omni 1.1 은 카메라를 안 돌린다** — 입력을 그대로 재인코딩해 돌려준다. belle 이 "호출 탓일 가능성도 있다"고 짚어서 **대조군으로 갈랐다**: 같은 REST shape 으로 구모델을 때리니 오늘도 7월과 똑같이 돈다 — 원본 대비 평균 픽셀차 **7월 preview 38.7 / 오늘 preview 37.7(회전함) / 오늘 1.1 = 2.4(원본 그대로)**. 즉 호출·프롬프트·입력 전부 정상이고 **1.1 만 편집을 안 한다**. belle 이 준 모델 카드가 같은 말을 한다 — preview 설명의 **"conversational editing" 이 1.1 에서 빠지고** keyframe control / scene extension / upscale 로 갔다. **모델 업데이트로 카메라앵글 트랙이 다시 열리지 않았다 — 7월 상태 그대로다.** ★★★**부산물: 7월 게이트의 맹점을 찾았다** — 벤더가 원본을 그대로 돌려주면 RTMW 재추론이 원본과 같은 관절을 뽑아 굴곡각 MAE 가 **0° 로 수렴**한다. **아무것도 안 한 출력이 자세 충실도 만점으로 "통과"**한다. 오늘 1.1 이 정확히 그 출력을 냈으니, 스모크 없이 10건 게이트를 바로 돌렸으면 **"1.1 이 MAE 0° 로 Wan2.7 을 압도"** 라는 거짓 통과를 belle 에게 보고할 뻔했다 → `run_gate_v11.py` 에 **회전 관문**(rotation_delta, 임계 10.0) 을 달아 자세를 재기 전에 먼저 태우게 했다. ★**전송은 stdlib urllib** — `google-genai` 1.75.0 은 서버가 "legacy Interactions API schema is no longer supported, upgrade to >= 2" 로 400 거부하고, SDK 를 올리면 backend/.venv 가 흔들린다 (visual_gen.py 가 같은 이유로 이미 REST). Interactions API 현행 shape 실측 박제 = SUMMARY §함정. **10건 게이트($8)·n=3($25) 은 회전 관문에서 떨어져 돌리지 않았다** — 원본 통과본의 자세를 재는 것은 무의미. **다음 = 어제 배포한 회전 묶음의 pdshape 외 동작 점수 미측정**(특히 kip-up 위양성 재발 이력) + belle 판정 2건(확대카드 원 위치 · 강사 교정 OTA). 게이트 4831 passed / 0 failed. Pod 0대, RunPod $2.65 ─ 이전: 2026-09-17 - ★★★**회전 묶음 배포 + 앱 E2E 확인 + 강사 교정 v2 개시 + 재학습 게이트 수리** (커밋 35cd652d/564efbb5/ec9c9ef4/02b1155a/acfd1ed7/c177488f/222e9c87). ★**인계서 = `.planning/quick/260917-day-closeout/HANDOFF.md` — 내일 첫 일 = Omni 영상 합성 spike**(belle 승인 "1~2편만", $17.50~35). ★★★**회전 묶음 배포**: `reference/_release.activeCandidate=rot180_v1`(11/11 verify PASS) + `ROT180_INVERSION_ENABLED=1` — **둘은 한 묶음, 롤백도 한 묶음**. **belle 영상 60→87점 · 정은지 자기비교 60→100점**(PROJECT.md 핵심 우려 해소). ★★★**앱 전 경로 E2E 로 확인**(앱→Lambda→Pod 실경로 첫 실측) — 로컬 예측 87 을 실제 파이프라인이 그대로 냈고 감점 기록 6건→1건, Gemini veto applied, 확대카드·음성 전부 생성(09-14 '회전이 렌더를 깬다' 오진 자리를 이번엔 후처리까지 기다려 확인). ★★★**전수 측정이 09-14 서술 2건을 정정**: (a) 기준 라이브러리는 낡지 않았다(저장본 vs 오늘 파이프라인 OFF = 88쌍 중 허용오차 초과 **0개**) (b) '허용오차 2.6배'는 **pdshape 1편의 성질** — 회전은 11편 중 **6편만** 바꾸고 5편은 전 프레임 차이 0.000000. ★★**Firestore 1MB 한도에 두 번 걸렸다**(candidate 쓰기 1,085,164 · top-level 미러 1,082,424) — 원인은 `keypointReport`/`referenceKeypointReport` 를 같은 값으로 두 번 저장하던 것. 두 필드 성격이 달라(후자는 consumer 필드가 아니라 top-level 필수) 같으면 저장 생략으로 수리. **라이브 ref-combo 는 이미 1013KB(99%) — 더 긴 기준을 넣으면 다시 걸린다**. ★**강사 교정 수집 v2 개시**(`c177488f`) — 보완운동 탭 '강사에게 확인할 점' 카드 안 한 행 + 입력 모달. 강사가 학생 폰에 직접 쓰므로 새 인증·엔드포인트·규칙 변경 0. 시뮬에서 입력→저장→Firestore→재렌더 왕복 확인. **OTA 미발행**(belle 실기기 확인 후). ★**재학습 승격 게이트 수리**(`222e9c87`) — SKIPPED(못 잼, 차단) vs DESCOPED(선언된 범위 밖, 무차단) 분리로 **승격이 도달 가능**해졌다. 정책이 아니라 **구현이 자기 독트린을 어긴 것**(모듈 주석이 이미 'kip-up/climb 은 명시 추적만'이라 적어뒀다). 단 **게이트 통과 ≠ 승격** — hard 게이트 4+4 그대로, artifact 부재는 여전히 차단. ★**belle 판정 대기 2건**: 확대 카드 `zoom_adv_left_shoulder` 원 위치(게이트가 '못 읽겠다'를 통과시켜 한쪽에만 표식 — 단정하지 않고 눈 판정으로 넘김) · 강사 교정 OTA. ★**내가 틀렸다가 고친 것 4건**(인계서 §6): 유령 결함 1건 보고 직전 회수 · 뼈위반 예측기 교정 실패 · S3 '멈춤' 오진 · 카드 원 위치 성급한 단정 철회. 게이트 **4831 passed / 0 failed**(기준선 4827+4), 앱 tsc CLEAN · node --test 268/0. Pod 0대, pod-expected=down, RunPod $2.79 ─ 이전: 2026-09-17 - ★★★**회전 묶음 배포 완료** (quick-260917-hjy, 커밋 35cd652d/564efbb5/ec9c9ef4/02b1155a). `reference/_release.activeCandidate=rot180_v1`(11/11 post-write verify PASS) + `start_server.sh:24 ROT180_INVERSION_ENABLED=1` — **둘은 한 묶음**. **belle pdshape 60→87점 · 정은지 자기비교 60→100점** = PROJECT.md 핵심 우려("정은지 41점 위양성") 해소. ★전수 측정이 09-14 서술 2건을 정정했다 — (a) 기준 라이브러리는 낡지 않았다(저장본 vs 오늘 파이프라인 OFF = 88쌍 중 허용오차 초과 **0개**, 최대 15.6°; fps 15↔10 차이도 무해) (b) "허용오차 2.6배"는 **pdshape 1편의 성질** — 회전은 11편 중 **6편만** 바꾸고 5편은 전 프레임 차이 0.000000, 20° 초과 9/88 중 5개가 pdshape 하나. 회전이 개선이라는 독립 증거 = 폐색 보간 1101→519(−53%), 건드린 6편에서 −75~−93%, 안 건드린 5편은 정확히 +0. ★**Pod 없이 답을 냈다** — 09-14 학생측 ON 분석본 + 재추출 각도로 채점 seam 을 로컬에서 직접 호출(재구현 0). 계기 검증은 저장 angle 차원 3건 정확 재현(45/38/41) + 저장 감점 근거 소수점 일치. ★**Firestore 1MB 한도에 두 번 걸렸다** — candidate 쓰기(1,085,164)와 top-level 미러(1,082,424). 원인은 `keypointReport`/`referenceKeypointReport` 를 **같은 값으로 두 번** 저장하던 것(실측 `==` True). 두 필드 성격이 다르다(후자는 consumer 필드가 아니라 top-level 필수, 전자는 resolver 가 candidate 에서 overlay) → 같으면 저장 생략으로 수리. 미러 실패 때 **ref-climb 한 편만 미러된 부분 상태**가 실제로 생겼다(포인터는 루프 뒤라 해석 경로 불변) — 실물 확인 후 수리·재실행. 곁가지: 라이브 ref-combo 는 이미 1013KB(한도의 99%) — **콤보보다 긴 기준을 넣으면 다시 걸린다**. ★유령 결함 1건 기각 — "저장 angles 와 joints3d 불일치"는 `temporal_fill` 단계 차이(갭필+5프레임 스무딩으로 11편 전부 0.00). 보고할 뻔했다. ★뼈위반은 각도 드리프트를 예측 못 한다(pdshape 뼈위반 최대 관절 드리프트 8.7° vs 최소 43.2°) → GPU 재추출이 유일한 길. 신규 `backend/scripts/promote_reference_version.py`(33-17 이 분리해 둔 flip-만 경로를 메움). 게이트 4827 passed / 0 failed. Pod 0대, RunPod $3.31. **다음 = belle 승인 3건**: 재학습 게이트 수리 · Omni 1~2편 · 강사 교정 수집 열기 ─ 이전: 2026-09-17 - **기준 11편 회전 드리프트 전수 측정 완료 — 09-14 의 "기준 라이브러리가 허용오차의 2.6배 틀려 있다"를 정정** (quick-260917-hjy). GPU 재추출 2벌(RTX PRO 4500 Blackwell, ORT-gpu 1.19.2, 리포 e3936dfb = 09-14 와 동일 코드). ★★★**계기를 먼저 검증했다** — 대조군 pdshape 이 09-14 GPU 산출과 OFF·ON 둘 다 **byte 동일(최대차 0.000000°)**, 그래서 나머지 10편 수치를 같은 신뢰도로 읽는다. ★★★**기준 라이브러리는 낡지 않았다(반증)** — 저장본(06-14~15 추출) vs 오늘 파이프라인(회전 OFF)은 88개 관절-기준 쌍 중 허용오차 20° **초과 0개**, 최대 15.6°. 09-14 가 곁가지로 적은 "237 vs 159 프레임" fps 차이도 각도에 무해. ★★★**재추출이 필요한 유일한 이유는 회전이다** — 같은 코드에서 플래그만 바꾼 OFF→ON 은 11편 중 **6편만** 바꾸고 **5편(climb·kip-up·peter-pan·power-spin·sideway-spin)은 전 프레임·전 관절 차이 0.000000**. 20° 초과 9/88 이고 **그중 5개가 pdshape 하나**; 나머지 4편(combo·elbow-twist-sister·foxtop·invert)은 각 1관절만 23~27°. 프레임별 최대차는 112~163° 인데 시간 중앙값이 상당 부분 흡수한다. ★★**회전은 '바꾼다'가 아니라 '개선한다'** — 폐색 보간(신뢰도 미달로 보간한 프레임) 합계 1101→519 (**−53%**), 회전이 건드린 6편에서 −75~−93%(invert 111→8), 안 건드린 5편은 정확히 +0. 비용은 추출 시간 +35%(2패스). ★**기각 2건** — (a) "저장 angles 와 joints3d 가 불일치한다"는 **결함이 아니다**: 갭필+5프레임 이동평균을 걸면 11편 전부 0.00 으로 닫힌다(추출 경로가 compute_joint_angles→temporal_fill 이고 불확실도 채널이 저장되지 않을 뿐). 유령 결함을 보고할 뻔했다 (b) 뼈위반 p90 으로 각도 드리프트를 예측하려 했으나 pdshape 정답에 교정하니 떨어졌다(뼈위반 최대 R.knee 드리프트 8.7° vs 최소 L.hip 43.2°) → **저장 좌표만으로는 못 재고 GPU 재추출이 유일한 길**. ★**belle 판정 1번의 성격이 바뀐다**: 묶음 요건은 유지되나 결정할 것은 "썩은 라이브러리를 갈아엎을 것인가"가 아니라 "좌표계를 바꾸는 김에 **기준 6편**을 같이 다시 뽑을 것인가"이고, 점수 연속성이 끊기는 것도 11편이 아니라 그 6편을 참조한 분석뿐이다. **운영 무접촉** — ROT180_INVERSION_ENABLED 는 start_server.sh:24 에서 여전히 0. Pod 종료 확인(0대), RunPod $4.97→$4.02. 근거 = .planning/quick/260917-hjy-reference-drift-all11/evidence/REFERENCE-DRIFT-ALL11.md ─ 이전: 2026-09-10 - **belle 실기기 반려 4건 → 앱 6건·분석 3건 수리, OTA 3회 발행** (HEAD 22729865, OTA 최신 b9b151af preview/1.2.4). ★★★**좌우 팔꿈치 건 종결** — belle 이 직접 재계산해 "왼팔이 맞다"로 닫았다(등 보임 + 역립 = 좌우 두 번 뒤집힘). RTMW 라벨은 옳았고 진짜 결함은 **관측 못 한 관절에 점수를 매기는 것**이었다(그 팔 좌표 conf 0.461, 오른팔 3점이 폴을 따라 가로 12px 안에 세로 220px 일렬 = 붕괴). 붕괴 사지 게이트를 넣었으나 **신뢰도 축은 계기가 위태로워 분리**했다 — 0.5 하한을 적용하면 6동작 14 멈춤 중 7건에서 그 멈춤 감점이 전부 사라지고, 정립 fixture 가 정확히 0.5000 으로 걸린다. ★★★**시뮬레이터 확인이 기계 검사를 통과한 결함 2건을 잡았다** — (1) 낡은 JS 번들 때문에 멀쩡한 코드를 고치러 갔다(설치 번들 13:00 vs 커밋 18:25), (2) 운동 이름을 한글로 바꾸니 **기존 분석의 모달이 전부 빈 화면** — 표시 문자열을 저장 doc↔라이브러리 조인 키로 쓰고 있었다. 불변 id 도입 + 레거시 폴백으로 수리. ★**배포 상태 실측**: pipeline Lambda 배포본이 **2026-07-22 판**(md5 대조), 레이어도 v16 vs 최신 v20 → 09-10 백엔드 변경은 하나도 반영 안 됨. Pod 0대인데 `pod-expected`=up(podwatch 가 읽는 파라미터). 다음 = **역립에서 관절 대부분을 못 보는 문제의 계기를 제대로 만드는 것**. 인계서 = .planning/quick/260910-day-closeout/HANDOFF.md ─ 이전: 2026-09-09 - **피그마 시안 ↔ 구현 대조 후 30건 수리, OTA 발행** (quick-260909-ji1, 커밋 1fe329b6/e6c0b184/c9032978/16392efd/cf84da6a, OTA 014ffbef preview/1.2.4). belle 판정 한 줄로 9건이 결정됐다: "볼드체랑 이런거 미세한건 시각적으로 좋게 조절하면서 시안대로 최대한 맞춰줘. 시안을 중심으로 WCAG 최대한 지켜줘" → 기본은 시안, WCAG 를 어기는 시안값만 색상 유지한 채 명도를 낮췄다(요약·모달 서브 #D6D6D6→#767676 등 8종). ★★★**근본 원인 3개가 30건 중 15건을 만들었다** — (1) `typography.ts` 주석이 한글 글자폭을 1em 으로 놓고 시안 fontSize 를 역산했는데 **번들 Pretendard 의 한글 advance 는 0.864em** 이라 결과 화면 글자가 전부 11~30% 작게 들어갔다. 그 주석은 "'75' 로 검증됨" 이라 적었지만 **검증 사례가 숫자라 한글 오류를 드러낼 수 없었다**(숫자는 advance 가 다르다). (2) 카드 여백에 화면 전용 36.6pt 대신 앱 공통 `spacing.screenX`(20)를 써 카드가 45pt 넓었다. (3) 벡터 **잉크** 치수를 Ionicons `size` 같은 **박스** 치수로 넘겨 아이콘이 시안의 61% 였다. ★★★**시뮬레이터가 잡은 것: 받침이 잘렸다** — 글자를 시안 크기로 키우자 '보완운동'이 '보와우동'으로 렌더됐다(`titleH`/`tabsH` 가 잉크 높이인데 행 height 로 쓰여 라인박스가 넘침). tsc 0 도 테스트 226/0 도 통과시켰다. 행 높이를 fontSize×1.2 라인박스로 바꾸고 marginTop 에서 0.165em 을 당겨 해결. ★배선 누락 2건도 이때 드러났다 — 시안의 우상단 공유 아이콘이 4탭 모두 안 그려지고 있었고(컴포넌트는 준비됐는데 호출부가 `onShare` 미전달), 교정포인트 첫 행이 시안과 달리 접혀 있었다(초기값 null). ★고치지 않은 것 = 브랜드색이라 belle 판정 필요: 감점 칩 글자 #FF4B33 on #FCEFED 2.97:1(4.5 필요), 흰 글자 on 브랜드 3.33:1. 탭 라벨만 17→18pt 로 올려 large-text 예외로 우회했다. 실기기 4탭+모달 실물 확인 완료. **다음 = belle 캡처 후 의견**(belle 지시: "실제 앱에 적용된 걸 가지고 내가 캡처해서 의견을 나누는게 좋을 듯"). 대조 전문 = .planning/quick/260909-bhc-result-redesign/audit/AUDIT-2026-09-09.md ─ 이전: 2026-09-07 - **Step 0 판정: 관절 좌표 학습을 시작하지 않는다** (quick-260907-itk, 커밋 bdaebd1c/3798bf0f/653dd7b7). belle 09-06 지시로 착수했으나 belle 09-07 재질문("학습 파이프라인이 1번인가? 300개 카운터 채우는 식 아닌가")에 **1번 아님**으로 답하고 되짚음 — 눈 플라이휠은 분석마다 라벨이 자동으로 줍히지만 좌표 라인은 "이 프레임 왼어깨가 여기 픽셀"을 사람이 찍어야 해서 카운터가 저절로 안 찬다. 그래서 Step 0 는 "학습이 필요한가"인 동시에 "라벨을 기계가 만들 수 있는가"를 물었다. ★사전 기준을 측정보다 먼저 커밋(bdaebd1c) — 임계 0.25×card_frac · 면별 판정 · 교차표 (학습 허가 칸 1개) · 사전 예측 2건 · 한계 6건. ★★★**착수 즉시 자기 점검으로 09-06 "불일치 8면"을 5면으로 정정** — 3면은 계기 자신의 크롭로그 조회 실패(advisory 카드는 묶음 region=arms/legs+criterion=none 으로 로깅되는데 관절 이름으로 찾아 vertex_centered 가 None 으로 남고 "중심 안 맞춤" 분기가 안 탔다). 정정 집계 ok 13·nca 18·mismatch 5, 눈 재질의 0. 측정 결과: **A(MediaPipe 대조) 대상 AGREE 5/0 · 대조군 10/2/1 → 학습 불필요** · **B(다중프레임 다수결) 대상 HOLDS 5/0 → 다수결은 수리가 아니다**. 사전 예측은 A 적중(박제한 5.5px 이 5.54px 로 재현) · B 빗나감. ★★★**적대 검증 3갈래 중 판정논리가 반려했고 받아들여 판정을 좁혔다** — (1) **B 는 빈 계기**다: 대칭 창 중앙값은 쓸려가는 관절에서 구조적으로 중심을 돌려준다(생존 표본이 임계의 1.20~2.09배로 흩어졌는데 이동량은 임계 밑; 붕괴 배제가 지운 이웃 108·116 이 내가 MOVES 근거로 지목한 바로 그 이웃들). 교차표 두 번째 다리로 쓸 수 없다 — B 가 말할 수 있는 건 "시간축 평활기는 수리가 아니다" 하나. (2) 심판 자격이 얇다: MediaPipe 이견이 전부 오늘 파이프라인 쪽(학생 대조군 7면 중 2면, 같은 프레임 한 장), 기준측 0면 — §6-6 "두 쪽을 합치지 않는다"를 어겨야 반증자를 경계에서 통과. (3) §3 "UNAVAILABLE 2면 이상"에 범위를 안 적은 것은 내 사전 기준 결함. (4) §6 한계가 산출물에 미탑재 — 라이브 확인 결과 **대상 5면 중 3면이 2026-06 기준 좌표(ref-climb=bukuroo-06-06, ref-elbow=direct-06-12)라 학습으로 못 고친다**. **벌어들인 결론 = 행동은 확정(학습 안 함), 근거는 A 한 다리이고 그 안에서 오늘 파이프라인은 대상 2면뿐(pdshape user left_knee 1.17px/47.9 · 파워스핀 user left_shoulder 5.54px/36.0). "좌표는 맞다"는 벌지 못했다.** ★★★**게이트 실측(GATE-COVERAGE.md): 게이트는 사진을 못 없앤다** — card_gates.py:972 "사진은 그 멈춤 그대로, 표시만 생략. 이 함수는 카드를 없앨 수 없다" = belle 09-03 규칙의 구현. 따라서 **승인된 방향 1번("자동 사진은 게이트 확인분만")을 인계서 문구대로 지으면 belle 09-03 규칙을 깬다 — belle 결정 필요.** 표시 기준 실측 = 확인 27·미확인 9·양쪽 확인 카드 12/18, **0장 되는 동작 없음**(최소 파워스핀·킵업 1카드). 그리고 **불일치 5면 중 3면이 게이트 pass 안에 있다** → 방향 1번만으로는 belle 이 본 문제가 안 고쳐진다(belle 방향 2번 = 음성 짚는 순간 멈춤·확대·캡처를 사용자 손에, 가 더 강한 근거 하나 추가). 다음 계기 = 이름표 골격 오버레이(skel_overlay_measure.py, GPU 불필요) — 좌표 오류/크롭 폭/눈 어휘를 가른다. 기준 재추출은 §7-7 대로 안 건드렸다. ─ 이전: 2026-09-07 - **n2j 라이브 실증 PASS** (사전 박제 기준 2개 다 통과: 게이트 없던 stage-1·advisory 경로 전부 실행·신규 경로 표시 8건 억제 / **오판 0**; gated 경로 moved=1 첫 사례; 문서당 눈 호출 15.5회, 6문서 644초). ★★★**판정에 쓴 계기 자체가 결함이었음을 먼저 잡아 자기 철회** — panel_center_eye_measure v1 의 결함 3건(크롭비율 0.42 리터럴 vs 실측 0.40~0.55 / 표시 그려진 그림을 무표시 전용 질문에 투입 → 36면 중 4면 판정 뒤집힘 / '패널 정중앙=앵커' 가정이 18카드 중 10카드에서 거짓)을 라이브 로그로 확정하고 v2 로 재측정: **36패널 ok 16 · mismatch 8 · not_center_anchored 12** (v1 은 mismatch 15). j8g '불일치 9장' 판정 철회. quick-260906-vho 완료 (advisory·legacy userMarked 방출 + 감사 marked_flags per-key + panel_error_kind + 마지막 줄 mark_errors=/center_errors= 두 열, pytest 4724/0/20 = 기준선 4717+7, replay total=18 mismatched=6 unresolved=0 mark_errors=1 center_errors=5). ★★★**belle '이렇게 판정해주는거 자체가 짜맞추는거 아냐?' → 어깨 각도 문법을 사전 박제(2906b272, 23:04:32) 후 측정.** M1(모델 없는 기하)에서 **내 기전 주장이 깨졌다** — beyond_hip 최대 0.884 로 한 면도 골반을 안 지나친다. 안 B 는 8면 렌더 sha1 이 A 와 동일(사전 예측 적중), C 는 대조군 2건 악화로 패배, D 는 지표 미정의. **A 승리도 선언 불가 — 사전 기준이 자기모순이었고 심판이 한쪽을 고르면 그게 짜맞춤이라 충돌을 그대로 보고.** 예상 못 한 사실 2: belle 이 본 파워스핀 학생 패널은 브랜드 픽셀 0(게이트 억제)이라 각도 문법 문제일 수 없다 · peterpan 2면은 라이브에서 이미 C 문법(멈춤 상속이 꼭짓점을 display_anchor 로 갈아끼우고 저장 안 함) = 대조군 6면 중 4면이 반사실. ★belle 승인 방향: 자동 사진은 게이트 확인분만 + **음성 짚는 순간에 멈춤·확대·캡처를 사용자 손에**(belle 제안). ★belle 지시: **내일 첫 일 = 관절 좌표 학습 라인** — 플라이휠(Qwen3-VL)은 결함 짚는 눈만 학습하고 좌표(RTMW 고정)는 학습 경로에 없다. 착수점 = CONTINUE-2026-09-07.md. 판정지 = https://claude.ai/code/artifact/30f7da02-169d-4b44-a203-319e86a9db55 ─ 이전: 2026-09-06 - quick-260906-n2j 완료 (stage-1·advisory 카드에도 앵커 부위 확인 게이트, pytest 4717/0/20 = 기준선 4701 + 신규 16). ★★★j8g 라이브 실증 + **세 번째 계기로 판정**: 배달된 카드 패널을 게이트와 같은 시야(패널 정중앙 42.9% = 원본 짧은변 18%)로 다시 잘라 5회 최빈으로 물었다 — 28패널 중 **중심이 딴 부위 9**. 처분별로 갈면 게이트가 잡은 것 3 · 놓친 것 3 · **게이트가 없던 경로 3**, 그리고 **게이트가 표시를 지운 3면은 전부 실제 불일치 — 오판 0**. 실패 방향이 안전한 쪽이라 게이트를 남기고 범위를 넓혔다(n2j). belle 09-04 '얼굴에 동그라미'(pdshape 오른팔꿈치 학생)는 두 계기 모두 head 판독, 라이브에서 표시 사라짐. ★감사(card_photo_audit) 지표(불일치 6/14)는 이 수리를 재는 계기가 아니다 — 표시를 지우면 mark_elsewhere → no_mark_and_center_elsewhere 로 바뀔 뿐 둘 다 불일치로 센다. **게이트가 고치는 것은 '틀린 표시'이지 '틀린 사진'이 아니다**(틀린 사진은 맞는 좌표 = 자세 모델의 일). **라이브 미검증**(n2j 는 다음 Pod 번들) — 다음 Pod 첫 일 = 09-03 6영상 재분석 → fault_zoom_anchor_check path=stage1|advisory 로그 회수 + 배달 패널 정중앙 재측정(panel_center_eye_measure.py). 남은 것: 게이트가 통과시킨 불일치 3(원본에선 허용·배달본에선 아님 — 원인 단정 금지), 감사에 '표시 오류/중심 오류' 열 분리, 어깨 각도 문법(belle 판정). ─ 이전: quick-260906-j8g 완료 (앵커 부위 확인 게이트, pytest 4701/0/20 = 기준선 4669 + 신규 32). ★★★09-06 진짜 원인 박제: **관절 좌표 자체가 틀린다** — 라이브 6문서 36패널을 좌표에 바짝 붙여(짧은 변 18%) 자른 뒤 눈에 물으니 **좌표 OK 15 / 틀림 14 / 못읽음 7**. conf 0.87 판독불가·0.75 무릎자리에 손·0.74 팔꿈치자리에 허벅지·0.64 어깨자리에 얼굴 — **신뢰도도 붕괴 검사도 예측 못 한다**(육안 확인 3건 첨부). 그래서 같은 날 f4j(붕괴 프레임 배제, 라이브 실증 moved 11·앵커 붕괴 14/42→4/36)가 지표를 못 움직였다 — 감점 카드 눈 불일치 4장 → 4장 그대로. 카드 감사가 통과시킨 카드에도 틀린 좌표가 있다(넓은 크롭 42%가 가림). j8g = 그래서 **보는 게이트**를 생산 경로에 붙임(gated 호출부, belle 09-03 규칙 '검사는 표시만 정한다' 그대로 — 카드 삭제 권한 없음). 비용 = 카드당 통상 눈 4회·최악 21회, 문서당 통상 ≈20. **라이브 미검증** — 다음 Pod 때 09-03 6영상 재분석 → fault_zoom_anchor_check 로그 회수 + audit_card_photos.py --rounds 5 재감사(감점 카드 불일치 4장 감소 여부). 남은 것: stage-1·advisory 호출부 배선(효과 확인 후 즉시), 어깨 각도 문법. ─ 이전: quick-260906-f4j 완료 (멈춤 상속 카드 붕괴 프레임 배제, pytest 4669/0/20 = 기준선 4655 + 신규 14). ★09-06 조사 박제: 확대 카드가 제목과 다른 부위를 보여주는 원인은 "신뢰도 미달 좌표로 crop"이 아니라 **붕괴 프레임**이고, 붕괴 14패널(학생 5·정은지 9)이 **전부 gated(멈춤 상속) 경로**에 있다 — 그 경로가 dtw_match=None + at_frame_idx=None 으로 프레임을 고정해 넘겨 08-10 붕괴 방어(_drop_collapsed·select_confident_frame·_ring 승급)가 구조적으로 미도달. stage1 3장·advisory 6장 붕괴 0. 반사실(학생 불건전 5패널 재렌더 + 기계 눈 재질문): 1장 불일치→통과, 4장 유지, **악화 0**. 수리 적용 실측 = moved 11 / stuck 3 / 비붕괴 28 무변화. belle 09-05 지적 7장 중 이 수리가 닫는 것은 3장 — 파워스핀 왼어깨는 별건(어깨 각도 문법: 꼭짓점=겨드랑이 _ARMPIT_T 0.15, 두 선=ANGLE_BAKE_MAP["shoulder"]=(elbow,hip), 승인 7R#2라 belle 판정 필요), 참고 카드 3장은 belle 09-03 보류 계열. **라이브 미검증** — 다음 Pod 때 09-03 6문서 재분석 → fault_zoom_gated_frame_shift 로그 회수 + audit_card_photos.py 재감사. 보고서 = https://claude.ai/code/artifact/75f3d993-2c13-4c93-bb81-34f7e063fe72 ─ 이전: quick-260905-ota 완료 (감사 2단화, pytest 4655/0, 라이브 2런 21/21 재현 — **21장 중 7장이 제목과 다른 부위**). 그 앞 quick-260905-mvm (감사 도구 반입). 그 앞 quick-260905-mbd (Gemini Flash 정본 3.8). ★조사 결과 박제: **양다리 벌림 축이 Gemini 단독 의존** — `required_split_deg` 가 항상 None 이라 기하 경로가 사문, 09-03 6동작 중 5동작 gemini_silent → 그 축 흔적 0(파워스핀 belle 지적의 원인). spotCheck 는 "정은지 대비" 상대 항목을 프레임만으로 판정 못해 6동작 거의 전부 uncertain. 그 앞: **확대 사진 대장 자체 감사 — 21장 중 5장이 제목과 다른 부위를 보여준다**(belle 이 먼저 발견: pdshape 오른팔꿈치·파워스핀 왼어깨·엘보 3장). 감사 대상은 중간 좌표가 아니라 완성된 카드 사진(경로가 rep12/align 17kp 두 갈래라 좌표 감사는 오판). 대장 아티팩트에 점검 칸 부착·게시. 미해결: 감사를 통과 기준으로 승격(현행 "6/6 PASS"는 장수만 셈), 게이트 미달 좌표로 crop 금지, 파워스핀 split 축 침묵. ─ 이전: 2026-09-04 새벽 마감 - 09-03 하루: 확대비교 앱 배선 3건(f2w·ftg·lr6) + 참고 카드 ×(ik4 되돌림) + 눈 3건(jka·jxn·lpl) + **멈춤 구간마다 사진 1장 보장(upx, 라이브 6/6)** + 1.2.4 배포(iOS 40·APK). belle 계정 pdshape 재분석 5장 확정(895603fb) — 내일 belle 폰 확인. Pod 종료·pod-expected=down. 착수점 = CONTINUE-2026-09-04.md
...
이전: 2026-08-31 - **1.2.0 양 플랫폼 빌드 발행 (belle "빌드해보자, 안드로이드도 같이")** — iOS 빌드 31 EAS FINISHED + TestFlight 자동 제출(빌드 30은 EAS 서버측 업로드 오류 SERVER_ERROR → 재시도로 해소), Android APK preview-android FINISHED (설치 링크 CONTINUE 상태 블록). 남은 확인 = belle 실기기(TestFlight 업데이트 + 구글 로그인 uid 승계 + 결과 화면 재구성 육안). 부수: apple-design 스킬 설치(.claude/skills/ — 오늘 결과 화면 진단에 사용), taste-skill 플러그인 존재 확인(설치는 belle 대기)

이전: 2026-08-31 - Quick task 260831-lcc: **결과 화면 재구성 (belle "보고서 유익한데 보기 힘들다" → apple-design 스킬 진단 → "구조 바꾸는거 오케이" 승인)** — 원인 5종(한 발견 5곳 반복·점수가 4번째 섹션·빨강 남용·면책 6곳·빈 카드 렌더) 수리: 옥타곤 첫 화면 이동, 발견 중복 제거(cuePill 삭제·심사카드 headline 제거), '없음' 삽입 버그 sanitizeFinding 소독, 본문 빨강 제거(highlightNumbers), 빈 상태 미렌더, "점수80점" 붙음(alignItems 수축) 수리, hangul-word 줄바꿈, 마이 스테일 문구, **v1.2.0 범프**. typecheck 0에러·node --test 208/0·정보손실0 감사 10/10 잔존·동작비교 diff 0. **시뮬 before/after 눈검증 PASS**(before-screens/ vs after-screens/, 크래시 0). 잔여: 팁 내 "강사와 확인" 반복은 연결어미 문장이라 접기 미발화(백엔드 생성 문구 — 표시 레이어 한계, 후속 후보). OTA 금지 유지 — 새 빌드로만. 커밋 643d7fd2~cbc89ef9

이전: 2026-08-31 - Quick task 260831-isk: **kip-up 방향 FAIL 수리 (belle "1번 수리하자")** — 진단(260831-kipup-diagnosis/DIAGNOSIS.md): vision 은 오늘도 결함을 변별(fault=split 20° 지지 1건 / correct=0건, 로컬 재현+대조)하는데 reference_relative tol 20° 재적용이 경계(dev==tol)에서 over=0 → 감점 소멸. 6월엔 같은 영상을 50° 로 추정해 -20(모델 세대 드리프트 50→20). 기하 split 은 여전히 180° 포화로 측정 불가(6월 소견 유지). 수리 = `_criterion_deduction` vision_sourced 플래그 — vision-측정 편차는 tol 우회(over=dev), geometry byte-불변, 새 상수 0. 예측 일치: kip-up fault record -20 → final 80 < correct 100 방향 복원. 4537/0(기준선 4532+신규 5, 기대값 정정 7사이트 정당화 — 결함을 "정상"으로 박제했던 재현 테스트 뒤집음). ★라이브 E2E 재확인은 다음 Pod 기동 때 킵업 페어 1회 재실행으로 묶기. 커밋 b35e2512~b23d9e2a

이전: 2026-08-31 - Quick task 260831-gyk: **hold 창 전환부 오염 수리 (belle "고쳐라")** — 파워스핀 정타 -20 위양성(belle × 판정 "아니 쫙 펴져 있어")의 뿌리 = Gemini hold ±2초 창(54,90)이 스핀 진입 전환부를 평균에 섞음(창 평균 135.81° = 감점 record 와 소수점 일치 역산). 수리 = `_select_window` 가 힌트 창 **내부에서** 기존 분산-최소 부창 재선택(단일 함수 4줄, 새 튜닝 상수 0, 동작명 분기 0, 33-A4 국면 게이트 유지). 실데이터 사전 박제 4/4 PASS: 부창 (54,90)→(80,89), correct 무릎 평균 173.4°(감점 소멸, line 0→93), fault 방향 보존(158.4<173.4). 테스트 4532/0(기준선 4528+신규 4, 기대값 변경 5건 정당화 주석). fps 9.0 리터럴은 명시 보류(후속 후보). ★플라이휠 TRAINING-DUE 자동 판정 떴음(admit 325≥60, 수집 371<400) — belle 알림 대상. 커밋 74d2c977~07df0c4f

이전: 2026-08-31 - **6동작 페어 스윕 (belle "돌려라" — 2차 L4 ~30분 $0.27, Terminate 완료)** — 정은지 잘못된예시/잘된예시 12런 mode1 E2E. **5/6 방향 PASS** (peter-pan 80→100, power-spin 60→80, climb 92→100, elbow 82→100, pdshape 63→100), **kip-up FAIL(100=100 — 틀린 영상 감점 0)**: 신호는 잡힘(어깨 델타 23°, DTW 27 vs 6.5 저신뢰)인데 deduction records 0 + visionVeto not_applicable 로 감점 입구 둘 다 닫힘 — phase24-A 이후 재발 관측, 수리는 belle 결정 대기. power-spin correct -20(무릎 135.8° 측정, 6.71s)은 사진 ○× belle 대기 — 사전 박제 예측: 원근 단축 측정 오류. 증거 .planning/quick/260831-pair-sweep/. 부수: 익명가입 연속 400(남용 방지)→uid 재사용 우회, 엘보 원본 107MB 정상 거절. 잔액 $10.14

이전: 2026-08-31 - **Pod 검증 4런 완주 (belle "켜서 하자" 승인 — L4 20분, $0.17, Terminate 완료)** — ① 코칭 수리(810eb959) 재측정 PASS: 75초 영상 코칭 완전 섹션 3/3 + 실전 잘림 발생 시 salvage 로그("완결 항목 2개만 사용") 실증. ② mode3 전후 페어 E2E PASS(데이터 면): isFirst:false·previousAnalysisId·deltaFromPrevious 생성 (이 페어 delta 0 — 양쪽 stability 98). ③ postureAxes(260831-bjj) 실코칭 PASS: 코칭 원인에 "상체가 기준보다 6도 정도 더 기울어지면서..." 인과형 반영, 방향 정확. mode1 sanity overall 80. 관측: GET /reference 10s 타임아웃 500 — 앱은 Firestore 직독이라 소비처 0. GPU 확증(RTMW 75초→25.5초). E2E 드라이버 리포 영구화 backend/scripts/e2e_app_path.py (ac8db8ff). 미푸시 커밋은 git bundle 로 Pod 반입(origin 무접촉). 잔액 $10.41

이전: 2026-08-31 - Quick task 260831-c3l: **미실행 꼬리 6건 삼진 분류 마감** (CONTINUE-2026-08-31 #2. "5건" 표기였으나 실측 6건) — 사문 2(01-24 RTMW research 스캐폴딩=프로덕션 rtmw_engine 이 대체 / 12.5-01 UI 투명성=화면 면이 D-03/D-12 로 제거돼 목표 사문), 실질완료 3(01-25 RTMW swap=파이프라인이 `_RTMW_ENGINE` 소비 중·NLF 표기 잔존 / 24-03 게이트=pytest 21 passed / 25-04 게이트=pytest 25 passed·07-04 스윕 FAIL 기록 실재는 관측 박제), 파킹 1(20-04 SEVERITY_CAP freeze-lock=산출물 4종 부재 진짜 미실행, **SCORE-09 는 별도 PENDING 잔류** belle 06-23 D-14/D-15). 각 스텁에 실행자 직접 재확인 커맨드+결과 박제(오케스트레이터 주장 승계 금지 규율 — 12.5-01 에서 실제로 오케스트레이터 오진 1건을 잡아 근거 교체). 코드 변경 0. 커밋 5c5718fb

이전: 2026-08-31 - Quick task 260831-bjj: **belle 08-17 판독 축 구현 (상체 꼿꼿함·머리-척추 1자)** — 13일 방치된 정답지를 코드로 옮김(CONTINUE-2026-08-31 #1). features.py 순수 함수 2종(`torso_uprightness_series` 척추벡터 vs up 기울기 0°=꼿꼿·델타 전용, `head_spine_alignment_series` 골반-어깨-귀중점 collinearity 180°=1자) + `posture_axis_summary`(nanmedian, significant≥5°). mode1 배선 = `_build_coach_context` postureAxes 키 → 양 writer 공유 `format_posture_axis_lines`(significant+학생 열위 방향만 발화, 미발화 시 프롬프트 byte-불변). **피터팬 사전 박제 검증 PASS**: 예측 부등식 박제 후 실측 ref 10.75° < user 20.59°(delta +9.84°) = belle 원문 "상체의 꼿꼿해짐" 방향 일치. 테스트 4528/0(기준선 4496+신규 32), 동작명 분기 0, 점수 경로 무접촉. 커밋 7aed6ff7~c5b2fe6c

이전: 2026-08-24 - Quick task 260824-pqc: **일러스트 기능 전면 제거** (belle 08-24 결정: "일러스트 기능을 아예 빼고 서비스를 구성하자. 확대비교랑 모션 분석에 집중") — 컴포넌트 1(DefectIllustration 621줄) + lib 4종(illustrationScene/illustrationHow/progressCaption/ghostPose) + 테스트 4종 + 에셋 21장 삭제, 배선 3면(시트 슬롯·illu-float 큐·발전 캡션) 제거, 총 −2,892줄. 시트 goalLine 항상-텍스트 경로 성립(그림 카드 분기 소멸), 실사진 비교·원인·수치·미션 잔존. VideoCompare·deductionSheet 코드 무접촉(주석만). 게이트 GREEN: typecheck + node --test 201/201 pass fail 0(기지 실패 8 = 파일 삭제로 소멸, 234→201). **OTA 미발행**(belle 한마디 대기 — 발행 준비 상태), 시뮬 실증 = 오케스트레이터 후속. 커밋 eddbdf0e/fb2eef19

이전: 2026-08-24 - Quick task 260824-gt1: 파워스핀 다리 ghost-noarrow 잔상 후보 4장 (belle 판정 전) — 실측 유래 오류 2유형(무릎 약 39° 굽음 / 스플릿 약 30° 좁음), 예측 박제 커밋 → 생성(gemini-3-pro-image 4호출) → 전 장 Read 실물 게이트. D-03 표기 0 = 4/4, **split_angle 2/2 방향 역전 탈락**(exq stage40-1 동일 실패 모드 — 각도 크기 차만 있는 오류에서 역전, 형상 범주 오류는 2/2 성립). 추천 = leg_extension-1·-2. app/ 무접촉·배선 없음(D-05), belle 판정 = 오케스트레이터 대화

이전: 2026-08-24 - Quick task 260824-bxf: 발전 캡션 문턱 criterion별 전환 완료 — split_angle 플립 교차표 재실측·원장 박제(historical 플립 52.7% — 08-22 구두 36.4% 불일치 그대로 박제, P95 20.00 은 원장과 일치), PROGRESS_NOISE_THRESHOLDS(기본 12 + split_angle 21) + criterion 배선(result.tsx→DefectIllustration→buildProgressCaption), 전량 게이트 GREEN(233 pass/기지 1). OTA 미발행(belle 결정 대기 유지), 시뮬 실증 = 오케스트레이터 후속
...
|---|-------------|------|--------|-----------|
| 260919-0d3 | **인수인계서 — 2026-09-18 오후~09-19 세션** (정리 1·2차 · 확대카드 짝 결함 규명 + belle 판정 4점 · 짝 품질 지표를 자세→시간 정렬로 교체 · 내 계기 오류 5건 교정) | 2026-09-19 | (이 커밋) | [260919-0d3-day-closeout-2026-09-19](./quick/260919-0d3-day-closeout-2026-09-19/) |
| 260918-k06 | **착수점 정리 — 진원지 3건 해소** (착수점 4개→1개 · STATE 원장 누락 46건 복구+날짜순 단일 표 · belle 유령 판정 8건 정정 · TRAINING-DUE 성적표 5건 정정 · .gitignore 사업문서 보호). 코드 무접촉 실측 0파일 | 2026-09-18 | `b47e53b5` | [260918-k06-entrypoint-consolidation-1-state](./quick/260918-k06-entrypoint-consolidation-1-state/) |
| 260918-gpx | 게이트 false-suppress 조사 — 결함 2건 | 2026-09-18 | eaafcfbe | [260918-gpx-gate-false-suppress](./quick/260918-gpx-gate-false-suppress/) |
| 260918-gate | 재학습 승격 게이트 — SKIPPED 와 DESCOPED 를 가른다 | 2026-09-17 | 222e9c87 | [260918-gate-semantics](./quick/260918-gate-semantics/) |
| 260918-fyf | 회전 묶음 배포가 pdshape 외 동작에 한 일 — kip-up 99→99, power-spin 73→73 (나빠진 동작 없음) | 2026-09-18 | 27990829 | [260918-fyf-rotbundle-other-motions](./quick/260918-fyf-rotbundle-other-motions/) |
| 260918-et1 | Omni 1.1 카메라앵글 재게이트 — 1.1 은 편집을 안 한다, 재게이트 중단 ($1.66) | 2026-09-18 | c5bd69b7 | [260918-et1-omni11-regate](./quick/260918-et1-omni11-regate/) |
| 260918-day | 인수인계서 — 2026-09-18 | 2026-09-18 | d96d18e5 | [260918-day-closeout](./quick/260918-day-closeout/) |
| 260918-0q8 | 강사 교정 수집 v2 개시 — 결과 화면에서 받는다 | 2026-09-17 | c177488f | [260918-0q8-coach-review](./quick/260918-0q8-coach-review/) |
| 260917-hjy | **회전 묶음 배포 — 기준 11편 rot180_v1 승격 + 플래그 ON (belle 60→87점 · 정은지 자기비교 60→100점)** + 09-14 서술 2건 정정 (GPU 재추출 OFF/ON 2벌, 대조군 pdshape 이 09-14 산출과 byte 동일). ★**기준 라이브러리는 낡지 않았다** — 저장본 vs 오늘 파이프라인(회전 OFF)은 88개 관절-기준 쌍 중 허용오차 20° 초과 **0개**(최대 15.6°), fps 차이(15fps↔10fps)도 무해. ★**재추출이 필요한 유일한 이유는 회전** — OFF→ON 은 11편 중 **6편만** 바꾸고 5편은 전 프레임·전 관절 차이 **0.000000**; 20° 초과 9/88 이고 **그중 5개가 pdshape 하나**, 나머지 4편은 각 1관절만 23~27°. 즉 09-14 의 "허용오차의 2.6배"는 pdshape 1편의 성질이지 라이브러리 성질이 아니다. ★**회전은 개선이다** — 폐색 보간 1101→519(−53%), 건드린 6편에서 −75~−93%, 안 건드린 5편은 정확히 +0; 비용은 추출 시간 +35%. ★기각 2건: (a) "저장 angles 와 joints3d 불일치"는 결함이 아니라 `temporal_fill` 단계 차이 — 갭필+5프레임 스무딩으로 11편 전부 0.00 으로 닫힌다(유령 결함을 보고할 뻔했다) (b) 뼈위반은 각도 드리프트를 예측 못 한다(pdshape 에서 뼈위반 최대 관절의 드리프트가 최소) → 저장 좌표만으로는 못 재고 GPU 가 유일한 길. **운영 무접촉**(ROT180 여전히 0), Pod 종료 0대, RunPod $4.97→$4.02 | 2026-09-17 | 02b1155a | [260917-hjy-reference-drift-all11](./quick/260917-hjy-reference-drift-all11/) |
| 260917-day | 인수인계서 — 2026-09-17 | 2026-09-17 | e392e9db | [260917-day-closeout](./quick/260917-day-closeout/) |
| 260914-rot180 | 인수인계서 — 2026-09-14 | 2026-09-14 | 35cd652d | [260914-rot180-gpu-validation](./quick/260914-rot180-gpu-validation/) |
| 260913-vqr | 3주간 조용히 죽어 있던 주간 수집을 sys.path 한 줄(x3)로 되살리고, 하위 프로세스 테스트로 박제하고, 다음 실패는 마커+알림으로 표면화되게 했다 | 2026-09-13 | e3936dfb | [260913-vqr-flywheel-collection-unbreak-sunity-share](./quick/260913-vqr-flywheel-collection-unbreak-sunity-share/) |
| 260913-udr | rot180 inversion pass — 요약 | 2026-09-13 | 601d60d4 | [260913-udr-rot180-inversion-pass](./quick/260913-udr-rot180-inversion-pass/) |
| 260913-day | 인수인계서 — 2026-09-13 | 2026-09-13 | e3936dfb | [260913-day-closeout](./quick/260913-day-closeout/) |
| 260910-woq | 운동 식별자를 표시 이름과 분리 | 2026-09-10 | 22729865 | [260910-woq-exercise-id](./quick/260910-woq-exercise-id/) |
| 260910-vwh | 보완운동 한글 이름 + 분석별 구성 | 2026-09-10 | 22729865 | [260910-vwh-kor-exercise](./quick/260910-vwh-kor-exercise/) |
| 260910-u67 | 보완운동 상한 6 → 5 복귀 | 2026-09-10 | 22729865 | [260910-u67-cap5](./quick/260910-u67-cap5/) |
| 260910-sus | unjudgedJoints 정규화 — 계획서 진단이 틀렸다 | 2026-09-10 | b14d6f4f | [260910-sus-unjudged-normalize](./quick/260910-sus-unjudged-normalize/) |
| 260910-pbs | 보완운동을 분석마다 다르게 | 2026-09-10 | b14d6f4f | [260910-pbs-exercise-count](./quick/260910-pbs-exercise-count/) |
| 260910-ovo | 붕괴 사지 판정불가 게이트 | 2026-09-10 | b14d6f4f | [260910-ovo-collapse-gate](./quick/260910-ovo-collapse-gate/) |
| 260910-oon | 관절선 토글 0초 깜빡임 제거 | 2026-09-10 | b14d6f4f | [260910-oon-swap-flash](./quick/260910-oon-swap-flash/) |
| 260910-hsk | 결과 화면 점수 아래 날짜 표기 | 2026-09-10 | 3af2517f | [260910-hsk-dial-date](./quick/260910-hsk-dial-date/) |
| 260910-euz | 관절선 토글 재생위치 유실 수리 | 2026-09-10 | 3af2517f | [260910-euz-overlay-toggle-seek](./quick/260910-euz-overlay-toggle-seek/) |
| 260910-day | 인수인계서 — 2026-09-10 | 2026-09-10 | d082c6f6 | [260910-day-closeout](./quick/260910-day-closeout/) |
| 260909-ji1 | 분석 결과 화면 — 피그마 시안 대조 30건 수리 (시안 중심 + WCAG AA) | 2026-09-09 | cf84da6a | [260909-ji1-sian-match](./quick/260909-ji1-sian-match/) |
...
| 260907-k43 | 확대비교 방향 전환 — 슬라이스 1 (OTA) · 마감 | 2026-09-07 | e9bf1bfc | [260907-k43-1-ota](./quick/260907-k43-1-ota/) |
| 260907-itk | Step 0 관절 좌표 학습 필요 판정 — 사전 기준 박제 후 두 측정 (판정: 학습 안 함) | 2026-09-07 | 653dd7b7 | [260907-itk-step-0](./quick/260907-itk-step-0/) |
| 260906-vho | 감사 계기 결함 2건 — advisory·legacy 카드도 userMarked 방출(refMarked 는 §11.9 정책대로 criterion 카드만) + 감사 marked_flags per-key + `panel_error_kind` 순수 함수 + 마지막 줄 `mark_errors=/center_errors=` 두 열. 라이브 replay = total=18 mismatched=6 unresolved=0 mark_errors=1 center_errors=5. pytest 4724/0/20 (기준선 4717+7). `_CROP_FRAC`·마커 반지름 byte 무변경(belle 판정 대기) | 2026-09-06 | 7583e7ca | [260906-vho-audit-mark-columns](./quick/260906-vho-audit-mark-columns/) |
| 260906-n2j | stage-1·advisory 카드에도 앵커 부위 확인 게이트 — build_fault_zoom_comparisons 카드별 anchor_check 콜백 + 공용 팩토리 1벌 두 호출부 배선, 재확인 프레임 미사용(라이브 moved=0/15) | 2026-09-06 | 105fa15e | [260906-n2j-stage-1-advisory](./quick/260906-n2j-stage-1-advisory/) |
| 260906-j8g | 앵커 부위 확인 게이트 — 카드 표시 전 양측 앵커 좌표를 좁은 크롭(짧은 변 18%)으로 기계 눈에 확인, 실패 시 창 안 최대 2프레임 재확인 후 **표시만 생략**(사진·장수 불변) | 2026-09-06 | e58a2edb | [260906-j8g-anchor-part-verification-gate](./quick/260906-j8g-anchor-part-verification-gate/) |
| 260906-f4j | 멈춤 상속(gated) 카드 경로 붕괴 프레임 배제 — 앵커 붕괴 시 ±4 안 가장 가까운 성한 프레임으로 표시 프레임 이동 (09-06 실측 42패널 중 붕괴 14, 전부 gated / 적용 결과 moved 11 · stuck 3 · 비붕괴 28 무변화) | 2026-09-06 | bf250cc6 | [260906-f4j-gated-card-collapse-frame-skip](./quick/260906-f4j-gated-card-collapse-frame-skip/) |
| 260905-ota | **감사 2단화 — 표시가 놓인 부위로 경계 판정**. 1단(정중앙 부위) 탈락 + 표시 있는 패널만 2단(`mark_part`: 빨간 표시가 어느 부위에) 질의 → `by=center|mark|none` 4결말. `card_gates` `mark_part` claim(좌우·기대관절 0, enum sha256 핀) + 순수 `adjudicate`/`card_verdict` + 스크립트 2단화 + `--replay`(눈 호출 0 재판정). pytest **4655/0**(+28). 라이브 2런: 둘 다 `total=21 mismatched=7 unresolved=0`, **카드 판정 21/21 일치**. 2단이 2장 구제(중앙 판독이 흔들리던 카드 — 표시는 제 부위). belle 의 2장은 사유까지: pdshape 6.1초=표시 없음+중앙 등허리, 파워스핀 왼어깨=표시가 무릎(3/3). ★**허용 집합 무변경** — 표시 측정이 넓힐 근거를 주지 않음(경계 3종 전부: 무마킹이라 잴 것 없음/1단 통과/얼굴 원). 근거를 `_KIND_PARTS` 주석에 날짜와 함께 + 실측 토큰 테스트로 고정. ★실행자 발견 버그: `userMarked/refMarked` 는 **확정 카드에만 있는 필드**(참고 카드 부재)인데 mvm 스크립트가 부재를 True 로 기본 처리 → 09-05 "21장 전부 문서·그림 일치" 진술은 15장에만 참이었음. 게이트 B 정책(기준 측 무마킹)으로 수정 | 2026-09-05 | 593fd19e | [260905-ota-mark-part-adjudication](./quick/260905-ota-mark-part-adjudication/) |
| 260905-mvm | **확대 카드 사진이 제목의 부위를 보여주는지 검사하는 감사 도구** (belle 09-05 "고객이 분석해도 잘 잡아주겠거니 신뢰"). 좌표가 아니라 **완성된 카드 사진**을 검사 — crop 중심 출처가 경로마다 달라(align 17kp / rep12) 좌표 감사는 오판이 난다. `card_gates` 에 `part` claim 추가(질문에 좌우·관절명 0 — 기존 설계 승계, 테스트로 강제) + 순수 대조 모듈 `card_photo_audit` + 스크립트 `backend/scripts/audit_card_photos.py`(카드 PNG 이등분 → 패널별 눈 → 대조, 마지막 줄 `card_photo_audit total=N mismatched=M`). pytest **4627/0**(+25). 라이브 4런(09-03 21장): 카드 판정 20/21 재현, belle 이 먼저 찾은 2장은 4/4 런 전부 불일치. ★실행자 발견: `part` 는 다수결이 unclear 에만 재질문해 `--rounds` 무효 → N회 호출 모달 토큰으로 교체(단발 2회는 2/21 뒤집힘). 대장 5장 중 1장(엘보 오른팔꿈치 기준=겨드랑이) 재현 안 됨(눈 10/10 `elbow`), 대장에 없던 3장이 안정적으로 걸림(2장은 neck↔shoulder·knee↔hip 인접 경계) — 허용 집합을 이번 1벌에 맞춰 넓히지 않음 | 2026-09-05 | 62656643 | [260905-mvm-card-photo-audit](./quick/260905-mvm-card-photo-audit/) |
| 260905-mbd | **Gemini Flash 정본 3.7 → 3.8** (belle 09-05 "3.8로"). 근거 = 확대 카드 사진을 기계 눈에 보여 제목 부위와 대조하는 감사를 09-03 라이브 21장에 돌리고, 판정이 갈린 3장을 5회 반복으로 3모델 비교: 3.8 이 반복 안정성 최고(2/3 카드 5/5), 3.7 그다음, omni-1.1 은 가장 흔들리고 카드당 $0.017 최고가 + `/v1beta/interactions` 전용. `DEFAULT_C_MODEL`·`ALLOWED_MODELS` 갱신하며 **3.7 도 화이트리스트에서 제거**(3.5 선례 — 남기면 놓친 하드코딩이 조용히 옛 모델로 돈다). 소비처 정리 시 sweep env 기본값·spike 를 리터럴 대신 `DEFAULT_C_MODEL` 승계로 바꿔 raw 문자열 박제 0 유지. 계획에 없던 소비처 6곳(테스트 4 + 라이브 docstring 2)을 실행자가 추가 발견. pytest **4602/0**(+1 = 3.7 거부 회귀). ★LLM 영향: region C(scene_finder·기계 눈·view reasoner) + distill judge + curate_vision 게이트 + visual_gen judge 가 다음 호출부터 3.8 — 데이터 변경·재학습 없음, 다음 증류 사이클 judge 통과 분포 변동 가능. Pod 는 재기동해야 승계(`start_server.sh` 가 config 에서 읽음) | 2026-09-05 | cd765baf | [260905-mbd-gemini-flash](./quick/260905-mbd-gemini-flash-gemini-3-7-flash-gemini-3-8/) |
| 260903-upx | **멈춤 구간마다 확대 사진 1장 보장** (belle 09-03 "확대사진을 그 멈추는 구간은 다 보여줘야 하고, 그걸 모든 동작에서 통과시켜야 한다" / "pdshape 는 5개잖아 … 왜 또 4개야"). 사진이 사라지던 6곳(상한 4·ref 대응 실패 미방출·양측 좌표 0 skip·배율 drop·게이트 hold/pair/eye 삭제·display_anchor 미방출·최종 부착 대체)을 전부 "사진은 남기고 표시만" 으로: `decide_card` 순수 함수, `suppress_marks`, `userMarked`, 1단계 카드 보존 병합, 불변식 로그 `expected_units=N emitted=N`. 계약 3중 미러(userMarked/holdState/pairState/eyeState) + 시트 "왼쪽 사진에는 … 표시를 넣지 않았어요". pytest 4601/0 · 앱 219/0. 실행자 2회 API 장애로 Task 2 검토~4 는 직접. **라이브 6동작 검증 6/6 PASS**(1회차 pdshape 만 렌더 로그 TypeError 로 실패 → 01ef4a9d 수정 후 5/5). 증거 = evidence/live_results_2026-09-03.md | 2026-09-03 | 01ef4a9d | [260903-upx-photo-per-pause](./quick/260903-upx-photo-per-pause/) |
| 260903-lr6 | 통과한 확정 확대 사진은 동작별로 전부 인라인 (belle 09-03 "이래선 끝이 안나겠는데 … 통과한거 다 넣고" — 사진마다 ○× 절차 종료). 일반 경로 '다른 감점 항목' 접힌 행에 확정 사진을 펼침(DeductionCard 접힘 모드 zoom → ZoomCompositeImage, pill 대신 사진, 탭=시트 그대로). tsc 0 · node --test 219/0. 시뮬 파워스핀 60(확정 3): topFix 1 + 접힌 행 2 사진 인라인 | 2026-09-03 | 79885d83 | [260903-lr6-all-passed-photos-inline](./quick/260903-lr6-all-passed-photos-inline/) |
| 260903-lpl | 기계 눈 검사에서 몸통 관절(엉덩이·어깨) 제외 (belle 09-03 "비교 사진이 있어야 뭘 보지 저것만 보고 어케 알아" — 눈은 내 영상 크롭 한 장만 받아 절대 판정, 엉덩이는 기준 없이 정해지지 않음; 측정 3/5·힌트 1/5). `EYE_SKIP_KINDS`+`eye_applicable` + `_eye_check` 조기 반환 `skip:torso_joint`(호출 0·원장 0, hold·pair 게이트는 그대로) + verdict 로그 survivors 에 `eye=` 사유. pytest 4589/0(+3). 라이브 확인은 다음 Pod(pdshape 왼엉덩이 카드 `eye=skip:torso_joint` 생존) | 2026-09-03 | 7090a317 | [260903-lpl-eye-skip-torso-joints](./quick/260903-lpl-eye-skip-torso-joints/) |
| 260903-jxn | 기계 눈 힌트를 사지 중간 관절(무릎·팔꿈치·발목·손목)에 한정 + 불일치 시 3회 다수결(`eye_judge_majority`, 일치 시 추가 호출 0) + 원장 `rounds` 추가 필드. 근거 = jka 회귀 측정(엉덩이 힌트 1/5 < 무힌트 3/5). pytest 4586/0(+7). **운영 경로 다수결 회귀 하네스 9종×5회 = 7/7 PASS**(클라임 오클루전 2크롭 5/5·5/5, kneepath 0/5, 팔꿈치·어깨·extended 무릎 5/5), 엉덩이 2종은 보고만(pdshape 엉덩이 3/5 — 눈이 엉덩이 굽힘을 못 가름 → belle 항목: 눈 게이트에서 엉덩이·어깨 제외 여부). **라이브 재분석 PASS**(L4 Pod 2대 ~$0.4, 클라임 890e3b8b 92점): 서버 로그 `card_gates eye … claim=bent observed=bent limb=leg match=True rounds=1` · `verdict total=1 survivors=[r00] dropped=[] eye_calls=1` — 09-02 탈락하던 확정 카드가 게이트 통과. 눈 원장은 판정 6초 뒤 terminate 로 업로드 전 잘림(로그 발췌가 증거). ★첫 시도 4b523acf 는 faultZoomStatus=done 만 보고 끈 무효 실증(게이트 미실행) | 2026-09-03 | bf3f0e3f | [260903-jxn-eye-midlimb-majority](./quick/260903-jxn-eye-midlimb-majority/) |
| 260903-jka | 기계 눈 질문에 관절 종류 힌트 (belle "2번 가자" — 클라임 눈). **측정이 먼저**: 같은 크롭 5회 반복 → 09-01 vlu "PASS" 는 2/5·1/5 의 운, 회귀 kneepath 0/5. 관절 종류+교차 가림 힌트("이 원은 무릎 관절 표시… 팔 뒤로 허벅지와 정강이가 이어지면 그 다리가 대상") = 5/5·5/5·0/5. 수리 = `joint_kind_ko` + `_claim_question(…, joint_kind)` 힌트 변형 + eye_judge/machine_eye 통과 + app.py 호출 전달. pytest 4579/0(+7). **운영 질문 회귀 하네스 9종×5회 = 7 PASS / 2 FAIL**: 무릎·팔꿈치·어깨 전부 5/5(클라임 09-02 크롭 4/5), kneepath 0/5, **엉덩이 2종 불안정**(pdshape 엉덩이: 힌트 1/5, 힌트 없이도 3/5 — 눈이 엉덩이 굽힘 자체를 못 가름; fresh r03 전환 프레임 2/5). → 후속 260903-(다음) 에서 힌트를 무릎·팔꿈치·발목·손목 한정 + 불일치 시 3회 다수결 | 2026-09-03 | e307009d | [260903-jka-eye-joint-kind](./quick/260903-jka-eye-joint-kind/) |
| 260903-ik4 | **참고(advisory) 확대비교 카드 노출 — belle ○× 대기** (검토 표 3 행 3, belle "1번부터 정확하고 신중하게"). 백엔드 select_advisory_joints 가 만든 tier=advisory 카드(확정 아닌 관절 중 편차>tol 최대 2, 표시 전용)는 앱 어디에도 사진이 없었다(최근 6동작 8장). 수리 = `selectAdvisoryZooms` 순수 함수 + result.tsx "참고 부위" 섹션(다른 감점 항목 뒤·기준 모션 카드 앞, 저신뢰·일반 공통, 승인 문구 ADVISORY_CHIP_KO/ADVISORY_NOTE_KO 만, 중립색, 탭 없음). tsc 0 · node --test 222/0(219+3). 시뮬 실물: pdshape 참고 1장(양팔·7.2s), 클라임 참고 2장(왼엉덩이·왼어깨 1.8s — 확정 0장이라 유일한 사진). **belle × → 되돌림(de2e704c·d4eb8e18)**. 사유(belle 원문): "저게 양 옆 사진들이 같은 자세로 보기에 매치가 안되는데 내가 어떻게 판단을 하냐, 초가 다른거 같은데" — 참고 카드 기준 프레임이 DTW 대응 7.2s↔13.4s 로 같은 국면이 아님(저신뢰 정렬). 참고 카드는 기준 대응 프레임 검증(백엔드) 전엔 못 나감. 게이트 복귀 tsc 0 · 219/0 | 2026-09-03 | d4eb8e18 | [260903-ik4-advisory-belle](./quick/260903-ik4-advisory-belle/) |
| 260903-ftg | 역립 저신뢰(IN-01) 경로에도 확정 확대비교 사진 인라인 — 09-03 시뮬 실측: belle pdshape 60점 doc 은 `attributionReliability.unreliable=true` 라 07-24 IN-01 락대로 topFix 카드·다른 감점 항목이 **미렌더**, 진입은 "예상 부위 확대 비교 보기" 링크 1개(|points| 최대 record 시트만) → 확정 2장 중 왼엉덩이는 도달 불가(belle "왜 사진이 하나밖에 없어"의 실체). 수리 = `ZoomCompositeImage` 추출(DeductionCard·IN-01 공유) + `selectEstimatedZoomEntries` 순수 함수(primary 맨 앞·숨김 제외·dedupe) + IN-01 링크 → "예상 부위 (참고)" 사진 카드 **전부**(관절명·수치 없음, 탭=각자 시트). tsc 0 · node --test 219/0(215+4). **시뮬 실물 PASS**: 카드 2장(왼팔꿈치 5.3s·왼엉덩이 10.7s) 인라인, screens/01 | 2026-09-03 | 57c711ed | [260903-ftg-in-01-zoomcompositeimage](./quick/260903-ftg-in-01-zoomcompositeimage/) |
| 260903-f2w | 확대비교 앱 배선 완성 (belle 09-03 "확대비교 기능부터 완성하자 — 한것들까지 앱에 반영이 안되있는게 많아") — 진범은 모양 불일치: DeductionCard 가 사진 2장 분리 URI `zoomPair` 를 기다렸는데 백엔드 fault_zoom._compose 는 [내|정은지] 합성 PNG 1장(726x360)만 방출 → 히스토리 전체 무배선(표 3 행 4 유령 스피너). 수리 = 카드 `zoom {imageUrl}` 1장 모델(정확 종횡비)·pending placeholder 1개·접힌 행 "확대 사진" pill(표 3 행 1)·resolveZoomImageUrl 단일 출처(시트·카드 공유). tsc 0 · node --test 215/0(212+3). 시뮬 새 번들 인트로→홈→기록 크래시 0, **결과 화면 실물은 belle doc 복사 뒤**(`app/scripts/copy-analysis-to-sim.mjs`, 분류기가 막아 belle `!` 실행) | 2026-09-03 | ce6e6c20 | [260903-f2w-topfix-png](./quick/260903-f2w-topfix-png/) |
| 260901-wbo | 로딩 단축 — 코칭 문장(coach_dual 43s)+hook(~14s) 을 complete 사후 스테이지로 이동 (belle "코칭 문장 뒤로 빼는거 포함"). coachStatus 계약 3중 미러 + update_analysis_coach_text(stub-map 가드·in-memory 동기 갱신) + 앱 coachPending placeholder(180s 상한) + 진행률 3상수 재보정(229.6s 곡선 폐기). 기대: 로딩 화면 ~2분10초 → ~1분10초. record 문구는 결정론 생성이라 감점 내역은 즉시 표시. 검증: pytest 4572/0(기준선 4548+24)·tsc 0·212/0·구 doc 시뮬 무회귀 11장·checker 경고 1건 반영·verifier 6/6 PASS(human_needed 4건 = Pod/실기기 이월). ★라이브 pending→채움 E2E = 다음 Pod | 2026-09-01 | 59692c93 | [260901-wbo-coach-defer-loading](./quick/260901-wbo-coach-defer-loading/) |
| 260901-vlu | 기계 눈 오클루전 위양성 수리 (belle "확대비교 칸 자체가 없어졌다") — 진범은 앱/재구성이 아니라 compare_render 사후 카드 게이트의 machine_eye: 뻗은 팔이 굽힌 무릎을 가린 프레임에서 limb=arm 오판(conf 0.95) → 확정 카드 삭제 → advisory 만 남아 앱 진입점 소멸. 수리 = _claim_question 신설(기대 사지 명시 + 오클루전 탈출구, 좌우 이름 금지 유지) + eye_judge 추출. _eye_verdict·app.py 무접촉. 라이브 양방향 재판정 1회차 동시 성립(오클루전 belle 실물 match=True / ii0 kneepath 마크-전위 match=False 유지). pytest 4548/0(기준선 4537+11). ★라이브 E2E(재분석으로 doc 에 카드 생존)는 다음 Pod 때 | 2026-09-01 | 79339859 | [260901-vlu-machine-eye-occlusion-fp](./quick/260901-vlu-machine-eye-occlusion-fp/) |
| 260901-nms | 로그인 화면을 앱 첫 관문으로 (belle "아예 로그인화면에서 게스트 로그인을 달고 진행", "보통 로그인과 회원가입이 있을거아녀") — 인트로 '시작하기'가 즉시 익명로그인 대신 /auth/login 으로 push, 로그인 화면에 '게스트로 시작하기' 추가(소셜/회원가입/게스트 3구성). ★belle 관측의 진짜 원인은 버튼 부재가 아니라 **익명 세션 영속으로 인트로가 스킵되던 것** — 복원 판정을 one-shot 으로 바꿔 비익명(멤버)만 자동 홈 진입. 게스트 uid 불변(signInAnonymously 는 기존 익명 사용자 반환, signOut 0회) = 분석 기록 유지, AsyncStorage authUser 로 실측. 튜토리얼 경로는 dismissAll 후 replace(스택 [intro,tutorial] 로 인해 종료 시 인트로로 떨어지던 것 선제 차단). 소셜 로그인 경로 0줄 변경(어제 belle 실기기 확인 경로 보존). tsc 0 · 212/0 · 시뮬 증적 7장 | 2026-09-01 | 7d72dda6 | [260901-nms-login-gate-guest](./quick/260901-nms-login-gate-guest/) |
| 260831-review | (SUMMARY 없음) | 2026-08-31 | 74925e5f | [260831-review-followup](./quick/260831-review-followup/) |
| 260831-posture | (SUMMARY 없음) | 2026-08-31 | 1aac5b8f | [260831-posture-axis-trust](./quick/260831-posture-axis-trust/) |
| 260831-pod | Pod 검증 4런 (L4 20분 $0.17) — 코칭 수리 재측정 PASS(75초 3/3+salvage 실증) · mode3 전후 페어 E2E PASS(delta 배선 확인) · postureAxes 실코칭 PASS("상체가 기준보다 6도...") · GET /reference 타임아웃 관측(소비처 0) · E2E 드라이버 영구화 | 2026-08-31 | ac8db8ff | — |
| 260831-pair | (SUMMARY 없음) | 2026-08-31 | 31a0f1cd | [260831-pair-sweep](./quick/260831-pair-sweep/) |
| 260831-my | (SUMMARY 없음) | 2026-08-31 | 067a47ed | [260831-my-tab-login-entry](./quick/260831-my-tab-login-entry/) |
...
| 260831-kipup | (SUMMARY 없음) | 2026-08-31 | 5794a765 | [260831-kipup-diagnosis](./quick/260831-kipup-diagnosis/) |
...
| 260831-c3l | 미실행 꼬리 6건 삼진 분류 — 사문 2(01-24, 12.5-01) / 실질완료 3(01-25, 24-03, 25-04) / 파킹 1(20-04, SCORE-09 별도 PENDING 잔류 D-14/D-15). 스텁 6건에 직접 재확인 커맨드+결과 박제, CONTINUE·STATE 원장 정정(진행중 5→4 phase), 코드 변경 0 | 2026-08-31 | 5c5718fb | [260831-c3l-6-summary](./quick/260831-c3l-6-summary/) |
| 260831-bjj | belle 08-17 판독 축 구현 — 상체 꼿꼿함(`torso_uprightness_series`, 척추벡터 vs up 0°=꼿꼿, 델타 전용)·머리-척추 1자(`head_spine_alignment_series`, 골반-어깨-귀중점 collinearity 180°=1자) 순수 함수 + `posture_axis_summary`(nanmedian, significant≥5°) + mode1 코칭 배선(postureAxes → 양 writer 공유 `format_posture_axis_lines`, 학생 열위·significant 만 발화, 미발화 시 byte-불변). 피터팬 align.json 사전 박제 검증 PASS(ref 10.75° < user 20.59°, belle 원문 방향 일치). 4528/0 무회귀, 동작명 분기 0, 점수 경로 무접촉 | 2026-08-31 | c5b2fe6c | [260831-bjj-belle-08-17-1](./quick/260831-bjj-belle-08-17-1/) |
| 260831-apple | (SUMMARY 없음) | 2026-08-31 | (미커밋) | [260831-apple-signin](./quick/260831-apple-signin/) |
| 260828-v34 | v34 — 표적 데이터 첫 학습 (2026-08-28) · NOT PROMOTED | 2026-08-28 | 4e0ade41 | [260828-v34-targeted-data](./quick/260828-v34-targeted-data/) |
...
| 260826-v34 | LR 강도 실험 결과 — 침묵 첫 돌파 (2026-08-26) | 2026-08-26 | dc920980 | [260826-v34-lr-intensity](./quick/260826-v34-lr-intensity/) |
...
| 260824-jw4 | 잔상 데이터 렌더 구조 (일러스트 기능 완성 시도, 인라인 실행 — API 529 로 서브에이전트 4연속 사망) — keypointReport@atVideoSec 스켈레톤 렌더(ghostPose lib+테스트 12축) → belle 기각(실존하지 않는 자세) → rotate 앵커 재활성(그림 다리 회전, measureCriterion 자 지정, 승인 스타일) + 베이스 교체(belle 실촬영 재현본) → "잔상 이상" — **미수렴, 같은 날 260824-pqc 로 전량 제거됨**. 남긴 것: 잔상 records 조인 도메인 결함 수리 이력, 발견 = 앱은 이미 keypointReport 전체를 받고 있어 백엔드 변경 없이 데이터 렌더 가능(다른 기능에 재사용 가치) | 2026-08-24 | f8650d87 | [260824-jw4-backend-ghostpose](./quick/260824-jw4-backend-ghostpose/) |
| 260824-gt1 | 파워스핀 다리 ghost-noarrow 잔상 후보 (일러스트 확장 1호 시도) — 실측 유래 잔상 4장 → belle 기각, 이후 8라운드(앵커 교체·편집·결정론 합성·통짜 재현) 전건 기각. **원장 = PREDICTION.md** (라운드별 예측·판정·수리 이력 — 생성 모델은 각도·크기·위치를 프롬프트로 못 지킨다는 실측의 기록). 이 실패 누적이 같은 날 일러스트 기능 전면 제거 결정(→ 260824-pqc)의 근거가 됨 | 2026-08-24 | 3642f10e | [260824-gt1-ghost-noarrow-gemini-belle](./quick/260824-gt1-ghost-noarrow-gemini-belle/) |
...
| 260818-nnm | (SUMMARY 없음) | 2026-08-18 | 52d88c95 | [260818-nnm-illustration-how-first](./quick/260818-nnm-illustration-how-first/) |
| 260818-nc2 | (SUMMARY 없음) | 2026-08-18 | f3f41e3f | [260818-nc2-illustration-goal-card](./quick/260818-nc2-illustration-goal-card/) |
| 260818-muy | (SUMMARY 없음) | 2026-08-18 | 165c0559 | [260818-muy-a-b-2](./quick/260818-muy-a-b-2/) |
| 260818-lik | (SUMMARY 없음) | 2026-08-18 | 359b9de5 | [260818-lik-gemini-flash-3-5-3-7](./quick/260818-lik-gemini-flash-3-5-3-7/) |
...
| 260816-ill2 | 일러스트 크롭·방위 배선 수리 + 기계 9항목 게이트 통과선. 반려 10장 재생성 → 5 통과 (19장 전체 14/19). 게이트 자체 결함 2건(크롭맹·응답잘림)도 수리 | 2026-08-16 | e0dda6e | [260816-ill2-illustration-framing-wiring](./quick/260816-ill2-illustration-framing-wiring/) |
...
| 260814-rcz | quick-260814-rcz: 캡션 원인 절 + 각도 표기 적용 조건 Summary | 2026-08-14 | 8543433b | [260814-rcz-caption-cause-layer](./quick/260814-rcz-caption-cause-layer/) |
...
| 260811-ii0 | 260811-ii0 요약 — 성립 게이트 + 5동작 스윕 | 2026-08-11 | d5460411 | [260811-ii0-card-gates-5](./quick/260811-ii0-card-gates-5/) |
...
| 260810-e4v | fps 실효 rate 단일 출처 교정 — U1·U2·U3 코드 완료, 백필·U4 대기 | 2026-08-10 | 278d5212 | [260810-e4v-fps-rate](./quick/260810-e4v-fps-rate/) |
...
| 260809-zcm | 확대 비교 = 합성 비교 영상의 순간을 물려받는다 (짝 미세 보정 + 원본 해상도) | 2026-08-09 | 6aa1edf8 | [260809-zcm-zoom-compare-inherit-moment](./quick/260809-zcm-zoom-compare-inherit-moment/) |
| 260809-jnb | 합성영상 전체 조정 — 재생UI 복귀·조정컨트롤 제거·각도 짝 lockstep·문구 재작성 | 2026-08-09 | 962386f4 | [260809-jnb-ui-lockstep](./quick/260809-jnb-ui-lockstep/) |
| 260809-ill | 미보유 일러스트 재시도 (33-14 실패분) — sideway-spin PASS 후보 / pdshape 해부학 2회 FAIL | 2026-08-09 | e0dda6e8 | [260809-ill-missing-illustrations](./quick/260809-ill-missing-illustrations/) |
...
| 260808-r82 | quick-260808-r82: Phase 34 수술 3건 Summary | 2026-08-08 | b7e5ef09 | [260808-r82-phase-34-3](./quick/260808-r82-phase-34-3/) |
| 260808-jix | 260808-jix: Phase 35 앱 통합 — 합성 비교 영상 프로토 → 운영 승격 (3-way 계약 + 단일 mp4 재생) | 2026-08-08 | da316dba | [260808-jix-phase-35-3-way-mp4](./quick/260808-jix-phase-35-3-way-mp4/) |
...
| 260715-wq9 | Phase 22 v6 Plan wq9: 학습셋 조립 3축 교정 (교사 밀도 + 재균형 + descope) Summary | 2026-07-15 | d5460411 | [260715-wq9-phase22-v6-gemini-teacher-build-jsonl-de](./quick/260715-wq9-phase22-v6-gemini-teacher-build-jsonl-de/) |
...
| 260705-wbs | Phase quick-260705-wbs Plan 01: 사이각 게이트 (split 확정 + 학생 측만) Summary | 2026-07-05 | 32f40210 | [260705-wbs-split-arc-gate-student-only-split-record](./quick/260705-wbs-split-arc-gate-student-only-split-record/) |
| 260705-r6x | Quick 260705-r6x: 스플릿 확대 카드 다리 사이각 시각화 Summary | 2026-07-05 | 32f40210 | [260705-r6x-split-zoom-angle-arc-render](./quick/260705-r6x-split-zoom-angle-arc-render/) |
| 260705-r6v | 뷰어 v3 + 드릴다운 시트 + 참고 지표 진단 문장 | 2026-07-05 | 32f40210 | [260705-r6v-viewer-v3-legend-drilldown-diagnostics](./quick/260705-r6v-viewer-v3-legend-drilldown-diagnostics/) |
...
| 260629-cej | Phase quick-260629-cej Plan 01: Split-Angle Geometric Measurement Primitive Summary | 2026-06-29 | 448b11cb | [260629-cej-split-angle-geometric-measurement-primit](./quick/260629-cej-split-angle-geometric-measurement-primit/) |
