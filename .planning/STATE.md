...
Last activity: 2026-09-19 - **belle 지시 2건**: ①*"분석의 정도를 높여서 왠만하면 다 찾아내야지"* ②*"앱에 이미 있는 것과 부딪히면 뭐가 맞는지 판단하는 것도 분석이야"*(= 충돌 판정을 belle 에게 올리지 말 것). ||| ★★★★**표시 게이트 `_KP_CONF_MIN=0.5` 가 아무것도 안 가른다**(라이브 실측): 문턱 바로 아래 0.45-0.50 과 바로 위 0.50-0.60 의 좌표 안정성이 **같다**(중앙값 6.23 vs 5.06도, >20도 9.2% vs 6.3%). 신뢰도가 품질을 예측하는 것은 **0.6 위**부터. 그 근거 없는 선 하나가 **각도 카드 절반**(55/109)을 침묵시키고 있었다. → `conf>=0.5 OR (conf>=0.35 AND 시간축 안정)` **순수 additive**(PNG sha256 바이트 동일로 증명) 배선, 개통 **50%→70%**. 꼭짓점은 미완화(크롭 중심 = belle 승인 문법). ||| 완료 4건: mhl(짝 이탈 방출) · o8v(억제가 각도까지 지우던 것 수리) · oxg(안정성 축) · openapi 커밋+낡은 산출물 삭제. 게이트 **4896 passed / 0 failed** 직접 실측. ||| ★**내가 틀렸다가 실측이 고친 것 2건**: (a) "감점의 절반이 안 보인다"→**틀림**, 카드 부족은 전부 옛 doc 이고 현행 게이트 경로는 6→6 1:1 이다(대조군이 잡음) (b) "개통 81%"→**틀림**, 실제 39%(어깨 꼭짓점은 겨드랑이 내분점이라 관절 2개가 필요한데 내 3점 모형이 못 봄). ||| ★belle 4/4 수치(인계서 260919-0d3 §6)는 **네 해석 전부 재현 실패** — 통과선을 그 표 위에 세우지 말 것. ★Pod 0대·잔액 $11.88·Lambda 는 3세대 전 주소. **belle 눈 판정(완성된 사진)은 Pod 재기동 후로 남았다.**
