...
status: executing
stopped_at: "2026-09-20 마감 — 고친 것 2건(동시 분석 오염 · 캐시 키). ★그 뒤 §14(바닥 해부)에서 앞 판정이 뒤집혔다: (a) 라이브 학생 875건은 학생이 아니라 정은지 fixture 12편 안팎의 재분석 — 동작당 고유 영상 correct 1편 + fault 1편, 실제 사용자 업로드 32건; (b) §12·§13 의 점수 이동은 2026-09-17 rot180_v1 기준 승격과 그 이전 학생 각도를 섞은 버전 불일치였다; (c) 버전을 맞추면 정은지 본인 영상 11/11 = 100점이고 §13 이 '죽었다'고 한 pdshape·elbow-twist 가 결함을 가장 크게 가른다(+37·+34); (d) 동작별 바닥의 출처 = 같은 원본 프레임에서 포즈 추정이 재현되지 않는 것이고 크기는 역립 프레임 비율이 정한다(Spearman 0.891) — 체형·촬영각도·시간격자·DTW 는 실측으로 탈락. 남은 것 = 여유 2.0~2.3도 · 학생 영상 0편 · 바닥의 동정(GPU 필요). ★§15 = 운영이 저장한 점수 실독 — 정은지 본인 영상 358건 중 46건(12.8%)이 100점 미만, 최저 10점. 재앙급 4건은 leg_extension(ipsf_absolute)이고 belle 06-27 yaml 결정 이전 건이라 재발 0. 다만 2026-08-16 climb 60점은 이 축이었다(바닥 7.3도인데 60.7도) · 분석 doc 에 techniqueProfile 이 없어 왜 그 점수인지 사후 감사 불가. 착수점 = .planning/quick/260920-cac-concurrency-contamination-fix/260920-cac-SUMMARY.md (§14~§17 부터 읽을 것). ★§17 의 수리는 quick-260920-ra8 로 실행 완료(커밋 97a451f8) — 국면 힌트 폐기, 게이트 4965/0. ★§17 = Pod 실측(belle 승인) — 정은지 파워스핀 정타가 오늘 80점이고 범인은 leg_extension(reference_relative 0건), 원인은 Gemini 가 10.6초 클립 hold 를 1.0초라 답한 것(같은 영상 5벌에서 8.0초/1.0초 이봉, 2/5 오답). power-spin 에만 발생. 수리안 4가지 belle 판정 대기. §16 = gemini_cache 미결 닫힘(170건 전부 옛 id 라 도달 불가) — 대가는 캐시가 사실상 비었다는 것, 실증 당일 Gemini 비용·지연에 반영할 것"
...
| 260920-ra8 | **EXTEND 채점 hold 창을 국면 힌트에서 기하 창으로** (belle 승인 *"너무 과하게 오르지 않고, 분석이 그게 맞다면 인정해야지"*). Pod 실측 — 정은지 파워스핀 정타가 80점이었고 범인은 `leg_extension`(`reference_relative` 0건), 원인은 Gemini 가 10.60초 클립 hold 를 **1.0초(진입부)**라 답한 것. 같은 영상 5벌(캐시 우회)에서 hold 가 **8.0초/1.0초 이봉, 2/5 오답**. `_select_window` 가 힌트를 버리고 기하 창만 쓴다 — 소비처 1개라 점수·화면이 같은 창(TRUST-01). **새 상수 0개.** 저장 122건 재계산 = 정타 위양성 4건 제거 + 실수 8건 신규 검출(양방향). 부수: `_hold_window_from_moments`·`profile.hold_window` 소비처 0 됨(주석 명시), 사용자 문구 일상어화(신전→펴짐, hold 구간→버티는 동안). 게이트 4965/0. ★실패 16건은 건별 판정 — 단언 완화 0건 | 2026-09-20 | `97a451f8` | [260920-ra8-extend-hold](./quick/260920-ra8-extend-hold/) |
