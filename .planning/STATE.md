...
Last activity: 2026-08-31 - **앱 리뷰 6건 수리 (정보 손실 4건 복원)** — `/code-review high app/src` 결과, 6건 전부 오늘 결과화면 재구성(260831-lcc)에서 생긴 것. ①보완운동 라이브러리 도달 불가(추천 없는 doc 에서 진입점 소멸 — 게이트 OR 복원+진입점 섹션 레벨 이동) ②exercise_detour 격상 카피 침묵(게이트에 escalation 추가) ③점수 보정 안내 소실(면책 병합했는데 심사 카드는 감점 record 있을 때만 렌더 — clean-pass·저신뢰 doc 이 안내 상실. SCORE_CALIBRATION_NOTE 분리, 미렌더 시에만 옥타곤 1회) ④강사 질문 3개→1개 축소(중복제거 의도가 축소로 구현. genericCount 제거) ⑤중복 Firestore 구독(motions prop) ⑥참고 칩 경계 구분 상실(iOS dashed→solid, textDisabled 로 구분). ★교훈: 260831-lcc 의 "정보손실0 감사"가 grep 으로 **코드 경로 실재**만 봤고 **도달 가능성**(게이트)은 안 봤다 — 경로가 있어도 게이트가 닫히면 못 간다. typecheck 0·187/0·시뮬 눈검증 PASS. 커밋 3927b1fa
