...
Last activity: 2026-09-03 오전 - 확대비교 앱 배선 완성(260903-f2w) — topFix 카드 인라인 합성 PNG + 접힌 행 사진 유무 pill + resolveZoomImageUrl 단일 출처. 앱 커밋 3개(d4926299·ca41216f·ce6e6c20) 미빌드(1.2.4 대기, OTA 금지). 게이트 tsc 0 · node --test 215/0. 시뮬 결과 화면 실물 = belle 이 copy-analysis-to-sim.mjs 를 `!` 로 실행한 뒤. 표 3 잔여 = 행 2 클라임 eye(belle ○×)·행 3 advisory 노출(belle 판정)·행 5 사후 스테이지 순서(belle 승인). 백엔드 기준선 4572/0 무변경
