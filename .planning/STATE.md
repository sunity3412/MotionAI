...
Last activity: 2026-09-19 - 짝 시간 정렬 이탈 방출 배선 완료 (quick-260919-mhl, 게이트 4872 passed/0 failed 직접 실측, 앱 tsc clean). ★라이브 실측 4건: motionAlignment **100% trim_only**(warped 0/36 — warp 가 사실상 항등함수, distance 52~72 vs 임계 25) · `refMatch='failed'` **0/192** · `pairState='match'` **24/24**(자세거리 게이트도 반려 이력 0) · Lambda 가 **3세대 전 Pod 주소**(elevev58iv4mox)를 들고 있다. ★★인계서 260919-0d3 §6 의 belle 4/4 검증 수치는 **네 가지 해석 전부로 재현 실패**했다(크기뿐 아니라 순서도 불일치 — 왼골반이 반대 끝). 산출 경로 복원 불가 → 통과선을 그 표 위에 세우지 말 것, Pod 뜨는 날 belle 눈 대조가 먼저다. `contract.md §11.12` 는 **잠정(provisional)** 으로 등재. `refMatched=true` 거짓 보증은 **그대로 살아 있다**(§7 2번, 별도 단위).
