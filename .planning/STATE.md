...
Last activity: 2026-09-23 - Completed quick task 260923-sqt: 감점 재료가 기준의 엉뚱한 구간과 비교되던 결함 수리 (학생 영상이 기준의 1.2~1.5배 길이일 때 발생, 재현 84~89점 → 수리 후 100). 게이트 4992/0.
...
|---|-------------|------|--------|-----------|
| 260923-sqt | **감점 재료가 기준의 엉뚱한 구간과 비교되던 결함 수리 (위양성·위음성 둘 다).** `_build_deduction_measured_deviations` DTW fallback 이 window-local path 로 **전체** 기준 배열을 인덱싱했다 — 33-M3-SPEC 이 점수 경로·표시·segment 는 잘라 고쳤고 이 builder 만 빠짐. 터지는 조건 = 학생 영상이 기준의 1.2~1.5배 길이(학생 10fps vs 기준 15fps, 창 미끄럼 문턱 0.80). 재현(운영 함수+실제 기준 doc, 정은지 영상 앞 15% 절단): 점수 경로 0도인데 감점 seed 29~33도 → **kip-up 89 · power-spin 85 · climb 84** (수리 후 전부 100). 합성에선 진짜 25도 차이가 19.7도로 읽혀 감점이 사라지는 반대 방향도 확인. 운영 기록 11건은 전부 창 [0,nr) = 영향 밖, 과거 점수 불변. 회귀 테스트 3건(수정 전 3 failed). 게이트 **4992 passed / 0 failed** 직접 실측. 산식·허용오차 무접촉. | 2026-09-23 | (this) | [260923-sqt-seed-ref-window](./quick/260923-sqt-seed-ref-window/) |
