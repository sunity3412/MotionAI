...
Last activity: 2026-09-01 - **로그인 게이트 + 게스트 버튼 (quick 260901-nms)** — belle 관측 "게스트는 그냥 로그인되고 마이페이지 들어가야 로그인 버튼이 나오던데" 의 원인은 로그인 버튼 부재가 아니라 **익명 세션 영속으로 인트로(로그인 링크 보유)가 스킵**되던 것. 인트로 '시작하기'를 /auth/login push 로 바꾸고 로그인 화면에 '게스트로 시작하기' 추가 → 소셜/회원가입/게스트 3구성이 앱 첫 관문. 멤버 세션만 자동 홈 진입. 게스트 uid 불변 실측(기록 유지). tsc 0 · 212/0 · 시뮬 7장(재실행에도 게이트 노출 74번). ★미푸시 3커밋 — belle 스크린샷 확인 대기. 그날 아침 /start 로 Pod L4 1시간 기동 후 terminate 완료
