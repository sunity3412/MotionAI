...
Last activity: 2026-08-14(13) - ★**발굴 일반화 스윕 완료 (quick 260814-ehz) — belle "pdshape 에서만 한겨?" 의 답**. wif 왼무릎 발굴 규율을 승인 5동작 13 record 전수로 일반화(discover_sweep.py — 동작/record 파라미터화 + **양방향 claim 유도**로 관절 하드코딩 제거 + 소스 게이트 선행). 소스 게이트 5/5 PASS(전 동작 로컬 replay, Pod 불요·fps 교차검증 14.94~15.03 vs 라벨 15.0) → 13/13 record 스캔 → claim 대조 88버킷 → 압축 37 → **육안 전수 29통과/8탈락** → 기계 눈 실호출 58회(record 당 최대 9 ≤ 상한 16) → **눈 PASS 5 / 기각 24** → 카드 5장(2회 md5 전건 동일). ★**일반화 무결성 증명 = pdshapefault r03 cand13B 카드 md5 `e891e7ae` == belle 채택 카드 byte-동일** (좌표 무입력 + 다른 doc 마운트 + 일반화 유도에서 같은 결론. 단 같은 원본 영상이라 독립 표본 재발견 아님을 박제). **동작별 사전 박제(판정 전 커밋 904b714)**: pdshapefault=cand17B(왼팔꿈치 16.47/15.13, 카드 가독 최상·최대 감점 record) · powerspin=cand01E(왼어깨 0.47/0.73, **방향 반대** 서사 = belle 판정 항목) · elbow·kipup·peterpan=**발굴 0 — 추천 없음**. ★침묵도 산출물: elbow 12후보가 눈에서 전량 기각됐고 그중 5건은 **트랙이 5~7도 극단 굽힘을 주장한 순간을 눈이 '펴짐'으로 뒤집은 것** (keypoint 환각을 눈이 잡은 실물) · peterpan 짝 1/7 성립 후 눈 기각 · **split 3 record(kipup r00·powerspin r00/r01)는 단일 마크 좌표 부재로 눈 유도 원리적 불가** → 운영 helper 의 align-peak 면제와 층이 다름을 시트 §7 에 대조 명기. 장부 집계 규칙 신설 = "발굴 0 = 추천 없음"도 추천으로 세어 판정과 대조(침묵의 무판정 회피 차단, 행 2~6). 부수 실측 = peterpan 승인 freeze 6.444s 가 align 클립 6.067s 밖(u8i 상류 의제 재확인). Gemini 58회 ≈ $0.013·추론만·학습 전송 0·원장 리포 evidence 만 · backend diff 0 · S3 업로드 0 · Firestore 쓰기 0 · Pod 무접촉 · 채점 무접촉. 확인 재료 = /Users/Shared/sunity-discovery-sweep-260814/ (한글 카드·전신짝 9장 + 안내.md). 다음 = belle 판정 5항(시트 §9) → 채택분 반영은 di7 일반 경로 별건. // 이전: 2026-08-14 - ★**발굴 정지 캡션·음성 교체 재렌더 완료 (quick 260814-chd) — belle 판정("영상은 괜찮음, 캡션만") 반영**. 발굴 정지(왼무릎 u12.8667/r12.40)의 캡션을 결함 서사 정합 박제 신규 문장("다리를 곧게 편 채 회전…펴서 회전한 뒤에 걸어보세요")으로 교체 + 같은 문장 Polly 재합성(Seoyeon neural 운영 기본값 미러, 1회 후 repo 고정 — 자막=음성 lockstep). 정지 길이 9.8→11.18s(mp3+0.4 운영 규칙). 원본 r04 정지(10.5s)는 무변경 — 무수정 판정기 H3 PASS 1건이 기계 증명. 리그 = stock FAIL 정확 2건(발굴 H2+H3) 국한 박제 후 사본 delta 2축(H2 tuple + H3 discover expected, 전부 라벨 명기·사칭 0) ALL PASS + diff 국한 3층(삽입 335프레임 @42.07s 외 bit-동일) + ★**카드 3장 md5 == 0p2 전건(STOP 미발동 — 무릎 카드 = belle 채택 카드 그대로, 카드는 캡션 비종속 증명)**. 육안 스틸 2장 확인(새 캡션 구움/구 캡션 잔존). Gemini 0·Polly 1회(TTS 비-LLM)·S3/Firestore 쓰기 0. 확인 재료 = /Users/Shared/sunity-freeze-inject-260814/ 갱신(구/새 문구 원문 대조). 다음 = belle 확인 → 반영 사이클(S3 업로드 + doc 갱신 + SUPPORT-SURFACE §5 + 발굴 캡션 doc 영속화 규약). // 이전: 2026-08-14(11) - ★**발굴 채택 순간 영상 반영 실증 완료 (quick 260814-0p2) — 11사이클 마감**. belle 판정: cand13b(12.9/12.4s) 채택·cand02b 반려(판독 불가) — **사전 박제 일치 1/1 로 승격 실적 장부 개시**. 0p2: fresh pdshape 비교 영상에 발굴 정지 삽입 로컬 재렌더 — 베이스라인 재현(운영 doc 와 0.01s 전건 일치·md5 결정론) + ★리그 게이트가 삽입을 설계대로 검출(H2 정확 1건)함을 먼저 박제 후 `"discover"` 라벨 명기 면제로 ALL PASS(사칭 0) + diff 3층 국한(삽입 293프레임+페이드 5 외 bit-exact) + **무릎 카드 상속 방출 = wif 채택 카드와 md5 byte-동일**(e891e7ae) + 기존 카드 2장 무회귀. ★S3 업로드 보류 — belle 실물 확인 후 반영 사이클(업로드+doc 갱신+backend 주입 레이어/H2 discover 라벨, 필요 변경 목록 = SUPPORT-SURFACE.md §5). ★캡션 검토 항목: 재사용된 r04 문구가 belle 결함 서사(펴고 회전 후 걸기)와 정합하는지 반영 시 확인. 확인 재료 = /Users/Shared/sunity-freeze-inject-260814/ (재렌더 영상 포함). Pod 터미네이트됨(belle). // 이전: 2026-08-13(10) - ★**왼무릎 신규 발굴 사이클 완료 (quick 260813-wif) — 시스템이 스스로 옳은 순간을 잡는 승격 경로 첫 실전 성립**. 좌표 입력 없이 전 구간 스캔(124/272프레임 홀드 통과) + ii0 확정 임계 게이트만으로 **cand13b(u12.87/r12.40)가 kpo belle 육안 인증 짝(12.80/12.24)의 같은 홀드에 도달** — 재발견 성립. 신규 발굴 1건 = cand02b(u1.53/r2.33, 초반 오픈 요소 — 라운드 5 요소 매핑과 정합, V 예각 vs 일자 대조). 눈 기각 2건 정직 박제(팔 겹침 — kpo 동형, 통과 조작 0). **사전 박제 = cand13b 추천, belle 판정 전 커밋**(DISCOVERY-LEDGER 승격 실적 장부 1행). Gemini 10/16회·학습 전송 0·backend diff 0·채점 무접촉. belle 판정 대기 = 후보 2안 채택/반려. // 이전: 2026-08-13(9) - ★**카드 초 라벨 ÷9.0 잔존 수리 완료 (quick 260813-u8i, 검증 passed 7/7)** — kpo 부터 이월돼 온 마지막 표기 빚 청산. `label_fps` 측별 실효 fps 환산(fault_zoom 분모 교체 + app.py 게이트/스테이지 두 경로, 미지정 = byte-동일 하위호환) + contract.md §11.8·analysis.ts 서술 동시 정정. 기계 증명 = 대조 런(9.0 강제) md5 == nh4 정본 전건(변경원이 라벨뿐) · 승인 10카드 라운드트립 ≤1.5프레임/eff · pytest 59 동일 · 산식 5파일 diff 0. **Pod 실증 fresh `p34fresh1786628533`**: 점수 60·records 15자리 동일 · **left_elbow 라벨 5.9→5.3s(freeze 실초와 Δ≈0) · left_hip 18.6→16.7s** — 검증자 양쪽 카드 픽셀 육안 대조. ★한계 박제 = peterpan freeze 초(6.444s)가 클립 끝(6.223s) 밖(구 타임베이스 잔재 — freeze 타임베이스 상류 의제). 다음 = 왼무릎 신규 발굴. // 이전: 2026-08-13(8) - ★**왼무릎 A/B/C 판정 종결 = B 채택 → 반려 2건 회복 완결** (belle: "A와 B가 같은 시간대, 이때는 B. C는 좀 더 지난 장면"). B = 현행 짝과 같은 초 → **override·재렌더·S3 업로드 전부 불필요** — 반려 원인은 짝이 아니라 ref 무마크+전신 크롭이었고 B 스펙 렌더로 기해소. C 기각 = freeze 상속 예외 불필요(원칙 무손상). 장부 = xa1 JUDGMENT 라운드 5 최종 절. 남은 대기열 = 미세조정 의제(freeze 장면 선정·왼팔꿈치 가독성·legs 크롭·마크 길이·편차 필드 조사)·÷9.0·왼무릎 신규 발굴·승인 정지 2건·실기기 확인. // 이전: 2026-08-13(7) - ★**운영 배선 2차 완료 (quick 260813-nh4, 검증 passed 8/8) — B 스펙 운영 이식 + Pod 실증, 왼무릎 짝은 A/B/C 갈림으로 belle 확인 대기**. ①B 스펙(V 베이크 align 유도) 운영 이식 — verify_port 재현 게이트: 소생 6/6·카드 8→10·hold 9/9+pair 9/9·md5 전건 동일(검증자 재실행 = git 변경 0 바이트 재현). pytest 59 기준선·산식 5파일 diff 0·분기 0 ②**Pod 실증 완료 11:14Z** — commitSha 일치·fresh `p34fresh1786613939` 점수 60·records 15자리 동일·display_anchor/angle_bake 운영 로그·카드 육안 (**이후 Pod 의존 작업 없음 = 꺼도 됨**) ③판정 장부 박제(+50/-0): B 채택·ref V 보정 지시·왼무릎 3.867s 추천 불일치 기각 + ★교훈 = 짝은 기술 요소 정체성 우선(포즈 유사도는 닮은 다른 요소를 고른다) ④왼팔꿈치 ref V 진단 = **좌표 정확**(conf 0.563~0.697, crosshair on-joint) — 보정 없음, 가독성 의제만 미세조정 이월 ⑤왼무릎 content-match: 스크린샷 우측 = ref 4.067s 접힘 확정이나 **3갈래 갈림**(스크린샷 실물 짝 = user 2.87s 접힘인데 카드 user 는 freeze 3.3s 벌림 고정 / 벌림-벌림 짝 = 반려 baseline 2.4s) → 해석 금지, 후보 A/B/C 스테이징(S3 쓰기 0) — C 선택 시 freeze 상속 예외 승인 필요. 세션 수습 3회(네트워크 순단·절전·스톨 — caffeinate 대응). // 이전: 2026-08-13(6) - ★**미세조정 1차 재료 완성 (quick 260813-m0k) — V 베이크 align A/B 소생 6/6 + 반려 짝 회복 후보 + 커버리지 실측**. ①A/B: A 무패치 런 == ivs 정본(기반 확인) 후 B(align 유도 V 스펙, 하네스 monkeypatch — 운영 무접촉)가 회복 후보 **6/6 옳게 소생**(육안 전수, 환각 0) — ref 무마크 2장 해소 + 무로그 침묵 2건 신규 카드. elbow r01 은 align conf 0.229~0.429 실측 = 회복 불가가 옳음. survivors ivs==A==B 3원 동일 ②반려 짝: 왼무릎 추천 = **ref 3.867s**(국소 포즈 거리 0.0837 vs 반려 짝 0.2081, 같은 접힘 국면 — 전신 랭킹 1위가 반려 baseline 이라 전신 지표 한계도 박제) / 왼팔꿈치 추천 = **현행 9.4s 유지 + B 채택으로 해소**(ref V 성립). pair-override(r01 선례) 경로 — 재정박 아님 ③커버리지(belle "모든 확대 비교?" 답): 13 record 중 현행 8 → **B 채택 시 10**, 잔여 3 = 짝 게이트 기각 2 + 좌표 실부재 1(정직한 침묵 근거 실측). belle 판정 대기 = B 채택 + 왼무릎 3.9s 확정. // 이전: 2026-08-13(5) - ★**스윕 판정 박제 + 왼어깨 질문 답변 재료 회수 (quick 260813-l0u)** — belle 스윕 시트 판정 전건 장부 박제: 반려 2(짝 국면 불일치 — 짝 게이트가 못 거른 케이스로 의제 등재)·통과 6·소프트 노트 2(엘보 "앞에 보는 장면?"·파워스핀 "베스트 장면?" = freeze 장면 선정 의제)·질문 1. 사전 박제 대조 = 적중 2(왼무릎 짝 상이·왼팔꿈치 가림)·불일치 1(내 1순위 피디쉐입 오른팔꿈치 V = 무언급 통과). 왼어깨 답변 재료: 전신 프레임 짝(content-match 기계 선정+육안 대조 PASS) + v7 승인 영상 freeze 실물(각도선+121°/147° 수치+자막 구움 — 영상에서는 수치가 차이를 말해주고, 카드는 수치 미노출 기결론이라 미세해 보임) + claim 실측(26.79도 초과/허용 20/-8.2점 · ★카드측 deficitDeg 70.0 과 doc 26.79 불일치 발견 — 미해석 보고, 미세조정 라운드 조사 대상). 보드 재구성판(08-13, 9.9→2.8MB) 게시 완료. // 이전: 2026-08-13(4) - ★**승인 5동작 전체 반영 스윕 완료 (quick 260813-ivs) — 미세조정 라운드의 진짜 의제가 드러남**. 방출 8/침묵 5(전부 정직한 침묵, 사유 실측·freeze 위반 0). ★핵심 실측 3건: ①**새 V 문법이 실제 보이는 카드는 2장뿐**(피디쉐입 오른팔꿈치·피터팬 왼어깨 — 둘 다 08-11 실눈 기각 이력 record, 스텁이라 방출) ②**V 미베이크 5/8** — rep12 스펙 게이트(user_gate/ref_gate/relaxed/unmapped)가 V 를 죽임. 마크 길이 이전에 **베이크 게이트 빈도가 1순위 의제** ③**골반 P3 하이브리드 실물 0**(승인 코퍼스에 hip 카드 방출 없음 — 유일 실물 = fxx fresh 왼골반). 어색 케이스 사전 박제 6건(V 저사이각·폴 겹침 / 원이 얼굴 둘러쌈 / ref 무마크 2장 / 저해상 / legs 크롭 비대칭 / ÷9.0). display_anchor drop 1건 실측(elbow r01 오른어깨 — fail-closed 스펙 동작, ufb 구 문법에선 있던 카드가 죽는 코퍼스 발견). 무로그 침묵 경로 실측(rep12 양측 신뢰 0 → build 내 skip). belle 판정 대기 = 보드 스윕 시트 8장. // 이전: 2026-08-13(3) - ★**Pod 실증 완료 — 선 문법 배선이 운영 경로에서 성립 (quick 260813-hlv)**. 새 Pod mddy6gsqmt24ud(4090, 기존 볼륨) 재진입 6단계 전건 PASS(코드 0f999619 동기·bootstrap·기동·health 4항목·start_server md5 정본 일치·**SSM v31 + Lambda env 새 proxy URL 재동기 완료** — 재조회 실측, 4키 보존, AWS 쓰기 2건 한정). 실증 = fresh 재분석 `p34fresh1786593512`(342.7s): **`display_anchor rid=r00/r03` 성립 2건(drop 0) + `card_gates verdict` 운영 로그 실물**, survivors·앵커 좌표 fxx 로컬 인증값 byte-동일, **점수 60==60·감점 -45.0·records 소수점 15자리 전건 일치**(채점 무접촉 Pod 증명). 카드 2장 회수: md5 상이는 디코드 노이즈 Δ3/255 + 1px AA 뿐(구조 차 0), 육안 = 골반 P3 하이브리드 4요소·팔꿈치 관절 위 V PASS(오케스트레이터 재확인). Gemini 4건(눈 2회)·학습 전송 0. ★남은 것 = belle 카드 최종 육안(보드 게시) → **마크 미세조정 라운드**(belle: "길이·위치는 영상별 동작별로 다를테니 전체 반영한 다음에 조정") · ÷9.0 표기. // 이전: 2026-08-13(2) - ★**선 문법 운영 배선 완료 (quick 260813-fxx, 검증 passed 8/8)** — belle 라운드 3 최종 판정(P3r1 PASS · EV4/EV5 오프셋 반려 "따로 각도 표기를 빼라는게 아님" = 팔꿈치 기존 관절 위 V 유지 · 위치 미세조정은 전체 완료 후 이연) 박제 + 운영 이식: 확정 카드 표시 좌표(마크 앵커·크롭 중심) = **게이트 freeze 순간 align 단일 출처**(fps 라벨 사슬 skew 근본 수리 — hip vertex 이동 실측 user 1.65px/ref 8.67px = 라운드 2 진단 적중), 골반 마크 = P3 하이브리드 이식(`HYBRID_ANGLE_SUFFIXES={"hip"}` 선언 데이터, 팔꿈치 무변경), display_anchor fail-closed(한쪽 미달 = 카드 드랍 + 로그). 기계 증명 = WIRING-CHECK 2회(결정론·freeze 전건 일치·의도-변경-국한·align 예측 독립 재계산·승인 hold 9/9+pair 9/9) · pytest 59 failed 동일/4157 passed · 산식 5파일 diff 0 · 분기 0 · Gemini 실호출 0. ★남은 것 = belle 실물 카드 육안 + **Pod 실증(새 Pod 필요 — 터미네이트 상태)** + 마크 미세조정 라운드. // 이전: ★**xa1 라운드 3 완료 — belle 번복 박제 + P3r1/EV4·EV5 산출, 추천 EV5 → belle 판정 = EV 반려(불일치 장부 기록)** (quick 260813-ebd). belle 08-13: 주변 검증 결과 모두 선 선호 → **스포트라이트(E3-r1·P4) 채택 철회, 선 문법 채택** — 골반 = P3, 팔꿈치 = V자 유지하되 얼굴·머리 관통 회피 보정(E1보다 길어도 OK, 표현은 내 추천). belle 픽 이미지 실물 대조: 팔꿈치 픽 = 베이스라인(08-11 반려했던 기존 V자 그대로), 골반 픽 = P3. 라운드 3: 베이스라인 md5 게이트 PASS(반려 freeze 실물) + P3r1(앵커만 align 게이트 순간 단일 출처로 수리, 이동 user 1.6px/ref 8.7px = 라운드 2 진단 적중) + ★제자리 V 3안(EV1 관절도달/EV2 비례/EV3 연장) 전원 붕괴 실측(vertex 가 머리 원반 ~5px 밖 → 모든 방향선 즉시 클리핑, EV2==EV3 md5 동일 — V 문법 소멸) → 오프셋 V(EV4 기본/EV5 x1.6) 성립(관통 0 기계 검증 + 육안), **추천 = EV5**(사전 박제). 통과 시 = 선 문법 운영 배선(표시 좌표 단일 출처 근본 수리 + 오프셋/제자리 전환 규칙 결정 + 승인 5동작 무회귀 + 새 Pod 실증). // 이전: 2026-08-12(심야2) - ★**xa1 라운드 2 완료 — E3-r1·P4 belle 판정 대기, 착수점 = CONTINUE-2026-08-12.md**. belle 라운드 1 판정 = E3 스포트라이트 채택(원 보정 조건)·P 선 문법 전부 반려 → 라운드 2: E3-r1 원 보정 성공(★뿌리 = rep fps 라벨 사슬 1프레임 조기 좌표, 수리 = 게이트 freeze 순간 align 단일 출처 — 오프셋·분기 0) + P4 골반 스포트라이트 + 선 진단(앵커 정확 9px, 병 = 허공 스텁 + 저사이각 V). 통과 시 다음 = 스포트라이트 운영 배선(표시 좌표 단일 출처 근본 수리 포함) + 승인 5동작 무회귀 + 새 Pod 실증. Pod cv8poc707mqtxh 터미네이트(belle). // 이전: ★**마크 문법 후보 6안 산출** (quick 260811-xa1). ufb 카드 belle 판정 = 왼골반 장면 PASS(freeze-only 구조 첫 육안 인증)·마크 반려, 왼팔꿈치 반려(각도선 얼굴 관통). 같은 freeze 에서 P1~P3(골반)·E1~E3(팔꿈치) 산출, 추천 = P2+E3 (JUDGMENT.md). ★기록 교정 박제: "영상에서 잘 잡은 걸 확대"는 설계 철학이 아니라 **내 실패로 강제된 바닥** — 목표는 시스템이 스스로 옳은 순간을 잡는 것 (memory freeze-inherit-is-fallback-not-goal). // 이전: ★**freeze-only 수리 완료** (quick 260811-ufb, kpo 반려 수리판). belle 반려("영상이라는 기본 승인 틀 / 하다못해 확대를 해도 되겠구만") → **재정박 코드 제거**, 카드 = 영상 freeze 그대로 + 확대만, 게이트 = 방출 판정 전용. 기계 증명 = fresh 2회 + 승인 5동작 전부 freeze 일치·결정론 2회·승인 9/9·pytest 59·분기 0. **Pod 실증 p34fresh1786458292 = freeze 상속 카드 2장(왼팔꿈치 5.3s·왼골반 16.7s) + 점수 60 + 404s + 눈 2회**. ★belle 판정 대기 3건 = ①카드 2장 육안(특히 왼골반 freeze 부활 건) ②왼무릎 신규 발굴 사이클 진행 여부 ③눈이 기각한 승인 정지 2건(r01 환각 적중·r00 unclear) 처분. ÷9.0 표기·크롭 알고리즘·표시 문법은 별건 유지. Pod cv8poc707mqtxh 유지(서버 5ddc1e3a). // 이전: ★**성립 게이트 운영 배선 + Pod 실증 완료** (quick 260811-kpo, 커밋 5개 push, HEAD 82d7eed0). ii0 게이트 3종이 운영 카드 경로에 들어갔다: 카드 = compare_render 영상 정지 **상속** + 게이트 생존자 \|dev\| 순 배정, 실패 전량 graceful(기존 카드 폴백 — 엉망 경로 없음), 채점 무접촉. **Pod 재분석 p34fresh1786433865 실증 = 왼골반 카드 소멸 + 왼무릎 카드 방출(홀드 짝 12.8/12.3s 육안 인증) + 왼팔꿈치 생존·귀속(pole_proximity)** — `card_gates verdict` 운영 로그 실물 + 점수 60 유지 + 승인 무회귀 9/9 + pytest 기준선 59 동일. ★**belle 확인 요망 1건 = 기계 눈 호출 상한 완화**(플랜 2회/카드 → 16회/record 지연 평가 — 광역 keypoint 전위 아래서 안 늘리면 왼무릎이 원리적으로 죽는 실측, 분석당 40~46회 ≈ $0.01). 유보 = left_elbow 귀속 로컬 근소 미달 0.1498 vs 0.15(창 기반 측정 다음 후보, Pod 은 성립)·카드 초 표기 ÷9.0 잔존·**방출 카드 belle 육안 최종 판정 다음 사이클**. LLM 학습: 추론만·학습 전송 0, 눈 원장 86건(마킹 크롭+판정) Phase22 씨앗 보존. Pod cv8poc707mqtxh 유지(스톱 안 함), 서버 = 2112975a(백엔드 최신과 동일). // 이전: ★**표시 문법 → 감점 성립의 병으로 승격** (quick 260811-bz5, 정본 = `260811-UNIFY-FINDINGS.md`). belle 실시간 판정 6라운드: 왼골반 카드 = 다른 장면 짝 + 전환 측정 + **진짜 결함(왼무릎)은 카드 상한에 밀려 탈락**. ★홀드 장면에서 **양쪽 트랙 keypoint 환각**(정은지 8.1s 육안 접힘 vs 트랙 178도) — conf 게이트 무력, 기하 검출 불가(음성 결과). **재현성 정정**: 같은 영상 20회 재분석 전부 동일 60점 — 병은 결정론적 오답. ★**기계 눈 게이트 성립**(Gemini가 환각 프레임 conf 1.0 적중 — 관절 마킹 크롭 판정 설계). 폴 근접도 축 실측(카드 순간 3/5 비대칭). ★record 5건 전부 홀드 아닌 순간 측정(각속도 141~1129도/초). 수리 스펙 v1 = 감점의 성립 조건(홀드·신뢰·짝 정합·기계 눈·dev 랭킹) — 채점 변경 belle 승인+점수표 게이트 대기. 철회 장부·frames-before-numbers 실물 게이트 신설. // 이전: **표시 문법 프로토타입 완료** (quick 260811-bz5). 로컬 재렌더 하네스(Pod 무관 — Firestore keypointReport + S3 원본 영상 + 운영 `build_fault_zoom_comparisons` 호출)로 기준선 confirmed 4/4 픽셀 재현 PASS. 후보 3종 = A 고스트(점선+쐐기) / B 쐐기(+화살촉) / C 하이브리드(쐐기 상시, 고스트 델타≥8도만 — A 는 델타 4~7도에서 본선 겹침 오독). ★핵심 실측 = **감점 근거 vs 순간 델타 괴리**: left_hip 20/10도 · right_elbow 5.9/6도 → 문법으로 풀림, left_elbow 29.1/4도 · right_shoulder 10/1도 → **국면 문제(문법 밖, pd_rest 축과 같은 뿌리)**. 판정 페이지 = https://claude.ai/code/artifact/27e89578-b701-47c3-881f-38e28d58287a · 실물 /Users/Shared/sunity-cards-260811-grammar/. ★저장 카드 인덱스는 rep 공간(18fps), 렌더 override 는 frames 공간(9fps) — ÷2 가 다리. Pod = 내려감(proxy 404), 문법 채택 후 운영 이식 검증 때 필요. // 이전: ★**착수점 = `.planning/CONTINUE-2026-08-10.md`** (이 파일만 읽으면 이어짐). **확대 비교 사진 수리 완료** (quick 260810-ms2, 커밋 8개 push, HEAD d0633227). belle 결정 = "영상은 그대로 둬도 괜찮을것 같음. 사진에서 더 자세히 볼 수 있도록". Pod 실검증 p34fresh1786363530: **붕괴 카드 3/5 → 0/5**, right_elbow 사이각 0/86도 → **174/163도**, 표시 방향차 81 → **20도**. ①★**U6(da89ea01)이 실행조차 안 되고 있었다** — 운영 카드 경로는 `select_pose_matched_pair` 로 프레임을 고르는데 배제는 `select_confident_index`(그 뒤 `if not pair_selected:`)에 들어가 있었다. 로그 `fault_zoom_pose_pair` 5/5 가 즉시 증인. 학생·기준 **양쪽** 배제 + fail-open 축별 4단. ②후보창 승급을 **실패 조건에 묶음** — 항상 ±4 로 넓혔더니 멀쩡하던 left_hip 이 17→72도 악화, 되돌리니 right_elbow 재붕괴, 승급 조건을 선택자 요구(`_member_pts` ∧ 붕괴아님)와 일치시켜 둘 다 해결. **시소 3회 = 매번 한쪽만 보고 통과시킨 탓.** ③크롭 폭 고정 0.61 → **부위 크기 기반**(목표 0.50 · 밴드 0.40~0.55, belle 이 40/50 고민 후 확정) — 부위/패널 22~58% → **34~65%**. ④크롭 소스 **원본 2160x3840**(종전 640px 축소본 = 원본의 1/6) + `probe_effective_fps` 신설(운영 Pod 은 프레임 캐시라 추출 이력이 없어 그대로면 **운영 경로에서만** 조용히 비활성됐을 것). ⑤공용 폭 px → **비율**(원본 해상도 학생 2160/기준 1080 → 같은 594px 가 27% vs 55% = 한쪽만 확대돼 보였다. 640px 시절엔 짧은 변이 우연히 같아 잠복, 테스트도 그 잘못된 계약을 박고 있었다). 채점 무접촉(records 5건 값 불변)·영상 무접촉. 게이트 **59 failed IDENTICAL** / 4141 passed · 채점 산식 5파일 diff 0. ★**다음 = 표시 문법** — belle "저길 잡으면 뭘 말하는지 모를것 같은데". 마크가 패널 대비 **고정 크기**(선 64/85px, 호 16px)라 확대해도 상대 개선만 있고, right_elbow 는 각도 174도라 호가 안 보여 **선 하나**로 읽힌다. 별건 = advisory 링이 얼굴에 찍힘(★변경 전에도 동일, 두 산출물 대조 확인)·기준 패널 마크 부재·`_OUT`=360 유지(키우려면 절대 px 상수 배율 필요). 미완 = SSM `runpod-analyze-url` stale(다음 `sam deploy` 가 옛 Pod 로 되돌림)·밴드 표본이 영상 1개/카드 5장·`fault_zoom` 카드 초 표기 아직 ÷9.0·U3 점수효과 Σ<40 doc 필요. // 이전: ★**착수점 = `.planning/CONTINUE-2026-08-10.md`** (이 파일만 읽으면 이어짐). **fps 실효 rate 교정 U1~U6 + 백필 + Pod 실검증 완료** (quick 260810-cbt 측정 / 260810-e4v 수리, 커밋 10개 push). 뿌리 = `frame_extractor` 정수 step 솎음(실제 9.997fps)인데 초 환산은 요청값 9.0 → 사진 앵커가 9.7~10% 틀렸다(프레임 수 4/4 재현). 기준 트랙 **11건 전수** 실제 14.88~15.00fps인데 라벨 18.0 → 백필 완료(read 경로 11/11 확인). **U2 Pod 라이브 검증**(3 record 가 예측 0.9003 배와 소수 3~4자리 일치, 2회 재실행 소수 15자리 동일=결정론). U3 는 `min(40,Σ)` **캡 포화**로 점수 미검증(Σ<40 doc 필요) · 허용오차 뒤집힘 0/40건. **U4 = 4/3 보정 제거 철회**(대수적으로 소거되는 값이었음 — 내 진단 2회 정정, 불변식으로 못 박음). ★**belle 실물 판정 → 카드 결함 3종 분리**(①붕괴 ②국면 ③방위). conf 게이트는 붕괴를 원리적으로 못 잡음(0.57 통과, 12관절 중 10개 한 줄, −11.6점 카드) → **U5 `pose_collapse` + U6 표시 프레임 붕괴 배제** 투입. ★**확대 비교 사진이 아직 영상에서 물려받지 않는다** — 운영 render 호출에 `zoom_dir` 없음(app.py:4148, belle 질문이 짚음). 다음 = ①`zoom_dir` 배선 ②Pod 재분석 실물 확인 ③창 확대 폭 belle 판정. 미완 = SSM `runpod-analyze-url` stale(다음 `sam deploy` 가 옛 Pod 로 되돌림)·`fault_zoom` 카드 초 표기 아직 ÷9.0·noise-floor 경계 미측정. // 이전: **fps 실효 rate 교정 U1~U3 코드 완료** (quick 260810-e4v, `b0dbe3c5`·`cfb3a447`·`c0b7488b`). belle 결정 = 전역 교정 + 점수표 게이트, "점수 변동은 정당하면 괜찮다". **U1** `effective_fps(src,target)` + `extract()` 가 **경로별** 실효 rate 기록(한 분석에서 사용자 target 9→step 3, 기준 target 18→step 2 로 rate 가 달라 단일 속성이면 덮어씀 — 실측 확인). **U2** `_pipeline_frame_fps(video_path)`·`_moment_video_sec()` → `atVideoSec` 가 실효 rate 로(**점수 불변** — tally 종료 후 각인되는 표시 전용 값). **U3** `_reference_angles_fps()` = `anglesRealFps` 우선·없으면 라벨 fail-open(백필 전까지 byte-동일) + 백필 스크립트. ★**기준 11건 전수** 실제 14.88~15.00fps인데 라벨 전부 18.0(아침엔 5건 표본) → 마진 9→8 프레임. ★**fail-closed 게이트가 내 유도식을 잡았다**: 교차검증을 `프레임/길이`로 했더니 강제 마지막 프레임 1장 때문에 전 건이 크고 짧은 클립(peter-pan)에서 1.56% 불일치 → `(n-1)/길이`로 교정, 기록값은 정본 `src_fps/step` → 11/11. 게이트: 신규 31 pass · 전체 **59 failed IDENTICAL** / 4096 passed · 채점 산식 5파일(motiondtw 명시 포함) diff 0. ★**미완 = 백필 미실행**(dry-run 만, 이 실행이 점수 이동 스위치 — 허용오차 20도 경계 record 의 감점 뒤집힘 미측정)·**U4 미착수**(4/3 보정 제거 + 승인본 대조 / 빠진 카드 링 폴백)·실 파이프라인은 Pod 재분석이 확정 수단. // 이전: 2026-08-10 - **표시 순간이 "안 보이는 순간"인 게 아니라 애초에 감점을 잰 순간이 아니었다** (quick 260810-cbt, `da3f549f`, 측정만 · `backend/` diff 0). 뿌리 = **fps 라벨 오차** — `frame_extractor` 가 정수 step 으로 솎는데(`step=round(src_fps/target_fps)`) 초 환산은 요청값 `target_fps(9.0)` 를 쓴다 → 30fps 원본에서 실제 **9.997fps**, 저장 초가 **9.7~10.0% 크다**. `select_pairs` 가 그 초를 **재탐색 없이 클램프**해 사용자 프레임으로 쓰므로 사진의 앵커가 틀리고, 기준만 ±2초 재탐색해서 **틀린 프레임에 기준을 맞춘 짝**이 된다. 저장 트랙 프레임 수가 이 규칙으로 **4/4 정확 재현**(180/182/62/83). ★**peterpan r00 = 저장 초 6.444s → 프레임 97 > 클립 91프레임**(belle 승인 5편 중 하나). ★`fault_zoom:852` 경험적 **4/3 보정이 바로 이 오차**(`18/14.93×9.96/9=1.335`) — 증상만 덮여 있었다. 기준 트랙 5건 전부 실제 15.0x fps인데 라벨 18.0. 유입 = 최초 ML 어댑터 커밋(2026-05-21) = 전 기간. **교정은 일관 개선이 아니다** — 렌더러 무접촉 A/B 에서 차이↑ 3 / 표시 상실 3 / 축소 2, 카드 4→2장. powerspin r02 의 현행 85.9도는 정렬 오차 과장(교정 16.8도/정렬 0.800→0.181). 할 것 1 = 두 축이 서로 잡아먹으므로 **나머지몸 정렬(pd_rest)로 직교화**(우세 후보 7/10), 단 **앵커 교정이 선행**이라 규칙 교체는 그 뒤. 할 것 2 = **`편차÷요동` 지표 기각**(요동 130~180도 = 회전·역립이면 항상 <0.1, "안정 국면인가"를 재고 있었다). ★**belle 결정 3건** = ①교정 범위(표시 전용=점수 불변 / 전역=점수 이동) ②카드 4→2 수용 ③4/3 보정 제거. 미완 = 2D 전단축 게이트 미설계·3D 카드 표시 미검토·구포맷 3픽스처 미측정·앱 미배선. // 이전: 2026-08-09 - **확대 비교 = 합성 비교 영상의 순간을 물려받는다** (quick 260809-zcm, `835b0417`). ★**착수 전 `.planning/MORNING-BRIEF-2026-08-10.md` 를 읽을 것** — 어제 belle 시간이 많이 든 이유가 이미 정해진 것을 다시 조사했기 때문이다. **전제가 뒤집혔다**: 각도가 안 그려지는 건 게이트가 보수적이라서가 아니라 **좌표가 틀려서**다(게이트를 열면 폴 위에 세로선이 그어진다 — 실물 확인). 더 큰 것 = **카드가 고른 프레임 자체가 붕괴 프레임**(pdshape 오른팔꿈치 프레임 118: 12관절이 폴 위 한 줄, 몸통 = 클립 중앙값의 **8%**, 가로/세로 **0.03**, 그런데 신뢰도 **0.67** 로 게이트 통과, 시스템 품질 라벨 **"medium"**). 원인 = 표시 프레임 선택이 관절 confidence 최대인데 **붕괴 프레임에서 모델이 더 확신** → 창 후보 9개 중 가장 나쁜 것이 뽑힘. 전수 = 카드 30장 중 4장. **belle 결정 = "영상에서 잘 잡은 걸 확대"(나 방식)** — 순간·짝·표시는 영상에서 물려받고, 기준 시각만 ±0.4s 미세 보정(belle "몇 프레임 차이를 맞춰주면서 해야 해"), 그림은 **원본 해상도**에서 새로 크롭(사용자 영상 실측 2160×3840인데 렌더는 608×1080). 결과 = 겨드랑이 카드에 **121°/147°** 양 패널, 무릎 링이 허공→무릎 위. **기본 off**(zoom_dir 미지정 시 영상 산출 무변경). ★**belle 판정 대기 3건** = 확대 정도·기준 패널 링·전 동작 전수. 미완 = 앱 미배선·파이프라인 미연결·pdshape 외 미실행. ★**다음 첫 작업 = 조건부 표시 순간 측정**((차이 큼)∧(포즈 신뢰)∧(정렬 신뢰) 프레임이 있는가 → 있으면 표시 순간만 교체, **채점 무접촉·점수 불변**). 폐기된 안 = "최대 차이 순간"(150~177도가 전부 정렬·포즈 실패), 피처 비교표 1차(몸통 기울기·좌우 비대칭이 관절 고유 피처가 아니라 58% 쓸어감 + r01 자기모순). ★**지표 자체를 belle 반려/통과 카드로 검증할 것** — 내 지표는 20건 중 17건인데 belle 체감은 "한두 개", 안 갈리면 지표를 버린다
