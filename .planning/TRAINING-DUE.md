...
| 4 | **인물·세션 분리 평가셋이 있나** | 🔶 **틀 있음(09-25)** — 평가 항목 = `sealed_tests.jsonl`(봉인 시험지) + 짝 + 같은 인물·세션 클립. 점검 = `backend/scripts/eval_split_check.py`(L1 같은 파일·L2 같은 세션 = 오류, L3 같은 인물 = 경고). **현황: 누수** — 정은지 fixtures 실수 6편이 manifest 학습 후보(L1 6), reference 11편이 같은 인물(L3). 인물이 1명뿐이라 **인물 분리는 아직 불가**. ★**belle 09-26 정책 확정**: "시험 영상(구 봉인 시험지)은 배우지 않는다, 나머지는 전부 배운다" — 정은지 실수 fixtures 6편은 이미 분석된 **연습 영상**이므로 `sealed_tests.jsonl` 에서 빼고 학습에 넣는다(L1 누수 해소). 시험 영상 = 처음 채점하는 영상만. `--mark-holdout` 은 손 결정이 아니라 이 정책의 구현(코드 반영은 다음 사이클 전) | `eval_split_check.py` exit 0 |
