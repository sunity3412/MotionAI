...
- ① 무릎: **"진짜 많이 나아졌는데 이정도면 상관은 없긴한데"** — 포즈매칭 방향 수용(3연속 반려 탈출).
  NEW 이슈 = **설명 결합**: 코치 글은 "무릎이 어쩌고" 할 텐데 두 패널이 별반 차이 없어 보임 —
  그 차이가 회전 때문인지 / 성공영상 고개 자세가 좋아서인지 사용자가 분간 불가.
  → 크롭+감점근거 텍스트 세트("정은지 이렇게/학생 이렇게→감점", §6.5 계열)를 **사용자 화면**
  관점에서 재설계 필요 (원래 내부검수용이던 것의 승격 여부 = belle 설계 결정).
- ② 어깨: "수평 반전 준것마냥 다르네 비슷한 자세인데. 화면만 같으면 어깨가 어떻다는 걸 말해줄
  수 있을 것 같은데" — **미러(좌우반전) 잔존이 남은 핵심.** 조사 갈래: (a) 매칭이 미러 자세
  프레임을 골랐나 (b) 정은지 원본이 반대 방향 회전이라 같은-방향 프레임 자체가 없나 —
  (b)면 기술이 아니라 표시/설명 문제로 전환.
- ③ 손: 사진도 안 맞고 "손을 뭐 어떻게 하라는지" 사진만으론 불명 — advisory 행동지침 설계
  항목(후속 ⑦) 재확인. ①의 글-결합 필요성과 같은 뿌리.
- ④ 기준(정은지) 패널 초 표기: belle "빼는 게 좋을 것 같은데 너도 고민해줘" + Claude 동의
  → **제거 확정 (다음 세션 반영).** 근거: 앱 동작비교의 싱크 미세조정(앞/뒤) 값을 서버 렌더가
  알 수 없어, 조정한 사용자에게 원시 초가 박히면 오히려 혼란. 학생 패널만 유지.
- ⑤ belle: "하나 수정하는데 오래 걸려서, 내일 마지막으로 한번 더 미세조정과 보완 해보고 진행.
  오늘은 여기서 마무리" → 세션 paused. 서버는 79221f0 포함 8a7a356 재기동 완료(잠들기 전 반영
  — 구버전 타임베이스 버그 서빙 방지). SCORE 60 불변.

## 다음 세션 안건 (belle "마지막 미세조정+보완" 1라운드)
1. **기준 패널 초 표기 제거** (④ 확정분, display 전용).
2. **어깨 미러/facing 조사** (②) — 매칭 선택 프레임의 실제 방향 실측 + 정은지 원본 회전 방향
   확인. pose_distance 는 좌우배치를 보존하는 설계인데 미러 결과가 나온 경위 규명.
3. **①③ 설계 논의 (belle 결정 필요, 무단 구현 금지)**: 크롭+감점근거 글 세트의 사용자 화면
   승격 / advisory 손 카드 행동지침. 크롭 단독으론 "왜 감점인지" 전달 한계가 belle 실측으로
   확인됨 — 사진+글 결합이 유력.
...
## belle 설계 확정 (2026-07-28 — framingexp260728 시트 육안 후, 메인 대화)
- **A. 프레이밍 = D 확정** (세로 패널 캡처 + 결함 부위 마커). belle verbatim: "D로 하는 걸로
  하고 마커가 없는 것보단 있는 게 나을 것 같고". C(전신 원본)는 인물 작아 기각, B는 마커
  부재로 기각.
- **화면 구조 = Ochy 패턴 확정**: 기본 화면 = D 캡처 1장(결함 부위마다 마커, 카드 반복
  제거) → **부위 탭 = 상세**(현행 A급 확대 크롭 + 감점근거 글 + A-7 일러스트 슬롯).
  belle: "부위가 두 군데면 그쪽 클릭하는 것마다 확대(현재 A)와 함께 그 러닝앱처럼".
  구현 = 앱+계약 변경 수반 별도 사이클, 시뮬 확인 후 OTA([[verify-ui-on-simulator-before-ota]]).
- **B. facing fix 조건부 승인**: 어깨/손 카드는 "회전이나 모습부터 잘 맞추고서" — 어깨/골반
  x-순서 facing 신호를 Pod 정량 A/B 통과 시에만 적용. 통과 후 렌더로 어깨/손 카드 재판정.
- 마커 원칙 재확인: 마커는 채점이 지적한 부위(confirmed/advisory)에만 — D-18 짝 없는 표시
  금지. belle의 "반대편 무릎 높이/굽힘도 신경 쓰임"은 표시가 아니라 **귀속/채점 확인 항목**
  → 이 fixture 반대편 무릎에 advisory 존재 여부 실측 후 회신 (TODO).
- 블로킹: Pod `ovblalej2102sb` down (proxy 404 + SSH 거부, 프로덕션 경로 다운) — belle
  콘솔 Start 대기. 기동 후: pull 27635ce → 서버 재기동 → driven 렌더 → facing A/B.
