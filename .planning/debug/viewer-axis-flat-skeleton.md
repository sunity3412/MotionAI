...
- OTA 5회 발행: `65a907a3` → `c7acf831` (수정 1차) → `e5a4f177` (축 수정, `89402fc`) → `138ec235` (프레임 시점+로딩 100%, `b0f1d52`) → `c153e0ec` (자세 비교 실프레임 전환, `683f6ce`). Pod `xps7co0m2njzpi` (4090) 가동 중, `/health` 200, repo `4f13092`.
...
next_action: "원 버그(가로 막대)는 실기기 종결(2026-07-21 00:20 실사진 카드 렌더 확인). 잔여 실기기 확인 = 로딩 100% 표시 1건뿐 — 다음 분석 때 자연 확인되면 /gsd-debug continue viewer-axis-flat-skeleton 으로 archive_session. 익일 순서(belle 총평 32-SEED §10): ①동작 비교 정렬 포기 문제 보완(belle 최우선 — 낮은 신뢰에서도 시간 오프셋 정렬 유지) ②참고 지표 텍스트 겹침 수리(실기기 버그) ③확대비교 크롭 배율(백엔드, 새 분석부터) ④/gsd-discuss-phase 32 (자세비교 카드 존치/방향매칭 프레임 선택 포함, /gsd-code-review 31 인계 4건 병행)"
...
- timestamp: 2026-07-21 — **belle 제품 결정: "사람이 나와야지"** — 스켈레톤 겹치기 자체가 기대 미달(버그 아님, 31-08 설계가 그랬음 — 사람 나오는 시각물 2종(교정 이미지·회전 영상)이 OFF 되며 남긴 결정론 대체물). 실데이터 목업 선제시 규칙([[ux-propose-user-centric-screens-first]])을 건너뛴 것이 갭이 배포 후 드러난 원인.
- timestamp: 2026-07-21 — 자세 비교 실프레임 전환 구현: 신규 `PoseCompareFrames`(비교 순간으로 seek 한 정지 VideoView 쌍 + KeypointOverlay 정적 관절점), ReferenceCornerSection 분기(URL 양쪽 있으면 실프레임, 없으면 스켈레톤 폴백), result.tsx 결선(URL=동작 비교와 같은 재발급 사다리, 시각=kr 인덱스/kr.fps). 커밋 `683f6ce`.
- timestamp: 2026-07-21 — **동반 잠복 크래시 발견·수정: KeypointOverlay Wave-1(정적, player 미전달) 모드가 useEvent(null) 내부 addListener 에서 Render Error** — 문서상 지원 모드였지만 정적 소비처가 처음 생기며 드러남. no-op 이벤트 소스로 교정. **시뮬레이터 게이트가 OTA 전에 차단** (강화된 검증 기준의 첫 실전 효과). 동작 비교(player 전달) 경로 무회귀 시뮬 확인.
- timestamp: 2026-07-21 — 확대비교(감점 행 탭 시트) 실측: 사진 URL 전수 정상(HTTP 206, 7일 서명)·시트 열림·다리 신전 행도 사진 표시(left_hip 매칭). **본질 문제 = 크롭 배율 불일치**(내 영상 관절 클로즈업 vs 정은지 와이드샷 — [[faultzoom-reference-crop-2x-wider-diagnosed]] 그대로) → 백엔드 Pod 크롭 생성 수정 필요, 새 분석부터 적용. 익일 작업으로 이월.
- timestamp: 2026-07-21 — OTA 발행: update group `c153e0ec-c0db-471a-861b-87264c60b6f3` (커밋 683f6ce). 직전 정상 group = 138ec235.
- timestamp: 2026-07-21 00:20 — **belle 실기기 결과**: c153e0ec 적용 확인 — 자세 비교 카드에 실사진 2장+관절점 렌더(크래시 없음, 뷰어 경로 전체 작동 = 원 버그 '가로 막대'는 실기기에서 종결). **그러나 제품 가치 부정**: 회전 동작에서 시간 대응 프레임끼리는 몸 방향이 달라 눈 비교 불성립("자세가 서로 동일하지도 않은데 뭐 어쩌라는거니") + 확대 시트와 실질 중복. ★프로세스 위반 동반: belle 노트의 질문("확대와 뭐가 다르냐")에 답만 하고 명시 승인 없이 구현·OTA — [[feedback-answer-before-next-question]] 재위반으로 박제.
- timestamp: 2026-07-21 00:30 — 오판 정정: **참고 지표 카드 텍스트 겹침은 실기기(TestFlight) 재현 = 프로덕션 버그.** 세션 중 "Expo Go 폰트 폴백 아티팩트"로 판단했던 것은 틀림(belle 스크린샷으로 반증). lineHeight 계열 추정, 익일 수리 목록 등재.
- timestamp: 2026-07-21 00:30 — belle 마감 선언 + 총평 브레인덤프 → `.planning/phases/32-SEED.md §10` 박제 (즉시 수리 3건: 동작비교 정렬 포기 문제[최우선]·참고지표 겹침·확대비교 크롭 / 32 설계 질문 8건 / MVP 방향: omni 최대 활용·SMPL 재점검). 방향 매칭 프레임 선택(관절 좌표로 정은지 유사 방향 프레임 선별)은 "있으면 좋고" — 32에서 카드 존치 여부와 함께 결정.
