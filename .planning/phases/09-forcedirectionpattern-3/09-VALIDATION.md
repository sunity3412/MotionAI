...
| 09-W1-C2 | Wave 1 | 1 | FORCE-01 SC#3,#4 / D-09-D3 | T-09-R1 (단정 표현 회귀) | AST 금지 표현 grep gate 10/10 = 8 substring (research §10.2 6종 + 신규 2종 `힘이 정확히` / `프로보다 못합`) + 2 regex (신규 `근육 힘 방향.*확정` + `\d+%\s*감점`). regex `.*` 가 조사 변형 (`근육 힘 방향이 확정` / `근육 힘 방향은 확정`) 까지 catch — SC#4 직접 정합 | unit (AST grep gate) | `pytest tests/phase09/test_force_pattern_copy_no_forbidden.py -x` | ❌ Wave 1 | ⬜ pending |
...
| 09-W1-P1 | Wave 1 | 1 | D-09-D6 | — | pipeline `_process` mode_context 산출 (mode1 / mode3_first / mode3_progress) + `infer_force_direction_pattern` 호출 | integration | `pytest tests/pipeline/test_pipeline_phase9.py -x` | ❌ Wave 1 | ⬜ pending |
| 09-W1-P2 | Wave 1 | 1 | D-09-U1 (lockstep) | — | `complete_analysis(force_pattern_inference=...)` 시그니처 + `forcePatternInference` Firestore key | integration | `pytest tests/pipeline/test_pipeline_phase9.py::test_complete_analysis_invocation -x` | ❌ Wave 1 | ⬜ pending |
