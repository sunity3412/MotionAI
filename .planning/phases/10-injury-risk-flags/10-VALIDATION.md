...
| D-05 real-elite | MULTIPLE pinned-source real-elite clips (ref-sideway-spin / ref-invert / ref-foxtop-split; mirrored + spin + both limbs) → zero hyperextension flags (real 3D keypoints, NOT reference-angles.json) | T-10-FP | unit | `pytest backend/tests/phase10/test_safety_flags_hyperextension.py -k "elite" -x` | W2 | ⬜ pending |
...
- [ ] D-05 real-elite source provenance: real `(T,17,4)` 3D keypoints from pinned motion IDs (ref-sideway-spin / ref-invert / ref-foxtop-split) via the RTMW extraction path or a checked-in compact 3D fixture WITH provenance — `reference-angles.json` (angle-only, 8 joints) MUST NOT be used to claim 3D cross-product coverage.
