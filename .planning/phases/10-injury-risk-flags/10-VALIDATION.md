...
| D-05 helper-contract (joint gate) | **Single-variable joint isolator:** `wrong_joint_same_phase_report` — instability in the SAME/overlapping phase but on a DIFFERENT joint (e.g. left_wrist) → no flag (test asserts the phase-overlap precondition first, so a pass can only come from the joint-locality gate) | T-10-11 | unit | `pytest backend/tests/phase10/test_safety_flags_hyperextension.py::test_d05_wrong_joint_no_flag -x` | W0 GREEN | ⬜ pending |
