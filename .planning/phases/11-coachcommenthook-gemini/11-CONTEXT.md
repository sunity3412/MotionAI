...
- **D-04:** "Gemini 번역만" 원칙 = Gemini 는 엔진이 이미 계산한 finding 을 한국어로 풀어 설명만 하고, **새 수치·좌표·점수·판정을 생성하지 않는다.** 수치(점수·실측 각도)는 앱에 계속 나오되 그 출처는 측정 엔진 — LLM 이 자기 숫자/판정을 만들면 위양성·신뢰붕괴 + 강사 대체 인상(FEED-03 위반). [[analysis-objectivity-no-human-scores]] · "수치는 보조, 원인이 핵심" 정합.
  ...
  - **[2026-06-17 iter-3/iter-4 correction]** number-free = **hook text 의 모든 Arabic digit(`\d`) reject** 로 확정 (degree/% 만이 아님). hook 전용 가드는 `"23도"`/`"23°"`/`"15%"` 뿐 아니라 bare/measurement `"3초"`/`"2회"`/`"15cm"`/`"180"` 도 전부 reject 한다 (LLM 이 새 숫자 cue 를 못 만들게 — D-04 본문 "새 수치 생성 안 함"의 정확한 구현). 가드는 **hook 전용**(`forbid_measurement_units`/`_enforce_no_hook_number_patterns`) — 전역 `_enforce_no_reject_patterns` 는 무수정(scene_finder/reference_extractor 의 "30도"/"50%" 정상 출력 보호).
- **D-05:** 강제 방법 = **프롬프트 제약 + 단위테스트(금지패턴 0 검증)**. 시스템 프롬프트에 "수치·좌표·점수·판정 출력 금지, finding 자연어 풀이만" + 골든 finding 입력 → 출력에 좌표/점수/판정/도(degree) 패턴 0 assert (13-C `forbidden_in_copy` 패턴 재사용). 런타임 sanitize 는 채택 안 함(자연어 손상 위험) — **reject-and-fallback** (금지패턴 매치 시 canned hook 대체). 강제 메커니즘 세부는 Claude 재량(belle: "강제는 네가 코드로").
  - **[2026-06-17 iter-3/iter-4 correction]** 골든 finding 단위테스트는 degree/% 가 아니라 **all-digit guard** 를 검증한다 — "3초"/"2회"/"15cm"/"180" 포함 fixture. 또한 fallback 책임 분리: writer(`build_coach_hooks`)는 `CoachHookBundle | None` 만 반환(api fail/4xx/guard reject/retry exhausted → None), per-report canned 채움은 **pure `coach_hook_builder.resolve_coach_hook_bundle(bundle, *, force_findings, body_findings)`** 소유(analysis layer — `gemini.schemas` import 금지, duck-type 변환). "분석 status done" 보장은 pipeline wiring(Wave 1 Task 2)/통합 레벨이지 Wave 0 pure helper 테스트 책임 아님.
