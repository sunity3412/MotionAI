...
2. `/gsd-plan-phase 12` — wave 분할 + plan 박제. 예상 wave 구조:
   - **Wave 0A** (12-00): data contract foundation — RTMW adapter keypoints_2d 실 채움 (R1) + kismam.assess() 3 call site wiring fix (R4 CRITICAL) + axisData polyline 정의 (R2) + targetSource enum (R4) + Keypoint2D.visibility confidence source 명시 (R6 — Keypoint2D 실 필드 = x/y/visibility, raw_visibility 는 Landmark2D 별개). **Codex 리뷰 2026-06-10 4 blocker 해소**.
