...
## 13-C 추가 디스커션 (2026-06-16)

belle 가 13-A/13-B 완료 후 "섹션형 듀얼 coach"를 추가. 4개 영역 논의:

1. **섹션 레이아웃** — 옵션: 세로 스택 / 아코디언 / 탭 → belle 선택 **세로 스택**(원인+처방 동시 비교 핵심).
2. **출처 라벨** — 옵션: 기능 라벨만 / AI 출처 표기 → belle 선택 **기능 라벨만**(벤더명 숨김).
3. **부분 실패 처리** — belle 가 "어떻게든 실패 안 하게 + 하나가 대신" 의도 + "고객별 결과 편차" 우려 제기. 트레이드오프(숨김 vs 대신채움) 분석 후 → **계층형**(재시도1회 → 다른 LLM 대신채움 → 둘다실패시 단일/수치 폴백) 선택. 실패확률 낮음(파일럿 rate-limit≈0, 소프트실패는 _normalize_entry 흡수) 확인. coach 성공/폴백률 로깅 추가 합의.
4. **보완운동(13-A) 연결** — 옵션: 별도 유지 / 통합 → belle 선택 **별도 유지**(큐레이션 라이브러리 vs 동적 LLM, 성격 다름).

추가 결정: "실증 시 둘 중 하나 drop 여부" = Phase 15 검증 기준. 파일럿 기본값 단일 Gemini 유지, 13-C 가 섹션용 양쪽 활성.
