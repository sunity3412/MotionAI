...
### D-14: Phase 20-04 흡수 (belle 2026-06-23 결정)
- **Phase 20-04(SEVERITY_CAP sweep 게이트)는 23-03 eval 에 흡수** — 독립 sweep 중복 회피. 23은 cap(50/75/90)을 그대로 쓰되 veto 입력을 still-frame 으로 바꾸므로, 20-04 를 옛 whole-video 로 따로 캘리브레이션하면 23이 다시 흔든다. **23-03 가 still-frame veto 에서 20-04 게이트를 소유·검증한다: 정은지 정타 95~100 / fault cap(kip-up≤50 등) / 결정론(같은 입력=같은 점수) / EVAL18 변별 4쌍 퇴행 0.** 23-03 의 case matrix·machine-checkable 게이트에 이 항목 포함. ROADMAP Phase 20-04 = superseded-by-23-03 표기. (belle: 21은 23 뒤로 / 22·10 은 파일럿 후.)
