...
### D-17: ITERATION8 — terminal gate tamper-proof 마무리 (2026-06-23, [[23-DIRECT-REVIEW-ITERATION8.md]], belle "마지막 점검")
- **(HIGH-1) ancestry 를 result run commit 기준으로** — `lock_git_commit` 이 현 HEAD 의 ancestor 인 것만으론 "결과가 lock 이후 생성"을 증명 못 함(eval 먼저 돌리고→freeze→result JSON 에 lock commit 끼워넣기 가능, 수동 워크플로 사고 포함). **fix:** eval harness 가 시작 시 lock 상태를 강제·기록한다 — `eval_stillframe_veto.py` 는 lock 존재 + 현 manifest sha == lock.manifest_sha256 + lock_git_commit 이 현 run HEAD 의 ancestor + worktree clean(출력 경로 제외) 아니면 **실행 거부**. result JSON 에 `run_git_commit`(eval 시작 시 HEAD) / `worktree_dirty_at_run` / `manifest_lock_git_commit` / `manifest_sha256` / `eval_command` / `result_generated_at` 기록. assert 검사: `lock_git_commit` 이 **`result.run_git_commit`** 의 ancestor(현 HEAD 아님) + `result.run_git_commit` 이 현 HEAD 의 ancestor + `worktree_dirty_at_run==false` + lock commit/sha 일치.
- **(HIGH-2) multi-arm result shape 고정** — `result row set == manifest row set` 가 arm 다중(production_still/direct_adapter_still/whole_video_baseline)과 충돌(case×arm 이면 extra rows 로 valid 결과 fail, 또는 arm 을 nested 로 숨기면 baseline/캐시격리 누락 silent). **fix:** result shape = `cases[]{row_id, arms:{production_still, whole_video_baseline, direct_adapter_still}}`. manifest row = `{row_id, required_arms:[production_still, whole_video_baseline], optional_arms:[direct_adapter_still]}`. assert: manifest row_id set == result cases[].row_id set / 각 case 의 required_arms 정확히 1회 존재 / unknown arm 0 / production recall·특이도 게이트는 `arms.production_still` 만 읽음 / baseline 비교는 `arms.whole_video_baseline` / 캐시격리는 같은 case 내 arm 간 키 비교.
- **(MED-1) EVAL18 pair mapping 을 manifest 소유** — `regression_pairs:[{pair_id, fault_row_id, success_row_id, min_margin}]` top-level. lock 이 pair policy snapshot, assert 는 이 mapping 에서만 margin 계산(row_id 네이밍/motion id 추론 금지 — hardcoded known-answer 재유입 방지).
- **(MED-2) gate_policy 를 manifest 소유** — top-level `gate_policy:{clean_coverage_min, clean_gate_case_classes:[elite_clean,imperfect_clean,occluded,spinning], alignment_abstention_case_classes:[tempo_shifted], known_fault_case_classes:[known_fault], required_arms:[production_still,whole_video_baseline]}`. lock 이 gate_policy 포함, assert 가 script 상수 아닌 locked policy 에서 coverage 재계산(미래 변경이 frozen manifest 안 거치고 게이트 강도 못 바꾸게).
- **(MED-3) FaultKey 단일 serializer/validator** — `FaultKey.to_dict()`/`from_dict()` 에 locked enum(part_scope/side/keypoint_set/fault_kind). manifest `expected_recall_keys` 는 `FaultKey.from_dict` 로 검증, result `recall_set` 은 `FaultKey.to_dict` 산출, assert 는 정규화 FaultKey tuple 비교(free-form dict 아님). 테스트: manifest 의 unknown fault_kind 가 Pod 결과 수락 전에 fail. enum 어휘는 `VisionFaultContext.to_trace_dict()` 가 방출하는 것과 동일해야(23-01 FaultKey 와 단일 owner).
