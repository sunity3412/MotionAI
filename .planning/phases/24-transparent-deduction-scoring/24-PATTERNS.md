...
(1) ACTIVATE criteria from TWO sources (HIGH-1 — the Gemini-silent defense): `seeded = ipsf_criteria.criteria_from_measured_deviations(measured_deviations)` (measurable criteria whose substrate is finite AND beyond tolerance, INDEPENDENT of Gemini) unioned with `pointed = union(ipsf_criteria.criteria_for_fault(fault_key, supported_difference, measured_deviations) over the supported_differences)` (the PUBLIC router — split vs leg vs reach vs line routed on body_part/fault_state + substrate, NOT a keypoint_set map). `activated = seeded | pointed`, then apply the HIGH-5 line/leg no-double-count exclusion and group joints per ACTIVATED criterion BEFORE the loop (one deviation per criterion, not per joint — cures −60 runaway). When Gemini is silent (no supported_differences), `seeded` still activates measured-geometry criteria, so a fault video still deducts (preserves Phase 20 — `test_gemini_silent_not_100`).
...
**Analog:** self (`VisionVerdict`, lines 125-141). `assess_fault_severity` already emits `score`-free `VisionVerdict(primary_fault, severity, differences)` — ND-02's demotion is half-built. Phase 24 reframes the three docstrings (L5/L129/L134) that call `severity` an `apply_downward_cap` input → the verdict/differences carry the criterion pointer (HIGH-5): the supported_difference / fault label locates which measured criterion is implicated (via criteria_for_fault on body_part/fault_state); severity is not read for selection or scoring and remains only a non-scoring label for coachRootCauseEligible continuity (MEDIUM-1). ALSO (iter4 MEDIUM-4) the `dominant_severity` schema `description` band prose (`none → cap 미적용`, L157-160) is reframed to a score-free string (`none; 점수 아님; scoring input 아님`) — a data string, not logic. The objectivity guards STAY: `_SCORE_PATTERN` leak guard (line 113), `score` field permanently absent (line 15), `response_schema` score/overall field-count 0 (line 148). No schema shape/enum/code change. No `apply_downward_cap` or `cap`/`캡` band string may remain in the docstrings OR the schema description.
...
- **Gemini severity in the score arithmetic** or read for criterion selection: the supported_difference / fault label LOCATES a criterion via `criteria_for_fault` (body_part/fault_state); severity is never a number, never read by the router, used only for coachRootCauseEligible continuity (ND-02, HIGH-1).
