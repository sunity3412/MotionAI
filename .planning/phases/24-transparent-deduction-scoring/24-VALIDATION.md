...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 24-01-T1 | 24-01 | 1 | SCORE-16, SCORE-12, SCORE-14 | T-24-01 | contract lockstep (+deviationSource/fallback); 4 criteria incl. `line`/clean_lines; criterion_for_fault_key TOTAL+PARTITIONING over all 8 FaultKey keypoint_sets (4 mapped + 5 gap, no overlap/omission); TRACKED deferred coverage gaps; no final ceiling; no per-criterion baseline_kind | unit | `cd backend && python -m pytest tests/test_deduction_engine.py -k "contract or criteria or lockstep or coverage" -x -q` + `cd app && npx tsc --noEmit` | ❌ W0 | ⬜ pending |
| 24-01-T2 | 24-01 | 1 | SCORE-10, SCORE-11, SCORE-12, SCORE-13, SCORE-14, SCORE-15 | T-24-01, T-24-02, T-24-03 | measured-substrate base (not 100−Gemini); unavailable→dimension_overall (never 100); gemini-silent still deducts; NaN-safe; severity not in arithmetic; coverage-gap not a band | unit | `cd backend && python -m pytest tests/test_deduction_engine.py -x -q` | ❌ W0 | ⬜ pending |
...
| 24-02-T2 | 24-02 | 2 | SCORE-10, SCORE-13, SCORE-14, SCORE-15, SCORE-16 | T-24-04, T-24-05, T-24-06 | tally fed measured overallScore+deviations (no 100−Gemini); baseline_kind derived by name/motion_id string-match + threaded as a string into apply paths (no profile→no NameError); cap_would_apply preserved (severity!=none) so eligible_for_coach/coach-injection still fire; result['deductionBreakdown'] set+validated in complete_analysis (no kwarg); nested-array validator; no silent no-op (WARNING+audit) | unit/integration | `cd backend && python -m pytest tests/test_pipeline_deduction_seam.py -x -q` | ❌ W0 | ⬜ pending |
...
| 24-02-T4 | 24-02 | 2 | SCORE-10, SCORE-16, SCORE-09 | T-24-05, T-24-06 | math-determinism; baseline_kind name-match test; gemini-silent<100 (Phase 20 preserved); coach-gate (eligible_for_coach) preserved; Mode3/toggle preserved; two broken caller test files (test_vision_veto/test_pipeline_vision_gate) migrated band-free | integration | `cd backend && python -m pytest tests/test_pipeline_deduction_seam.py -x -q && python -m pytest tests/ -q` | ❌ W0 | ⬜ pending |
