...
| 24-02-T1 | 24-02 | 2 | SCORE-10 | — | band code+comments removed; gemini_vision_scorer docstrings + schema description reframed — verdict/differences carry the criterion pointer, severity is a non-scoring label (HIGH-5a/MEDIUM-4, no schema-shape/code change); to_audit_dict reshaped band-free (tallyFinal, no capApplied); helpers preserved | unit | `cd backend && python -c "from sunity_shared.analysis import vision_veto; assert not hasattr(vision_veto,'apply_downward_cap') and not hasattr(vision_veto,'SEVERITY_CAP')"` | ❌ W0 | ⬜ pending |
