...
fix:
  ...
  fixed: 6
  skipped: 5
  detail: CR-01/CR-02/WR-01/WR-02/WR-03/WR-04 fixed — IN-01~05 skipped (각 finding Status 참조)
---
...
**Status:** fixed (57ca9fd) — `_upload_and_wait_active` broad except → graceful None + 폴링-실패 orphan delete. 회귀 테스트 2건(upload/poll transport 오류) 추가.
...
**Status:** fixed (ab713b7) — 업로드 성공 즉시 개별 append (부분-업로드도 finally delete). 회귀 테스트(두 번째 업로드 실패 → 첫 핸들 delete) 추가.
...
**Status:** fixed (b925156) — 명시적 executor 관리: break/예외 이탈 시 `shutdown(wait=False, cancel_futures=True)`. 집계/fail-closed 의미론 무변경(기존 테스트 green). 회귀 테스트(budget break 시 blocked in-flight 비대기 반환) 추가.
...
**Status:** fixed (26d1e27) — `unlink_on_error` kwarg 신설(기본 True=기존 계약), prefetch 활성 시 False 로 즉시-unlink 봉인 + `_process` 조기실패 except 가 `shutdown(wait=True)` **후** unlink. 불변 주석 실동작 정합. 테스트 stub 4곳 시그니처 정합.
...
**Status:** fixed (38cf52c) — except 에서 `ref_tmp.name` 정리 (+ ref_tmp None-guard).
...
**Status:** fixed (1e013a9) — `d.get("error")` 로 교체, 기존 isinstance(dict) 분기가 `.code` 추출.
...
**Status:** skipped — 현 `_process` 배선(항상 shutdown(wait=True) 후 close)에서 미도달 경로. closed 플래그+finally 분기는 >5줄 동시성 변경이라 trivial 기준 초과. 후속 phase 후보로 기록.
...
**Status:** skipped — try/finally 재구조화 2개 함수 ×~15줄(재들여쓰기)로 trivial 기준(≤5줄) 초과. 비-ASCII 파일명 업로드에서만 발동하는 기존 결함(27 diff 밖). 후속 정리 후보.
...
**Status:** skipped — zero-risk 아님. 해당 default 는 파일 주석(53-57행, 2026-06-15)이 "video multimodal 완전 지원 3.x 미출시 → 이 경로 한정 2.5-pro 가 올바른 중간 모델"로 명시한 **의도된 vision-only 예외**. 3.1-pro-preview 의 video 지원 검증 + belle 결정 없이 모델 string 교체 시 moment 추출 전면 실패 리스크. 운영 레버는 env `GEMINI_MOMENT_MODEL`(신규 Pod 부트스트랩 체크리스트에 반영 권장).
...
**Status:** skipped — pair 생성~assess 사이 ~35줄을 try/finally 로 감싸는 재구조화(들여쓰기 변경 포함)로 trivial 기준 초과. 좁은 창 + 분석 흐름 영향 0. 후속 정리 후보.
...
**Status:** skipped — 리뷰 자체가 (선택) 표기한 알려진 한계. 로컬 타이머 병용은 신규 앱 로직(상태/이펙트 추가)이라 trivial 아님. 180s 보수값이 대부분 흡수.
