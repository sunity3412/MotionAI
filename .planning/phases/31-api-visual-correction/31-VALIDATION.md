...
| 31-02/T3 | 31-02 | 1 | 원자 예약·CAS·전용 timestamp (H-06/B-03) | T-31-05/06/07 | 이중 예약 0, updatedAt 미기록, URL 파라미터 부재 | unit | `python -m pytest backend/tests/phase31/test_visual_jobs.py -x -q` | W0 | ⬜ pending |
...
| 31-05/T3 | 31-05 | 2 | H-03 before/after judge | T-31-18 | 불확실=None, display/training 분리 | unit | `python -m pytest backend/tests/phase31 -x -q` | W0 | ⬜ pending |
...
| 31-09/T1 | 31-09 | 3 | B-03 state machine + continuation | T-31-32/33 | 고아 불가 불변식, batchItemFailures | unit | `python -m pytest backend/tests/phase31/test_visual_worker.py -x -q` | W0 | ⬜ pending |
| 31-09/T2 | 31-09 | 3 | correctedPose 잡 (이중 게이트+정리+페어) | T-31-34/35/36 | key equality, finally 삭제, training 별도 임계 | unit | `python -m pytest backend/tests/phase31/test_visual_worker.py -x -q` | W0 | ⬜ pending |
...
| 31-10/T2 | 31-10 | 3 | H-06 원자 요청 + dispatch 복구 + M-06 | T-31-38/39/40/42 | fail-closed env, 전용 timestamp dedupe | unit | `python -m pytest backend/tests/phase31/test_visual_request.py -x -q` | W0 | ⬜ pending |
| 31-10/T3 | 31-10 | 3 | H-01 enqueue-only + IaC + flag OFF | T-31-43/44 | 인라인 생성 0, ReportBatchItemFailures | lint+unit | `(cd backend && sam validate --lint) && python -m pytest backend/tests/phase31/test_visual_dispatch.py -x -q` | ✅ 수정 | ⬜ pending |
...
**Approval:** planner 2026-07-19 (structural replan — 리뷰 BLOCK 반영, task map 재작성)
