...
| **pdshape fault** | 58 → **46** (−12) | 변화(검출) | left_elbow 24.45°→**33.02°**(−5.3→−15.6) 등 (아래 표). ⚠ **판독 정정 (2026-07-22)** — "결함이 팔로 선명 귀속" 은 **철회**. shadow 재채점 실측상 −12 의 대부분은 기준모션 기질 비대칭(기준 18fps·PR 무보정 vs 학생 9fps·PR 보정)이다. 밀도만 맞춰도 **57.2 로 복귀**. 근거 = `32-15-ASYMMETRY-CHECK.md` §6 |
| **pdshape success** | 100 → **99** (−1) | 변화(검출) | left_knee 21.19° 신규 경계 활성(−1.4). ⚠ **판독 정정** — "95~100 엘리트 밴드 내" 는 결과적으로 맞지만 **원인은 위양성**이다. success 위양성 여유 = **−1.2°** (tol 20° 이미 초과) — 자세 결함이 아니라 기준모션 무보정 좌표가 만든 감점. `32-15-ASYMMETRY-CHECK.md` §5 |
