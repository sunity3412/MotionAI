...
## D-07 · 미해결 / belle 판단 대기

1. **인트로 배경** — Figma 는 다크레드 사진, CLAUDE.md §4·design.md §10 은 "다크 배경 금지".
   belle 의 "피그마를 따라줘"가 이 화면에도 적용되는지 실물 보고 판정 필요.
   (같은 충돌이 튜토리얼 화면에서도 미해결 — CONTINUE-2026-08-29)
2. **로그인 배경** — Figma 연분홍, design.md §5-1 은 "가입/로그인/서브 = 흰색(#FFFFFF)".
3. **카피 전체** — belle 이 "논의 후" 라고 명시. 특히:
   - 로그인 헤드라인 "희연님, 오늘도 한 발 더!" — 이름이 필요한데 **최초 로그인 전에는
     이름이 없다**. 폴백 문구 필요.
   - 가입 "3초만에 가입하고..." (마케팅 톤) vs 현재 앱의 기대설정 톤
     ("강사님을 대신하진 않아요") — 튜토리얼과 같은 계열의 충돌.
4. **약관 링크 목적지** — sunity.ai `/terms` 재사용 가능 여부 + 영상/자세 데이터 커버 여부.
5. **`credential-already-in-use`** — 게스트 기록을 버릴지 유지할지.
