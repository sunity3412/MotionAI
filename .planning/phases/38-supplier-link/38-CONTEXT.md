...
- **D-22:** (belle 09-26 오후, 이 세션) **UI 디자인은 반드시 Figma 파일을 참고한다** — `https://www.figma.com/design/jrdI7kp245HkPfLB0nclsz/모션-분석-디자인?node-id=0-1`. 공급자 페이지·마이 탭 한 줄·문구 정정 전부: 구현 전 Figma MCP `get_design_context` 로 아래 원형 노드를 실측하고(색·반경·타이포·치수), 문구는 Figma 원문 그대로(창작 금지), design.md 는 보조. 노드 이름은 자동생성명이라 **텍스트 내용으로 찾는다**(메모리 figma-search-by-text). Figma 원격 이미지 URL 은 앱에 넣지 않는다(7일 만료).
...
### 디자인 · 문구
- **Figma `jrdI7kp245HkPfLB0nclsz` 페이지 `0:1` "디자인 초안" — 공급자 페이지가 재사용할 원형 노드(09-26 스크린샷 대조, [확인])**
  - `1:960` 가입 — 소셜 로그인 버튼 4종(`1:977` "Google로 시작하기" = 흰 배경 1px 테두리 + G 아이콘, 전폭 둥근 사각) · 약관 동의 목록(`1:1064` "전체 동의" 테두리 박스 + 행: 체크박스 · `[필수]`(브랜드 빨강)/`[선택]`(회색) · 라벨 · 오른쪽 chevron · 선택 항목 아래 작은 회색 부연) · 하단 전폭 CTA "다음"(비활성 = 연분홍, 활성 = #FF4B33) · 상단 "STEP 01 / 02" 빨강 소문 라벨 + 큰 굵은 제목
  - `1:393` 영상 가져오기 — 선택 카드 2종(`1:406` "즉석 촬영" · `1:407` "앨범에서 선택": 아이콘 + 제목 + 회색 부제 + chevron, 반투명 흰 카드) · 하단 안내 알약(`1:399`, 빨강 "!" 아이콘 + 2줄 팁 — **정정 대상 문구 "측면 45°…3~10초"**) · 권한 거부 상태(`1:419` 제목 · `1:420` 본문 · `1:423` "설정으로 이동" · `1:424` "앨범에서 선택" 테두리 버튼 2개)
  - `1:428` AI분석 — 실패 상태 원형: 원형 X 아이콘 + 제목(`1:479` "영상 안에 사람이 보이지 않아요. 전신이 화면에 들어오게 촬영해주세요." 본문) + "촬영 TIP!" 카드(`1:482`·`1:483` — **정정 대상 "-측면 45°, 2~3m 거리"**) + CTA `1:486` "다시 분석하기" · 오류코드 칩(`1:458`) · 완료 상태(`1:498` "분석이 완료되었어요"). 이 화면들은 다크(의도된 예외)라 **공급자 페이지는 색만 라이트로 옮기고 구조(아이콘·제목·본문·팁 카드·CTA)만 가져온다**
  - `1:499` Group 53 — 검증 실패 다이얼로그 2종("용량이 너무 커요" · "지원할 수 없는 파일이에요"): 연분홍 카드, 빨강 "!" 아이콘, 굵은 제목, 회색 2줄 본문, 버튼 2개 "닫기"(흰 테두리) · "다른 파일 선택"(빨강 채움). **길이 3~90초 / 5~30초 실패는 이 형식으로**
  - `1:717` 메인화면 — 동작 목록 행(`1:743` "오늘 도전해볼 동작" 섹션 제목 + "전체보기 >" 회색 링크, 행 = 48pt 회색 사각 썸네일 + 굵은 이름 + 회색 부제(레벨·상태 "고급 새로 추가됨") + chevron, 구분선) · 빈 상태 회색 카드("분석을 2번 이상 하면 AI 그래프가 보여요") · 점선 테두리 STEP 카드("STEP 1 종목을 골라주세요" + 빨강 알약 CTA) · 하단 탭 4개(`1:754` "마이") · 흰 카드 radius 15 · 브랜드 그라디언트 헤더. **"내 동작" 카드 = 이 목록 행 형식, 첫 등록 안내 = STEP 카드 형식**
  - `1:646` 종목선택 — 선수 프로필 카드(`1:663`/`1:713` "정은지 선수", 풀블리드 사진 + 종목 밑줄 + 이름, `1:709` 이력 카드: 소속·경력·수상 목록 + 닫기). design.md §5-6 "내 공급자 카드"의 원형. **"내 코드" 카드는 이 카드의 이름 자리 아래에 코드 + 복사 버튼**
