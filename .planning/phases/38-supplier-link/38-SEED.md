...
## 업로드·촬영 기준 (belle 09-26 "아무 영상을 업로드할 수 없잖아? 길이·용량·촬영 각도는?")

**belle ○ (09-26)**: *"현재 정은지 영상 정도의 각도나 이정도면 괜찮지? 유튜브 보면 알겠지만 동작의 기교를 빼고 보면 각도와 거리가 거의 비슷한게 폴스포츠라 좀 괜찮을거야."* → **촬영 기준 = 정은지 기준 영상의 조건**(세로 · 고정 · 허리~가슴 높이 수평 · 폴 전체 + 전신 · 약 4~5m · 한 사람 · 밝은 실내). 각도 고정값(정면/45°)은 두지 않는다 — 폴스포츠 촬영은 원래 이 조건으로 수렴한다(belle 관측). 수요자 안내 문구는 "기준 영상처럼 찍으세요"로 통일. 고스트 오버레이·자세 인식 자동 시작은 끝 그림의 선택 항목. 각도 차이가 점수에 새는지는 시험 영상으로 잰다(Phase 34 앵글 분리).
...
## 추가 요구 (belle 09-26 "둘 다 오케이, 보관은 영구로 가자. 공급자 가이드도 잘 만들어주도록")
6. **길이 하한·상한** — 수요자 3~90초, 공급자 5~30초(콤보 60초). 앱과 링크 페이지에서 즉시 검사(`analyze.tsx` validate 에 duration 추가, IA 3-3 문구 재사용).
7. **문구 정정** — 앱 팁 "측면 45°, 2~3m"(`loading.tsx:519,546`), IA 3-2 "측면 45° / 2~3m", 실패 문구 "정면으로"(`loading.tsx:490`) → "기준 영상처럼 찍으세요(폴 전체와 전신이 들어오는 거리, 세로, 고정)". 촬영 가이드 문서 §2 "정면 우선"도 같은 말로.
8. **원본 영상 보관 영구** — `uploads/` 30일 만료 수명주기(버킷 설정, template 밖 `template.yaml:135` 주석)를 **해제**. `s3keys.py:3` 주석 정정. 기록 화면의 원본 재생이 30일 뒤 사라지지 않는다. 비용 30MB × 1,000편 ≈ 30GB ≈ $0.7/월. 동의분 연습 영상 접두사도 영구.
9. **공급자 가이드** — 링크 페이지 안의 "촬영 전 체크 5" + 상세 가이드 화면 + 문서 정본 `docs/supplier-guide.md`(지금 `docs/reference-capture-guide.md`는 운영자용·기술어라 공급자용 쉬운 말로 다시 쓴다, 운영자 절차 §6 은 그대로 둔다). design.md 톤(라이트·#FF4B33·Pretendard).
   내용: ① 촬영 조건 = 정은지 영상 조건, 예시 사진은 정은지 프레임 ② 동작 1개 = 영상 1개, 5~30초, 서 있는 자세에서 시작, 무음 ③ 폼 선언 4개를 왜 묻는지 한 줄씩 ④ 등록 뒤 보게 되는 것(재현성 점수, 실패 4형과 대처) ⑤ 권리·동의 설명(공급자 소유, 이용 허락, 철회) ⑥ 내 코드를 수강생에게 알려주는 법 ⑦ 자주 틀리는 것(너무 가까움 2~3m · 손 촬영 · 뒤에 사람 · 역광 · 음악 · 여러 동작 이어 찍기).
   벤치: 골프 온라인 레슨 가이드(삼각대·높이·조명·배경) · UNFORCE "테스트 촬영 먼저" · Onform 스튜디오 조명 [확인 §F-7]. 폴댄스 7곳엔 강사용 가이드 소스 없음 → 이것도 빈 자리.
...
- **수요자 본인 영상**: `uploads/` 는 30일 뒤 자동 삭제(현 정책, `s3keys.py:3`, `template.yaml:135`). 분석 결과(`results/` 각도·카드·확대 사진·비교 영상)는 남는다 → 30일 뒤 기록 화면에서 원본 재생만 사라진다. 비용은 작다(30MB × 1,000편 ≈ 30GB ≈ $0.7/월). **belle ○ (09-26): 영구.** 동의분 연습 영상은 별도 접두사에 영구.
...
6. (09-26) ~~촬영 기준·길이 구현·문구 정정·보관 기간~~ → 전부 belle ○: 길이 하한/상한 + 문구 정정은 이 phase 안 · 원본 보관 **영구** · 공급자 가이드 요구 추가. 아래 "추가 요구" 절.
