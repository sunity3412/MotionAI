...
**합성(이 파일) = 내가 만든 경계값에서 규칙이 작동하는지 본다(통제).
실물(`backend/evals/realfixture/replay.py`) = 내가 만들 수 없는 실제 분포에서 그 규칙이
무엇을 내는지 본다(대표성). 전자는 코드가 맞는지를, 후자는 답이 맞는지를 묻는다.**
둘 다 남는다 — 이 파일은 실물 하네스(quick-260802-czw)로 대체되지 않는다.
