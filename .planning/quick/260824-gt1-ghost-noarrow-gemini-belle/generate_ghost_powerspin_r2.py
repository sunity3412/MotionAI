...
GUIDE_R2 = (
    ...
    "'before' ghost of the two legs traced EXACTLY from the FIRST image, forming a "
    "NARROW silhouette that must NOT look like a split: the raised ghost leg lying "
    "almost along the pole, its thigh brushing the pole line with only a softly "
    "bent knee; the lower ghost leg with its thigh angled just slightly forward "
    "and its knee clearly BENT so the shin hangs straight down, the ghost foot "
    "ending roughly below the knee - the two ghost feet stay close to the pole "
    "line, the ghost legs never fanning out, never straightened into a diagonal, "
    "and never wider than the legs in the FIRST image - the two ghost legs must be "
