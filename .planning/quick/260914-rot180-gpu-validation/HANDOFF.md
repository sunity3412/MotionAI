...
- 세 필드는 **판정 규칙이 없다** — `scene_finder.py:197` 이 Gemini 응답을 그대로 통과시킨다.
  유일한 '규칙'은 프롬프트 안 한국어 산문(`scene_finder.py:67-84`).
...
- 지금 세 필드를 전부 False 로 하드코딩해도 **깨지는 것이 없다**(write-only).

**순서 제안**: 계약 3벌을 여는 것은 나중이다. 열어도 상수를 보여줄 뿐이다.
먼저 C 영역 TODO 7건에 실제 영상·라벨을 붙여 혼동행렬을 내야
"모델이 못 맞히는가 / 프롬프트가 못 묻는가"가 갈린다.
