> ## ★ 2026-09-17 정정 — 이 문서는 pdshape 1편만 잰 것이다
>
> 전수 측정(`.planning/quick/260917-hjy-reference-drift-all11/evidence/REFERENCE-DRIFT-ALL11.md`)이
> 아래 두 서술을 정정한다. **대조군 pdshape 은 byte 동일하게 재현됐다** — 이 문서의
> pdshape 수치는 옳다. 틀린 것은 그것을 라이브러리 전체로 일반화한 부분이다.
>
> - §3 "저장 기준은 8관절 중 5개가 26~52° 어긋나 있다 — 최대 2.6배" → **pdshape 1편의 성질**.
>   전수로는 88개 관절-기준 쌍 중 9개(10%)이고 그중 5개가 pdshape 하나다. 5편은 회전을
>   켜도 전 프레임·전 관절 차이 0.000000 이다.
> - §4 "회전 이전에 이미 드리프트가 있다" → 있으나 **크기가 허용오차 아래**다.
>   저장본 vs 오늘 파이프라인(회전 OFF) = 88쌍 중 20° 초과 **0개**, 최대 15.6°.
>   **기준 라이브러리는 낡지 않았다.** 재추출이 필요한 이유는 오직 회전이다.
> - §5 "회전 + 기준 11개 재추출은 한 묶음" → 묶음 요건은 유지. 단 실제 재추출 대상은 **6편**.
