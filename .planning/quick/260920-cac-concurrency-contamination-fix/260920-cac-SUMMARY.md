...
> ## ★★★★ 먼저 읽을 것 — 이 문서의 §11~§13 은 **뒤집혔다** (2026-09-20 오후)
>
> §14~§16 이 그 앞을 정정한다. 아래 §0~§13 을 인용하기 전에 §14 부터 읽어라.
>
> - **표본에 학생이 없었다.** §11~§13 이 "라이브 학생 78/875건" 이라 부른 것은
>   **정은지 fixture 영상 12편 안팎의 재분석**이다(동작당 고유 영상 correct 1편 + fault 1편).
> - **§12·§13 의 점수 이동은 기준 버전 불일치였다.** 저장 분석 871/875 는 2026-09-17
>   `rot180_v1` 기준 승격 **이전**인데 오늘의 기준과 비교했다.
> - **버전을 맞추면 정은지 본인 영상은 11동작 전부 100점**이고, §13 이 "죽었다"고 한
>   pdshape·elbow-twist 가 결함을 **가장 크게** 가른다(+37·+34).
> - **동작별 바닥의 출처** = 같은 원본 프레임에서 포즈 추정이 재현되지 않는 것.
>   크기는 **역립 프레임 비율**이 정한다(Spearman 0.891). 체형·촬영각도·시간격자·DTW 는 탈락.
> - **운영 실독**(§15): 정은지 본인 영상 358건 중 **46건(12.8%)이 100점 미만, 최저 10점.**
>
> §13 의 "배포 금지" 판정은 무너졌지만 **"된다"로 바뀐 것은 아니다** —
> 학생 영상이 이 축에 **한 번도 태워진 적이 없다.**
>
> ### ★ §17 (Pod 실측, belle 승인) — **위양성은 지금도 살아 있다. 다만 범인이 다르다**
> - 정은지 본인 파워스핀 영상이 오늘 **80점**. 범인은 `leg_extension`(ipsf_absolute)이고
>   **이 조사가 §11~§14 내내 쫓던 `reference_relative` 는 0건**이다.
> - 원인은 Gemini 가 10.6초 클립의 hold 를 **1.0초**라 답한 것. 같은 영상을 5번 새로 물으면
>   **8.0초(정답) 아니면 1.0초로 갈리고 중간값이 없다 — 2/5 가 틀린다.**
> - 범위는 좁다 — `power-spin` 에만 난다(다른 동작은 yaml 에 EXTEND 관절이 0개).
> - 수리안 1번(기하 창)을 저장 122건에 적용하면 **위양성 4건 제거 + 결함 8건 신규 검출**,
>   새 상수 0개. **belle 판정 대상.**
...
## 16. `[확인]` §6 의 캐시 미결이 닫혔다 — 그리고 그 대가를 적어둔다

§6 은 *"수리 이전에 캐시에 박힌 판정이 있다면 계속 재생된다. 착수 전에 라이브 doc 으로
실재 여부를 먼저 재야 한다"* 를 미결로 남겼다. 쟀다(읽기 13건).

```
gemini_cache 총 문서 = 170건
표본 12건 전수 : doc id = 해시 단독(64 hex) · motion_query = None
```

§8 수리가 doc id 를 `_doc_id(video_hash, motion_query)` 로 바꿨으므로 **옛 id 는 조회조차
되지 않는다** — 코드가 스스로 적어둔 그대로다(`technique_cache.py:366-367`
*"2026-09-20 이전 doc(id = 해시 단독)은 이제 읽히지 않는다"*). 이중 방어로 `motion_query`
불일치 검증도 걸려 있다(`:316-320`). **오염분이 재생될 경로가 없다.**

★ **대가** `[확인]` — 그래서 **캐시가 사실상 비었다.** 170건이 전부 죽은 데이터이고,
파일럿에서 도는 첫 분석들은 **전부 Gemini 를 새로 호출한다**(비용·지연). 이건 수리의
부작용이 아니라 의도된 결과다(질의를 안 담은 캐시를 재사용하면 안 되는 것이 맞다).
다만 실증 당일 지연·크레딧을 계산할 때 이 사실을 넣어야 한다.

`[미확인]` 죽은 170건을 지울지 둘지는 판정하지 않았다 — 저장 비용은 미미하고,
지우면 되돌릴 수 없다. **belle 판정 대상으로 남긴다.**

---

## 17. ★★★★ Pod 실측 — **위양성은 지금도 살아 있다. belle 이 08-31 에 × 친 그것이다**

§15-5 가 남긴 `[미확인]`("지금도 그런지는 표본이 2건이라 말할 수 없다")을 belle 승인으로 닫았다.

```
Pod      : lx8hfw9pn32gge · NVIDIA L4 24GB · $0.49/hr
코드      : commitSha a5ec1b08 (origin/main)
envFlags : ROT180_INVERSION_ENABLED=true · PR_INVERSION=true · RTMW_DETERMINISTIC=true
기준      : referenceRelease = rot180_v1
경로      : 앱과 동일 (upload-url → Firestore 문서 → S3 PUT → SQS → Lambda → Pod)
           정본 드라이버 backend/scripts/e2e_app_path.py 무수정
```

학생측·기준측이 **양쪽 다 rot180 으로 맞춰진 완전 현행 조합**이다 — §14-3 의 버전 불일치가 없다.

### 17-0. 판정

**`[확인]` 위양성은 현행이다.** 정은지 본인 파워스핀 영상이 오늘 **80점**을 받는다.
**`[확인]` 범인은 이 조사가 §11~§14 내내 쫓던 축이 아니다.** `reference_relative` 는 0건이고
(`angle` 차원 100), `ipsf_absolute` 의 `leg_extension` 이 −20 을 낸다.
**`[확인]` 기전은 채점 로직이 아니라 "언제를 보느냐"다.** 같은 각도 데이터로 창만 바꾸면 감점 0이다.
**`[확인]` 근본 원인은 Gemini 국면 인식이 10.6초 클립의 hold 를 1.0초라 답한 것이다.**
같은 영상을 캐시 우회로 5번 새로 물어보니 답이 **8.0초(정답) 아니면 1.0초(진입부)로 갈리고
중간값이 없다 — 2/5 가 틀린 쪽에 떨어졌다**(§17-7 B). 흔들림이 아니라 **이봉**이다.
**`[확인]` 범위는 좁다** — 오늘 위양성은 `power-spin` 에만 난다. 나머지 동작은 yaml 에
EXTEND 관절이 0개라 이 criterion 이 **발화할 수 없다**(§17-7 A).

### 17-1. 사슬 — 운영 함수로 라이브 수치를 정확히 재현했다

```
입력 : fixtures/phase15/power-spin/correct.mp4 = ref-power-spin 기준 영상 그 자체 (10.60초)
라이브: overallScore 80 · dimensionScores {angle:100, line:0, stability:84}
       감점 record 1건 = leg_extension 88.69 (−20.0, ipsf_absolute)
       mission.baselineDeviation 91.31 / targetValue 180.0  → 채점된 무릎 = 88.69도
```

Gemini 응답 원문(오늘 생성된 `gemini_cache/8a6f9f8f…__ref-power-spin`):

```
setup 0.0초(conf .88) · hold 1.0초(conf .92) · peak 1.2초(conf .90) · release 2.3초(conf .87)
→ 10.60초 클립의 네 국면이 전부 앞 2.3초(22%) 안에 뭉쳐 있다
```

`_hold_window_from_moments` 가 단일 hold 를 **±2초 창**으로 만들고(프레임 변환 fps **9.0 하드코딩**),
`dimensions._select_window` 가 그 안에서 안정 부창을 재선택한다. 운영 함수를 그대로 호출해 재현:

```
가설                    힌트 창      부창        무릎 R    line_score   leg_extension
오늘 Gemini hold 1.0초   (0, 27)   [13, 19)     88.7        0           −20.0   ← 라이브와 일치
옛  Gemini hold 8.0초   (54, 90)   [80, 89)    173.4       93             0
기준 doc execPeakS 7초  (45, 81)   [72, 81)    167.5       80             0
힌트 없음(기하 창)          —      [79,105)    171.6       91             0
```

**대안 셋이 전부 감점 0이다. 오늘의 답 하나만 깎는다.**
클립의 무릎은 0~70% 구간에서 굽고(평균 55~112도) **70~100% 에서 펴진다**(169~176도) —
즉 실제 홀드는 뒤쪽인데 Gemini 가 앞쪽을 짚었다.

### 17-2. ★ 같은 파일에 대해 인식기가 두 번 다르게 답했다

`gemini_cache` 에 그 영상 해시로 doc 이 **두 개** 있다:

```
옛  (gemini-3.7-flash) : setup 4.0 · hold 8.0 · peak 9.0 · release 10.5   → 감점 0 (line 93)
오늘(gemini-3.8-flash) : setup 0.0 · hold 1.0 · peak 1.2 · release  2.3   → 감점 −20 (line 0)
joint_expectations 는 두 doc 모두 동일(양 무릎 extend) — 바뀐 것은 타이밍뿐이다.
```

`[확인]` Flash 정본은 **2026-09-05 에 3.7 → 3.8 로 바뀌었다**(`85105aca`, config.py 단일 소스).
`[미확인]` **모델 교체가 원인이라고 단정하지 말 것** — 3.7 시절인 2026-08-31 에도 같은 위양성이
났다(`leg_extension=135.8(−20)`). 이 클립에 대해 모델당 표본이 1건씩이다.

`[확인]` 라이브 이력은 이것이 **복권**임을 보여준다 — power-spin correct 62건 중 `line` 차원 보유
49건인데 그중 **45건이 line 91(정답), 4건만 line 0**(8%). 오늘 첫 런이 그 8% 에 걸렸다.

### 17-3. ★★ 캐시가 복권을 **영구 판정**으로 바꾼다

run 2 이후 `stage=recognizer elapsed_ms=71`(첫 런 33.6초) — `gemini_cache` 적중이다.
같은 영상을 다시 올리면 **같은 hold 창을 물려받아 같은 점수가 나온다**(실측: 80·80·…).

- 좋은 면: 수강생이 같은 영상을 두 번 올려도 점수가 흔들리지 않는다.
- ★ 나쁜 면: **틀린 창도 그대로 물려받는다.** §8 수리(캐시 키에 질의 추가) 이전에는 재분석이
  새 추첨이었는데, 이제 **첫 추첨이 그 영상에 대해 고정된다.**
  §8 수리는 옳지만 이 부작용은 §8 에 적히지 않았다.

### 17-4. 이건 새 결함이 아니다 — 코드가 belle 의 × 를 이름으로 적어놨다

`dimensions._select_window` docstring (`dimensions.py:303-311`):

> quick-260831-gyk (belle 08-31 × 판정 **"아니 쫙 펴져 있어"**): Gemini 국면 창
> (hold moment ±2초)이 스핀 진입 전환부를 측정 창에 섞어 **파워스핀 정타에
> leg_extension -20 위양성** + line micro-bent 0점을 만들었다. 수리 = profile 창을
> verbatim 쓰지 않고 그 창 **내부에서** 안정 부창을 재선택한다.

그 수리는 오늘도 돌았다(부창 [13,19) 가 힌트 창 (0,27) 안에 있다). **그런데 못 막는다** —
힌트 창 **전체**가 진입부면 그 안의 어느 부창도 굽은 무릎이다.
08-31 수리는 "힌트 창이 홀드와 진입부를 **섞을 때**"를 고쳤지 "**통째로 빗나갈 때**"는 못 고친다.

### 17-5. `[확인]` 부수 결함 — 프레임 변환 fps 가 9.0 하드코딩

`_hold_window_from_moments` 의 `fps = 9.0` 은 `frame_extractor` 의 **목표값**이고 실측 rate 는
~9.96 이다([[fps-label-vs-actual-decimation-rate]] 와 같은 종류). 시각이 뒤로 갈수록 창이
앞당겨진다 — t=8초면 약 0.8초(8프레임) 일찍 잡힌다. 오늘 건의 주범은 아니지만 같은 자리에 있다.

### 17-6. ★ 정답이 이미 파이프라인 안에 있다 `[확인]`

`ref-power-spin` 기준 doc 의 `clipRange` = `prepStartS 0 · execStartS 0.5 · execPeakS 7 ·
landEndS 9.9`. 실제로 7초부터 무릎이 펴진다(163.7 → 175.6도). 파이프라인은 그 doc 을
**이미 가져온다**(`_match_reference_by_motion_id`).

`[확인]` 그런데 `clipRange` 는 **채점의 국면 지정에 안 쓰인다** — 소비처는
`segments.ref_boundary_frame`(shared-base DTW 경계)과 기준 영상 추출(`frame_extractor`),
그리고 앱 표시뿐이다. Gemini 가 그것과 6초 어긋난 답을 내도 **아무도 대조하지 않는다.**

`[미확인]` `clipRange` 도 Gemini 산출이다(`gemini/reference_extractor.py`, 등록 시 1회, Pro).
"정답"이 아니라 **안정적이고 대조 가능한 제2 소스**다. 그리고 학생 영상은 기준과 타이밍이
다르므로 기준 시각을 그대로 못 쓴다 — 기준시각→학생시각 사상이 필요하고 그 경로는 이미
계산돼 있다(`reference_dtw_match.path`).

### 17-7. 실측 표

**A. 정은지 본인 영상 10건 (앱과 동일 경로, 오늘 코드)**

```
동작           런   점수   line  angle  stab   감점 record
power-spin     #1    80      0    100    84   leg_extension=88.7(−20.0)
power-spin     #2    80      0    100    84   leg_extension=88.7(−20.0)
power-spin     #3    80      0    100    84   leg_extension=88.7(−20.0)
power-spin     #4    80      0    100    84   leg_extension=88.7(−20.0)
pdshape        #1   100      -    100    93   (없음)
pdshape        #2   100      -    100    93   (없음)
climb          #1   100      -    100    94   (없음)
climb          #2   100      -    100    94   (없음)
peter-pan      #1   100      -    100    97   (없음)
kip-up         #1   100      -    100    97   (없음)
```

★ **위양성은 power-spin 하나에만 있다.** 나머지 6건은 `line` 차원 자체가 없다 —
그 동작들의 yaml 에 EXTEND 관절이 0개라 `leg_extension` 이 **발화할 수 없다**(§10-1).
즉 오늘의 위양성은 **객관 EXTEND criterion 을 가진 유일한 동작**에서만 난다.
★ power-spin 4회가 완전히 동일한 것은 캐시 때문이다(§17-3) — 재현성이 아니라 고정이다.

**B. ★★ 같은 영상 5벌 — 캐시를 우회해 인식기를 5번 새로 불렀다**

`power-spin/correct.mp4` 를 ffmpeg **스트림 복사**로 5벌 만들었다(메타데이터 comment 만 다름).
프레임 316개·길이 10.54초·해상도 2160x3840 전부 동일, 해시만 다르다 → 캐시 미스 → 새 호출.
**달라지는 것은 인식기뿐이다.**

```
변형   점수  line   setup   hold   peak  release
#1      93    93     4.5    8.0    9.0    10.0
#2      80     0     0.3    1.0    1.2     2.2   ← leg_extension −20
#3     100    93     4.5    8.0    9.0      -
#4     100    93     5.5    8.0    9.0      -
#5      80     0     0.0    1.0    1.3     2.7   ← leg_extension −20

hold = 8.0초(정답, clipRange execPeakS 7 과 일치) 아니면 1.0초(진입부). **중간값이 없다.**
같은 영상의 점수 = 80 · 80 · 93 · 100 · 100
```

★ **인식기에 끌개가 둘 있고, 2/5 가 틀린 쪽에 떨어졌다.** 흔들림이 아니라 **이봉(bimodal)**이다.

`[미확인]` **비율은 못 박는다** — 5건이다(이항 95% 구간이 대략 5~85%). 라이브 이력은 8%
(4/49)였으니 **오르지 않았다고도, 올랐다고도 말할 수 없다.** 모델 교체(3.7→3.8, 2026-09-05)와
엮으려면 표본이 더 있어야 한다.

`[미확인]` 국면이 맞은 3건도 93·100·100 으로 갈린다 — 감점 record 는 0건인데 93 이 나왔다.
**제2의 변동원이 있다**(dimension 폴백 계열로 보이나 안 이었다).


### 17-8. ★ `[확인]` 수리안 1번의 파급을 저장 코퍼스로 미리 쟀다

"EXTEND criterion 은 힌트를 무시하고 기하 hold 창으로만 잰다"를 저장된 power-spin 분석
122건(line 차원 보유)에 그대로 적용해 봤다. EXTEND 관절은 yaml 이 못박은 양 무릎이라
추정이 필요 없다.

```
                        저장(힌트 창)              기하 창
correct (정은지 정타) n=49   line 중앙 91 · 0점 4건    line 91 (49/49) · leg_extension 0 (49/49)
fault   (일부러 실수) n=60   line 중앙  0 · 비0 8건    line  0 (60/60) · leg_extension −20 (60/60)

바뀌는 것:  위양성 4건 제거(0 → 91)  ·  결함 8건 신규 검출(69 → 0)  ·  정타 5건 정규화
            나머지 104건 무변동
```

**한쪽을 얻고 한쪽을 잃는 교환이 아니라 양방향 개선이다.** 새 상수 0개(힌트를 빼는 일이다).

`[미확인]` ★ **이 코퍼스는 영상 2편(정타 1 · 실수 1)의 재분석이다.**
49 와 60 은 표본 크기가 아니라 재분석 횟수다([[firestore-analyses-are-fixtures-not-students]]).
따라서 이것은 **두 영상 사이의 분리**이지 모집단 분리가 아니다. 학생 영상으로는 미측정이다.

`[미확인]` 그리고 `_select_window` 는 `line_score`·`extension_deviation` 외에도
대표 프레임 선택·`safety_flags` 국면 localize·`_hold_window_median_dict` 가 함께 쓴다.
**위 측정은 EXTEND criterion 에 한정한 좁은 판이다** — 함수를 통째로 바꾸면 파급이 더 넓다.

### 17-9. 수리 방향 — **belle 판정이 필요하다 (내가 고르면 커브핏이다)**

넷이 보이고 전부 "점수를 올리는" 방향이라 내가 임의로 고를 수 없다.

1. **기하 창 우선** — EXTEND criterion 은 힌트를 무시하고 `hold_window`(분산 최소)로만 잰다.
   yaml 의 `hold_moment` 정의("동작이 완성돼 정지한 지점")와 기하 정의가 일치한다.
   새 상수 0개. 비용 = Gemini 가 국면을 제대로 찍는 동작에서 국면 게이트를 잃는다.
2. **불일치 시 미방출** — 힌트 창과 기하 창의 EXTEND 관절 대표 각도가 크게 어긋나면 그
   criterion 을 안 낸다. 새 상수 1개. 이 criterion 은 **한쪽으로만 틀린다**(정타를 깎는다).
3. **기준 `clipRange` 와 대조** (§17-6) — 재료가 이미 다 있다. 새 임계 0개.
   다만 `clipRange` 필드와 yaml 국면명의 **대응 계약이 없다** — 그것부터 정해야 한다.
4. **Gemini 국면 인식 자체를 고친다** — 근본이다. `[확인]` 프롬프트는 읽었고 **문제가 없다** —
   *"hold: 동작이 완성되어 정지한 시점"* 으로 기하 정의와 같고 예시 timestamp 도 7.2초다.
   틀리는 것은 모델의 답이다. 다만 파서가 `timestamp_seconds >= 0` 만 검증하고
   **클립 길이 상한이 없다**(`gemini_moment_extractor.py:135`) — 범위 가드는 상수 0개다.

**그리고 이건 mode1 이다** — 파일럿 성공기준 2번(정은지 모션 불러와 비교)이 직격이다.

---

# §18 — mode3 에 고정 잣대를 걸었다 (배선 완료, 아직 안 켰다) · 2026-09-20

> 정본: `.planning/quick/260920-m3r-mode3-reference-axis/SUMMARY.md` · 커밋 `37da77fc`

## 18-1. belle 이 프레임을 바로잡았다

> *"mode3 학생 비교는 틀리고 맞고가 아니라 발전해냐 안했냐겠지?"*

나는 §16 까지 mode3 를 **정타/결함 분류**로 재고 있었다. 틀린 축이다. mode3 는
같은 사람의 두 판 사이에서 **나아졌나**를 읽는 자리다.

## 18-2. 왜 못 읽었나 — 잣대가 없어서다

`[확인]` mode3 종합점수 = **떨림 단독**. 두 갈래가 같이 막고 있었다:

1. 동작을 모른다고 보아 `line` 차원이 **구조적으로 안 생긴다**
2. 이전 영상 대비 유사도는 **일부러 종합에서 뺐다** — 주석에 이유가 박제돼 있다:
   발전하면 못하던 과거의 나와 **덜 비슷해져** 점수가 역전된다

★ **2번은 옳은 판단이다.** 유사도는 발전을 못 재는 축이 맞다. 다만 그것을 빼고 나니
mode3 에 **움직이지 않는 잣대가 하나도 안 남았다.** 기준 선수의 각도가 그 잣대다.

## 18-3. 배선은 새 계산이 아니라 **이미 있는 관문에 이미 있는 재료**였다

- mode3 는 이미 `_match_reference_by_motion_id()` 로 기준 doc 을 손에 쥐고
  `bodyNormalizationProfile` 한 필드만 쓰고 버린다. **각도는 이미 거기 있었다.**
- 감점 관문(`app.py:2906`)은 `if reference_dtw_match is not None and ...` —
  **모드 게이트가 아니라 데이터 게이트**다. 채우면 발화한다.

## 18-4. 좁게 잡았다 — 그 변수의 소비처가 여섯이다

`reference_dtw_match` / `reference_angles_for_veto` 를 mode3 에서 채우면 여섯이 같이
움직인다. 그중 **확대카드(fault-zoom)** 가 "지난 영상 대비"에서 "기준 선수 대비"로
조용히 바뀐다 — belle 이 승인한 것은 **채점 축**이지 카드 문법이 아니다(mode3=progress).
그래서 mode3 전용 변수 3개를 따로 두고 **감점 builder 한 곳에만** 넣었고, 그 경계를
구조 게이트로 상시 강제한다(확대카드 호출부에 일부러 새게 해 변이 검증 통과).

## 18-5. 화면이 채점 출처를 틀리게 말하지 않게 — 라벨도 같이

`[확인]` 계약은 *"Mode3 허용 scoringBasis = 정확히 4 값 … reference_motion 은 Mode1
전용, Mode3 에 들어오면 거짓 reference 비교 함의"* 로 못박혀 있었다. 그 전제
("Mode3 에는 reference 비교가 없다")가 **배선이 켜지면 거짓이 된다.**

절대트랙 라벨을 단 채 기준 비교 점수를 내는 것이 **TRUST-03 이 막으려던 바로 그것**
이라, 값을 6개로 늘리고 계약 4곳(`assemble.py` · `models.py` · `contract.md` ·
`analysis.ts`)을 함께 갱신했다. 라벨은 **의도가 아니라 산출을 따라간다** — 플래그가
켜져도 기준을 못 찾거나 정렬이 실패하면 종전 라벨이 붙는다.

## 18-6. 실측 — 운영 함수를 플래그 켜고 그대로
...
정타 vs 결함 분리(중앙):      +2점  →  +19점        기준 축 발화 32/32
```

`[확인]` **kip-up 은 이 축으로 안 고쳐진다**(±0, 결함판이 100점). 알려진 건 —
geometric split 측정이 keypoint saturate 로 confounded. 이 배선의 공이 아니다.

게이트 **4984 passed / 20 skipped / 0 failed**(기준 4965 + 신규 19) · 앱 tsc clean.

## 18-7. 아직 안 켰다 — 켜는 조건 셋

`[미확인]` **학생 영상은 이 축을 한 번도 안 탔다.** 위 수치는 전부 기준 영상·자기비교
코퍼스다. 영상이 오면 셋을 재고, 하나라도 안 되면 안 켠다:

1. 같은 학생의 두 영상이 **같은 기준에 일관되게** 붙나
2. 강사 ○× 가 발전 방향과 일치하나
3. 카메라 각도가 바뀌어도 견디나

켜는 법: Pod env `MODE3_REFERENCE_RELATIVE_ENABLED=1` — **배포 없이 스위치만.**

`[미확인]` 라이브에서 이 플래그를 켜고 돌린 적이 없다 — Pod 실행 로그 0건.

## 18-8. 이 배선이 §17-9 의 belle 판정 대기를 지운다

260920-stn §6 의 **(나)** — *"mode3 가 전건 미보유라 결과화면에 점수가 안 뜨는 상태로
실증에 들어가도 되는가"* — 는 이 플래그가 켜지면 **질문 자체가 사라진다.** mode3 가
인식된 동작에 대해 근거 있는 점수를 갖기 때문이다. 미인식 동작은 그대로 미보유다
(D-08 유지).
