...
`[확인]` **kip-up 만 ±0** — 결함판이 100점을 받는다.

`[정정]` 나는 이것을 "이 축이 kip-up 의 결함을 못 본다(split 측정 포화)"라고 적었다.
**틀렸다.** §5-1 에서 재보니 **축은 결함을 정확히 본다**(왼어깨 3.5→20.6도). 막힌 것은
축이 아니라 그 다음 두 칸(허용오차 20도 · 신뢰구간 억제)이었고, 발전 읽기는 §5-1 로
해결했다. 원인을 안 따라가고 아는 이름표를 붙인 것이 실수다.
