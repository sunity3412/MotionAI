...
## 1-f. 내 몫 — 높이 자의 바닥을 폴에서 잡기 (belle "너의 몫을 하라") → **안 됐다, 배선 0**

목표: 서 있는 프레임이 없는 영상(power-spin·peter-pan)에서도 높이를 재도록 바닥 = 폴 바닥.
- `[확인]` 폴 검출기(HoughPoleDetector)는 **선**(x·방향)만 내고 끝점을 버린다. 끝점을 살려 봤더니(`scripts/pole_base_proto.py`) 아래 끝이 0.68~0.74(영상마다 다름) —
  진짜 바닥판은 프레임의 ≈0.85. 선분이 바닥 위에서 끊긴다(사람·그림자·바닥 반사).
- `[확인]` 폴 열의 수직 에지 에너지로 연속 구간을 찾아도(`scripts/pole_extent_proto.py`) 위 0.375·아래 0.737 로 5편 전부 같다 — 그건 폴이 아니라 **창문 밝은 배경과 겹치는 구간**이다.
  흰 천장·흰 바닥 앞에서는 크롬 폴의 대비가 낮아 에지가 안 잡힌다.
- `[확인]` 단위(몸길이) 대체: 분절 최대합은 불안정(kip-up 서 있는 0.29 vs 0.45), 몸통 p90 은 같은 사람 안에서만 일정(서 있는 몸길이/몸통 ≈ 2.0~2.2, 서 있는 프레임이 진짜일 때).
