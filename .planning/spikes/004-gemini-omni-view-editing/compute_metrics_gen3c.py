...
입력: kpts/<name>.npz (원본) + kpts_gen3c/<name>.npz (GEN3C 합성)
축:
...
def main() -> None:
    ...
    for orig_npz in sorted((HERE / "kpts").glob("*.npz")):
        name = orig_npz.stem
        omni_npz = HERE / "kpts_gen3c" / f"{name}.npz"
        ...
        rows.append({
            ...
            "per_joint_mae": per,
            "bone_cv_orig": round(ocv, 3), "bone_cv_gen3c": round(mcv, 3),
            "bone_cv_ratio": round(mcv / ocv, 2) if ocv else None,
            "jerk_orig": round(oj, 4), "jerk_gen3c": round(mj, 4),
            "conf_orig": round(oconf, 2), "conf_gen3c": round(mconf, 2),
        })
    (HERE / "gate_out" / "gen3c_metrics.json").write_text(json.dumps(rows, ensure_ascii=False, indent=1))
    for r in rows:
        if r["status"] != "ok":
            print(f"{r['clip']:<20} {r['status']}")
        else:
            print(f"{r['clip']:<20} MAE={r['angle_mae_deg']:>5}° map={r['lr_mapping']:<6} "
                  f"boneCV {r['bone_cv_orig']}→{r['bone_cv_gen3c']} (x{r['bone_cv_ratio']}) "
                  f"jerk {r['jerk_orig']}→{r['jerk_gen3c']} conf {r['conf_orig']}→{r['conf_gen3c']}")
