...
### ★ 보고 규칙 — 관측과 진단을 섞지 말 것 (2026-09-20 신설)

belle: *"세션이 시작할 때마다 자꾸 틀렸다 그러면 뭐 어떻게 진행하라는 거여"*

그 꼬임의 원인은 **하나**다. 2026-09-19 인계서에서 정정된 4건이 전부 같은 실수였다 —
**숫자가 화면/점수에 닿는 마지막 한 칸을 안 따라가고 보고했다.**

```
stability 72→91 을 보고 "점수가 올랐다" → 그 차원은 종합 입력 밖이었다
RATE_MIN/MAX 라는 이름을 보고 "클램프다" → 위반 검출기였고 라이브에선 폐기된다
"저신뢰 → ... → 짝이 틀린다" 사슬을 가정   → 짝 선택은 그 입력을 안 쓴다
"라이브 39건 27.0~72.0"                    → 어떻게 나온 숫자인지 안 남겼다
```

**그래서 보고에 쓰기 전에:**

1. **숫자·상수는 소비처까지 따라가라.** 차원 점수(`dimensionScores`)와 화면 점수
   (`overallScore` = deduction final)는 **다른 물건**이다. "X가 움직였다"에서
   "그래서 점수가 움직였다"로 넘어가려면 그 사이를 코드로 이어야 한다.
2. **못 이었으면 이었다고 쓰지 마라.** `[확인]` / `[미확인]` 을 문장마다 붙인다.
   미확인을 빼는 게 아니라 **미확인이라고 적는 것**이 답이다.
3. **관측과 진단을 다른 절에.** 관측(잰 값·코드 사실)은 다음 세션이 승계해도 되고,
   진단(왜 그런가)은 승계 전에 재검증 대상이다. 섞어 적으면 다음 세션이 진단을
   사실로 물려받는다. → [[handoff-observation-not-diagnosis]]
4. **belle 관측은 정정 대상이 아니다.** 지금까지 틀린 것은 전부 belle 의 관측이 아니라
   내가 붙인 인과 설명이었다. belle 말을 의심하기 전에 내 설명부터 의심할 것.
