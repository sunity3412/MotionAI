...
- 라이트 테마 전용. 다크 모드 없음. (루트 CLAUDE.md §4 / design.md §10 — 다크 배경 금지)
...
라우팅: Expo Router (파일 기반, `src/app/`). 온보딩 스택 → `(tabs)` 그룹.
...
바텀 네비게이션      : 흰 배경 #FFFFFF, 활성 #FF4B33 / 비활성 #A5A5A5 (design.md §5-2)
CTA 버튼            : 전체너비, 높이 54pt, radius 13pt (design.md §5-3)
카드                : 배경 #FFFFFF, radius 15pt, 테두리 #D9D9D9 (design.md §5-4)
분석 로딩           : 단계별 메시지 (스피너 금지)
레벨 다이얼          : PanResponder로 원형 슬라이더 직접 구현
```

> 컬러/타이포/radius/spacing은 `src/theme/`에 토큰화됨. 하드코딩 금지, 토큰만 사용.
