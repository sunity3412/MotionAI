...
# 목표절 구분 상수·조립 함수 — sunity_shared.analysis.cue_text 로 순수 이동
# (quick-260808-jix). 렌더러(합성 비교 영상 자막)와 Polly 합성이 같은 모듈을
# 읽어 자막=음성 lockstep 이 구조로 보장된다. 기존 이름은 alias 로 유지 —
# 테스트·참조 무파손 (로직 문자 단위 동일, cue_text 모듈 참조).
from sunity_shared.analysis import cue_text as _cue_text

_GOAL_CLAUSE_PREFIX = _cue_text.GOAL_CLAUSE_PREFIX
_GOAL_CLAUSE_SEPARATOR = _cue_text.GOAL_CLAUSE_SEPARATOR
_cue_action_line = _cue_text.goal_clause_action_line
_coach_audio_speech_text = _cue_text.coach_audio_speech_text
...
def _process(bucket: str, key: str, uid: str, analysis_id: str) -> None:
    ...
    try:
        ...
        # 정한다 (상한 5, 하한 없음 — belle 2026-09-10 / quick-260910-u67).
