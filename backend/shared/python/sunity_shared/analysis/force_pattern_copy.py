...
_FORCE_PATTERN_COPY_DATA: dict[tuple[ForceSourceSignal, ModeContext], str] = {
    # 카피 톤 메모 (2026-06-10 belle 결정 정합):
    # 전문 용어 (코어/고관절/내전근/광배/견갑) → 일반 사용자 어휘 (몸 중심/엉덩이 관절/
    # 허벅지 안쪽/등 근육) 로 교체. 실증 테스트 시점 belle + 강사 + 수강생 함께 톤 검수.
    # 가능성 언어 유지 ("…모습이 보여요" / "…다시 살펴볼 수 있어요").

    # axis_tilt — 몸 중심이 옆으로 기울어지는 패턴
    ("axis_tilt", "mode1"): (
        "정은지 선수 기준 패턴과 비교했을 때, 몸 중심이 옆으로 기울며 중심 잡는 "
        "힘이 약해지는 모습이 보여요."
    ),
    ("axis_tilt", "mode3_first"): (
        "이번 첫 분석에서, 몸이 한쪽으로 기울면서 중심 잡기가 어려워지는 모습이 "
        "보여요."
    ),
    ("axis_tilt", "mode3_progress"): (
        "지난 영상 대비, 몸 중심이 더 흔들리는 모습이라 중심 잡기를 다시 살펴볼 "
        "수 있어요."
    ),
    # pelvis_drop — 골반이 한쪽으로 내려가며 윗몸이 균형을 맞추는 패턴
    ("pelvis_drop", "mode1"): (
        "정은지 선수 기준 패턴과 비교했을 때, 골반이 한쪽으로 내려가며 윗몸이 "
        "균형을 맞추려는 모습이 보여요."
    ),
    ("pelvis_drop", "mode3_first"): (
        "이번 첫 분석에서, 양쪽 골반 높이가 어긋나며 엉덩이 관절에 부담이 가는 "
        "모습이 보여요."
    ),
    ("pelvis_drop", "mode3_progress"): (
        "지난 영상 대비, 골반 좌우 차이가 커지는 모습이라 윗몸 균형을 다시 살펴볼 "
        "수 있어요."
    ),
    # late_contact — 다리가 봉에 닿는 타이밍이 늦어지는 패턴
    ("late_contact", "mode1"): (
        "정은지 선수 기준 패턴과 비교했을 때, 다리가 봉에 닿는 타이밍이 늦어지는 "
        "모습이 보여요."
    ),
    ("late_contact", "mode3_first"): (
        "이번 첫 분석에서, 다리가 봉에 잘 안 걸리며 허벅지 안쪽 잡는 힘이 풀리는 "
        "모습이 보여요."
    ),
    ("late_contact", "mode3_progress"): (
        "지난 영상 대비, 다리 접촉 타이밍이 더 늦어지는 모습이라 허벅지 안쪽 "
        "잡힘을 다시 살펴볼 수 있어요."
    ),
    # high_jitter — 자세 유지 중 몸 떨림 (부위 불명확)
    ("high_jitter", "mode1"): (
        "정은지 선수 기준 패턴과 비교했을 때, 자세 유지 중 몸 떨림이 늘어나는 "
        "모습이 보여요."
    ),
    ("high_jitter", "mode3_first"): (
        "이번 첫 분석에서, 정지 자세에서 몸이 잘게 떨리는 모습이 보여요."
    ),
    ("high_jitter", "mode3_progress"): (
        "지난 영상 대비, 자세 유지 떨림이 더 늘어나는 모습이라 호흡과 버티는 "
        "힘을 다시 살펴볼 수 있어요."
    ),
    # high_jerk — 동작이 부드럽지 않고 가속이 거친 패턴 (부위 불명확)
    ("high_jerk", "mode1"): (
        "정은지 선수 기준 패턴과 비교했을 때, 동작 흐름이 부드럽지 않고 거칠어지는 "
        "모습이 보여요."
    ),
    ("high_jerk", "mode3_first"): (
        "이번 첫 분석에서, 움직임이 갑자기 빨라졌다 느려졌다 하는 모습이 보여요."
    ),
    ("high_jerk", "mode3_progress"): (
        "지난 영상 대비, 동작이 더 거칠어지는 모습이라 부드러운 흐름을 다시 "
        "살펴볼 수 있어요."
    ),
    # abnormal_release — hold 중 봉 잡기가 풀리는 패턴 (등 근육 잡는 힘)
    ("abnormal_release", "mode1"): (
        "정은지 선수 기준 패턴과 비교했을 때, 자세 유지 중 봉 잡은 손이 풀리는 "
        "모습이 보여요."
    ),
    ("abnormal_release", "mode3_first"): (
        "이번 첫 분석에서, 자세 유지 중 봉에서 손이나 다리가 떨어지는 모습이 "
        "보여요."
    ),
    ("abnormal_release", "mode3_progress"): (
        "지난 영상 대비, 자세 유지 중 봉 잡기가 더 풀리는 모습이라 등 근육 잡는 "
        "힘을 다시 살펴볼 수 있어요."
    ),
}
...
2026-06-10 belle 결정 정합: 코어/고관절/내전근/광배 → 일반 사용자 어휘 교체.
