...
#
# 'whole_fanout_hybrid1' (quick 260705-g1d): 하이브리드 granularity 전환 — upper_body
# scope 호출만 worst-pose 정지 이미지 페어를 입력으로 받는다. 하이브리드는 upper scope
# 산출 분포를 바꾸므로 옛 'whole_fanout' 키 공간을 재사용하면 옛 full-video 분포의 결과가
# stale-hit 로 살아남는다(90d038f kip-up FP 이력과 동형) → 값 자체를 bump 해 구조적 분리.
...
def assess_fault_context_video(
    ...
    **입력만 full-video** 다.
    ...
    key = _build_fanout_key(
        [stills["student_idx"], stills["ref_idx"]] if stills else None
    )
    ...
    try:
        ...
        if stills is not None:
            try:
                ...
                # 다음 성공 run 의 still 결과와 섞인다(오염). 'fi-' bucket 으로 이동.
                key = _build_fanout_key(None)
                fallback_cached = cache.lookup_rich(key)
                if fallback_cached is not None:
                    log.info(
                        "VisionVetoCache rich hit (video fallback): %s", student_hash[:8]
                    )
                    tel = dict(fallback_cached.get("telemetry") or {})
                    tel["cacheKey"] = key
                    tel["cacheHit"] = True
                    fallback_cached["telemetry"] = tel
                    return fallback_cached
        result = _run_part_frame_fanout(
            client, ref_uploaded, student_uploaded,
            part_scopes=scopes, at_seconds=at_seconds,
            upper_still_handles=upper_still_handles,
        )
    except Exception as exc:  # noqa: BLE001 - 업로드/fan-out 실패 graceful (Pitfall 5)
...
def _run_part_frame_fanout(
    ...
    *,
    ...
    upper_still_handles: tuple | None = None,
) -> dict:
    ...
    upper_still_handles (quick 260705-g1d 하이브리드): (ref_img, student_img) 이미지
    핸들 페어. 존재 시 upper_body scope 호출만 이 핸들 + media_kind='image' 로 보내고,
    그 외 scope 는 기존 full-video 핸들 유지 — 근거는 상단 하이브리드 helper 블록 주석
    ...
    # upperGranularity — eval 이 upper scope 입력 형태를 관측하는 telemetry 마커.
    # upper_body 가 scopes 에 포함될 때만 기록 (미포함이면 의미 없는 라벨이라 미기록).
    if "upper_body" in scopes:
        telemetry["upperGranularity"] = (
            "still_frame" if upper_still_handles is not None else "video_fallback"
        )
...
def _call_gemini_comparison(
    ...
    media_kind (quick 260705-g1d 하이브리드): 'image' 일 때만 contents 라벨을 "정지
    이미지" 로 분기하고 프롬프트 끝에 이미지-모드 정합 문구 1개를 덧붙인다 — 하이브리드
    fanout 의 upper_body scope 전용. default 'video' 경로는 프롬프트/라벨 byte-동일
    ...
    # 이미지-모드 분기 (quick 260705-g1d) — 하이브리드 fanout 의 upper_body scope 전용.
    # default 'video' 경로는 라벨/프롬프트 byte-동일 유지(기존 캐시 무효화 0).
    if media_kind == "image":
        ref_label = "기준(정타) 정지 이미지:"
        student_label = "평가 대상(학생) 정지 이미지:"
        # 이미지-모드 정합 문구 — _build_comparison_prompt 본문의 "두 개의 영상"/"동작
        # 시작부터 끝까지 전 구간" 등 영상 전제 지시와 충돌하는 지점을 커버한다.
        prompt = (
            f"{prompt}\n\n입력 정정: 이번 호출의 두 입력은 영상이 아니라, 각 영상에서 "
            "결함이 의심되는 핵심 순간을 하나씩 골라낸 **정지 이미지 페어**입니다 "
            "(첫째 = 기준 정지 이미지, 둘째 = 학생 정지 이미지). 위 지시 중 '동작 "
            "시작부터 끝까지 전 구간을 보라'는 등 영상 구간 탐색 지시는 이 호출에는 "
            "적용되지 않으니 무시하고, 두 정지 이미지에 보이는 이 자세끼리 기준 대비 "
            "편차를 비교 판정하세요. 나머지 규칙(정타/사소차/촬영조건은 결함 아님, "
            "각도쌍 추정, differences[] 구조화)은 그대로 적용됩니다."
        )
    else:
        ref_label = "기준(정타) 영상:"
        student_label = "평가 대상(학생) 영상:"
    response = client.models.generate_content(
        model=DEFAULT_VISION_MODEL,
        contents=[
            ref_label,
