...
박제 정신:
  ...
  · [[gemini-latest-model-versions]] — `gemini-3.1-pro-preview` (suffix 박제), `gemini-3.8-flash`(2026-09-05 갱신).
...
DEFAULT_C_MODEL: str = "gemini-3.8-flash"
...
# ★2026-09-05 (quick-260905-mbd): Flash 를 3.7 → 3.8 로 올리면서 **3.7 도 같은 이유로
# 뺐다.** 근거 = 확대 카드 기계 눈 감사에서 판정이 갈린 3장을 5회 반복으로 세 모델 비교
# (3.7 / 3.8 / omni-1.1): 3.8 이 반복 안정성 최고(2/3 카드 5/5), omni 는 가장 흔들리고
# 최고가인데다 호출 형태도 다르다(/v1beta/interactions). belle 승인 "3.8로". 3.7 은 이제
# resolve_model 를 타면 ValueError 로 터진다(3.5 선례와 동형). 프로덕션 start_server.sh 는
# GEMINI_MOMENT_MODEL 을 이 값에서 읽어 가므로 Pod 재기동 시 자동 승계.
