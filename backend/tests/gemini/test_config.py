...
class TestDefaultModelConstants:
    ...
    def test_default_c_is_flash_with_pro_override(self) -> None:
        ...
        assert DEFAULT_C_MODEL == "gemini-3.8-flash"
...
class TestAllowedModels:
    def test_allowed_models_exact_two_members(self) -> None:
        ...
        assert ALLOWED_MODELS == frozenset({"gemini-3.1-pro-preview", "gemini-3.8-flash"})
...
class TestResolveModel:
    ...
    @pytest.mark.parametrize("retired", ["gemini-3.5-flash", "gemini-3.7-flash"])
    def test_c_override_rejects_retired_flash(self, retired: str) -> None:
        ...
        ★quick-260905-mbd — 3.7 도 같은 이유로 은퇴(3.8 갱신). 같은 자리에서 터져야 한다.
        """
        with pytest.raises(ValueError, match="ALLOWED_MODELS"):
            resolve_model("C", env_override=retired)
