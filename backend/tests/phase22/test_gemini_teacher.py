...
핵심 불변식:
  ...
  · 모델 string — 교사 gemini-3.1-pro-preview / judge = Flash 정본(DEFAULT_C_MODEL), 2.5 계열 0.
...
def test_model_strings():
    """교사=gemini-3.1-pro-preview / judge=Flash 정본(DEFAULT_C_MODEL), 2.5 계열 0."""
    assert gt.TEACHER_MODEL == "gemini-3.1-pro-preview"
    assert gt.JUDGE_MODEL == DEFAULT_C_MODEL
    assert "flash" in gt.JUDGE_MODEL  # judge 는 Flash 축 — Pro 로 새면 비용·속도 계약 위반
...
def test_judge_uses_judge_model():
    """judge 호출 모델 string = DEFAULT_C_MODEL (회귀 없음)."""
    client = _FakeClient(text="8")
    gt.judge_report(client, {"faults": []})
    assert client.models.calls[0]["model"] == DEFAULT_C_MODEL
...
def test_run_trial_batch_passes_row_motion_to_judge(tmp_path, monkeypatch):
    ...
    assert judge_call["model"] == DEFAULT_C_MODEL
