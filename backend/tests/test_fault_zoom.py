...
# ── criterion 별 crop 멤버 (quick-260802-gny) ────────────────────────────────
# split_angle 카드의 crop 멤버가 그 카드의 **드로잉 점 집합**보다 좁아서 생긴
# 두 증상(사이각 미드로잉 / leg_extension 과 같은 사진)을 가르는 계약.
# 표시 전용, 채점 무접촉.
...
# 실 doc 의 visionVeto.faultJoints 형상 — 8-keypoint 이름공간이라 **발목이 없다**.
_GNY_FAULT_JOINTS = ["left_hip", "right_hip", "left_knee", "right_knee"]


def _gny_unit(criterion: str, source: str, fault_joints=None) -> dict:
    units = fz.criterion_units_from_records(
        [{"criterion": criterion, "source": source, "points": -12}],
        _GNY_FAULT_JOINTS if fault_joints is None else fault_joints,
        _GNY_KISMAM_MAP,
    )
    assert len(units) == 1, f"{criterion}/{source} unit 1개 기대: {units}"
    return units[0]


def test_split_angle_crop_members_vision_branch():
    """계약 1행 — vision 분기에서도 발목이 crop 멤버에 든다.

    실 doc 의 split_angle record 는 전부 `source='vision'` 이고, 그 분기의 산출은
    `faultJoints 교집합 부위멤버` 다. faultJoints 는 vision veto 의 8-keypoint
    이름공간이라 발목을 **원리적으로** 담을 수 없다 — 추가 멤버를 분기 안에
    넣으면 실 doc 이 그대로 깨진 채 남는다.
    """
    joints = _gny_unit("split_angle", "vision")["joints"]
    assert "left_ankle" in joints and "right_ankle" in joints, joints


def test_split_angle_crop_members_geometry_branch():
    """계약 2행 — geometry 분기(mode3 seed 형상)에서도 같은 멤버."""
    joints = _gny_unit("split_angle", "geometry")["joints"]
    assert "left_ankle" in joints and "right_ankle" in joints, joints


def test_leg_extension_and_arm_extension_members_unchanged():
    """계약 3·4행 — 다른 criterion 은 두 분기 모두에서 불변.

    leg_extension 은 무릎 신전 criterion 이라 골반과 무릎이 계측 부위 전부이고,
    `angle_vs_reference__*` 는 단일 관절 계열이라 애초에 region 표를 안 탄다.
    """
    for source in ("vision", "geometry"):
        assert _gny_unit("leg_extension", source)["joints"] == tuple(
            fz.REGION_MEMBERS["legs"]
        ), source
        assert _gny_unit("arm_extension", source)["joints"] == tuple(
            fz.REGION_MEMBERS["arms"]
        ), source
    single = _gny_unit(
        f"{fz.ANGLE_VS_REFERENCE_PREFIX}left_knee", "geometry"
    )["joints"]
    assert single == ("left_knee",), single


def test_split_angle_unit_joint_order_contract():
    """계약 7행 — joints[0] 은 left_hip, 발목은 **끝 2칸**.

    `_CropUnit.joint = joints[0]`(fault_zoom.py 2524행)과 `_anchor_xy` 기본값
    `valid[0][1]`(1150행)이 첫 멤버를 쓴다 — 앞에 끼워 넣으면 카드 kind 조회와
    앵커 대표 관절이 조용히 바뀐다.
    """
    for source in ("vision", "geometry"):
        joints = _gny_unit("split_angle", source)["joints"]
        assert joints[0] == "left_hip", (source, joints)
        assert joints[-2:] == ("left_ankle", "right_ankle"), (source, joints)
        assert len(joints) == len(set(joints)), joints


def test_split_angle_crop_box_unchanged_without_ankles():
    """계약 5·6행 — fail-closed 무회귀. 발목이 못 쓰는 좌표면 box 가 완전 동일.

    (a) 발목 좌표가 아예 없는 8관절 report, (b) 발목 conf 가 `_KP_CONF_MIN`
    미만인 report 두 경우 모두, 6멤버 호출의 `_side_crop` box 가 4멤버 호출과
    튜플 동등이어야 한다 (근사 비교 금지). 기하는 재계산하지 않고 프로덕션
    `_member_pts` / `_side_crop` 을 그대로 부른다.
    """
    frame = _frames(1, h=640, w=360)[0]
    four = tuple(fz.REGION_MEMBERS["legs"])
    six = four + ("left_ankle", "right_ankle")

    def box_for(rep, members):
        valid, relaxed = fz._member_pts(rep, 0, members)
        return fz._side_crop(
            frame, [xy for _n, xy in valid], relaxed,
            anchor=fz._anchor_xy(valid, None) if valid else None,
        )[3]

    # (a) 8관절 report — 발목 키 자체가 없다.
    rep8 = _report_pos_conf(1, 9.0, _LEGS_XY, {})
    assert box_for(rep8, six) == box_for(rep8, four)

    # (b) 발목 좌표는 있으나 저신뢰 — relaxed 로 분류되고 valid box 는 그대로.
    xy12 = {**_LEGS_XY, "left_ankle": (0.2, 0.95), "right_ankle": (0.8, 0.95)}
    rep_low = _report_pos_conf(
        1, 9.0, xy12,
        {"left_ankle": fz._KP_CONF_MIN - 0.01, "right_ankle": fz._KP_CONF_MIN - 0.01},
    )
    assert box_for(rep_low, six) == box_for(rep_low, four)

    # 대조군: 고신뢰 발목이면 box 가 실제로 넓어진다 (위 두 동등이 "발목을 아예
    # 안 쓴다"가 아니라 fail-closed 때문임을 보이는 짝).
    rep_hi = _report_pos_conf(1, 9.0, xy12, {})
    assert box_for(rep_hi, six) != box_for(rep_hi, four)


def test_mode3_zoom_extra_members_mirror():
    """계약 8행 — 파이썬 2표 동등성 (드리프트 자물쇠).

    pipeline 미러가 조용히 어긋나면 mode1 카드만 고쳐지고 mode3 는 구 동작으로
    남는다. 세 표를 항목 단위로 묶어 둔다.
    """
    import importlib.util
    from pathlib import Path as _Path

    path = _Path(__file__).resolve().parents[1] / "functions" / "pipeline" / "app.py"
    spec = importlib.util.spec_from_file_location("gny_pipeline_mirror", str(path))
    mod = importlib.util.module_from_spec(spec)
    sys.modules["gny_pipeline_mirror"] = mod
    spec.loader.exec_module(mod)

    assert mod._MODE3_ZOOM_CRITERION_EXTRA_MEMBERS == fz.CRITERION_CROP_EXTRA_MEMBERS
    assert mod._MODE3_ZOOM_REGION_MEMBERS == fz.REGION_MEMBERS
    assert mod._MODE3_ZOOM_CRITERION_REGION == fz.CRITERION_REGION
