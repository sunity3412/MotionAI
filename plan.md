...
*2026-05-29 — 실분석 가동(RunPod Blackwell) + IPSF 채점 재설계 (균형 제거·기술조건부):
 ...
 ⑤ 게이트 실행 결과 (Pod 새 코드로 정은지 영상 재분석): overall 41→60, '균형 15'
    항목 사라짐 확인 ✓ (balance 제거 1차 통과). 그러나 점수 신뢰도는 아직 미해결 —
    화면이 드러낸 3가지:
    - (a) 라인이 None 으로 빠짐 → overall 이 사실상 '안정성 60' 한 차원으로 결정됨.
      fallback 이 굽은 홀딩 포즈에서 EXTEND 관절을 못 찾음(그립 자세). → 인식기 필요 증명.
    - (b) '각도 정확도 100' 아티팩트 — mode3 2회차가 *같은 영상(이전 업로드)* 과 비교라
      당연히 100. "각도 정확도"가 아니라 "지난 영상 대비 일관성"이고 같은 영상이면 무의미.
      라벨/로직 수정 필요(같은·유사 영상 케이스 처리).
    - (c) overall 이 차원 1개에 휘둘림(라인 빠지면 안정성만) → 구성 재고 필요.

 다음 (재개 시):
   1. ★ Gemini 기술 인식기 어댑터 (GeminiTechniqueRecognizer) — 핵심 레버.
      영상 프레임 → 기술 인식 → 관절별 EXTEND/BENT 판정 → 라인이 안정·의미있게.
      필요: belle Gemini API 키(Google AI Studio). Pod env/Parameter Store 에 주입.
   2. (키 없이 가능) 각도 100 아티팩트 처리 (b) + overall 구성 취약 (c) 수정.
   3. 통과 후 확장: 다른 영상·비폴 영상(not_pole 거부) 검증.
   - 카메라 앵글 합성(CameraCtrl II/UCPE)=데이터 증강/시점보정/코치뷰 = Phase 2.
   - 측정 차원 확장(정렬/자세)은 포즈 데이터 업그레이드(toe/head keypoint) = Phase 3.
